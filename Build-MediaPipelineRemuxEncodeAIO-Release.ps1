...
$script:SourceRoot = if ($PSScriptRoot) { $PSScriptRoot } else { Split-Path -Parent $MyInvocation.MyCommand.Path }
$script:SourceRoot = [System.IO.Path]::GetFullPath($script:SourceRoot)
$releasePolicyModule = Join-Path $script:SourceRoot 'Pipeline\Modules\ReleasePolicy.ps1'
...
if ($Verify) {
    $releaseVerifier = Join-Path $destinationFull 'Test-MediaPipelineRemuxEncodeAIO-Release.ps1'
