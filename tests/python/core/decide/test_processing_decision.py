...
from mediapipeline.core.decide.processing_decision import REQUIRED_REASON_CODES
...
class ProcessingDecisionTests(unittest.TestCase):
    ...
    def test_resolution_aware_bitrate_buckets_select_thresholds(self) -> None:
        ...
        cases = [
            ("1080p under 20", load_source("tv_h264_1080p_12mbps_mkv.json"), EffectiveDecisionPolicy(), "REMUX", 20.0, "1080p"),
            ("1080p over 20", load_source("tv_h264_1080p_24mbps_mkv.json"), EffectiveDecisionPolicy(), "ENCODE", 20.0, "1080p"),
            ...
            ("in-between uses 1440p cap", between_under, policy, "REMUX", 35.0, "1440p"),
        ]
    ...
    def test_resolution_bitrate_boundaries_route_exact_line_to_higher_bucket(self) -> None:
        policy = EffectiveDecisionPolicy(
            route_1080p_max_video_bitrate_mbps=21,
            route_1440p_max_video_bitrate_mbps=31,
            route_4k_max_video_bitrate_mbps=41,
            movie_route_size_limit_gb=80,
        )
        cases = [
            (1200, "1080p", 21.0),
            (1201, "1440p", 31.0),
        ...
        for height, expected_bucket, expected_cap in cases:
            with self.subTest(height=height):
                source = load_source_variant(
                    "movie_hevc_4k_hdr_high_bitrate_mkv.json",
                    height=height,
    ...
    def test_resolution_output_size_targets_follow_movie_tv_height_buckets(self) -> None:
        policy = EffectiveDecisionPolicy(
            route_threshold_mode="size",
            movie_route_size_limit_gb=50,
            tv_route_size_limit_gb=25,
            movie_route_1080p_size_limit_gb=7,
            movie_route_1440p_size_limit_gb=11,
            movie_route_4k_size_limit_gb=17,
            tv_route_1080p_size_limit_gb=2,
            tv_route_1440p_size_limit_gb=5,
            tv_route_4k_size_limit_gb=9,
            route_1080p_max_video_bitrate_mbps=100,
            route_1440p_max_video_bitrate_mbps=100,
            route_4k_max_video_bitrate_mbps=100,
            h264_direct_copy_max_bitrate_mbps=100,
        )
        cases = [
            ("movie", 1080, "1080p", 7.0, "source_height_bucket_movie"),
            ("movie", 1440, "1440p", 11.0, "source_height_bucket_movie"),
            ("movie", 2160, "4k", 17.0, "source_height_bucket_movie"),
        ...
        for media_type, height, expected_bucket, expected_size, expected_source in cases:
            with self.subTest(media_type=media_type, height=height):
                fixture = "tv_h264_1080p_12mbps_mkv.json" if media_type == "tv" else "movie_h264_1080p_30mbps_mkv.json"
                source = load_source_variant(
                    fixture,
                    height=height,
                ...
                self.assertEqual(decision.source_facts_used["route_size_limit_bucket"], expected_bucket)
