...
ENTRYPOINT = PROJECT_ROOT / "ops" / "pipeline" / "engine" / "entrypoint.ps1"
BUNDLED_PWSH = PROJECT_ROOT / "ops" / "pipeline" / "runtime" / "PowerShell-7.6.0-win-x64" / "pwsh.exe"
BUNDLED_FFMPEG = PROJECT_ROOT / "ops" / "pipeline" / "tools" / "ffmpeg" / "bin" / "ffmpeg.exe"
...
class StageEntrypointTests(unittest.TestCase):
    ...
    def test_probe_stage_fails_closed_when_bundled_ffprobe_is_missing(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="mediapipeline-stage-missing-tool-") as tmp:
            ...
            shutil.copytree(PROJECT_ROOT / "ops" / "pipeline" / "engine", temp_engine)
            env = os.environ.copy()
