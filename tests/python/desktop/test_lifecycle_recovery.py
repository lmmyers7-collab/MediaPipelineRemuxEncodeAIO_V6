...
class LifecycleRecoveryTests(unittest.TestCase):
    ...
    def test_recovery_parks_media_mutation_without_proven_boundary(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            store = LifecycleLeaseStore(root, pid_alive=lambda _pid: False)
            lease = store.acquire(scope="Pipeline start", command_id="command-1")
            lease.set_recovery_descriptor(route="/api/pipeline/start", request={"mode": "once"})
            status = LifecycleRecoveryCoordinator(store_factory=lambda state_root: LifecycleLeaseStore(state_root, pid_alive=lambda _pid: False)).run(SimpleNamespace(state_root=root), resume=lambda *_args: {"ok": True})

            self.assertEqual(status["status"], "blocked")
            self.assertEqual(status["classification"], "parked")
