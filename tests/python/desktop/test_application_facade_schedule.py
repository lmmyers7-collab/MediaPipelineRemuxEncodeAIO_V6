...
from tests.python.desktop.application_facade_test_support import DummyFacadeService, DummyProc, _resolved
...
class ApplicationFacadeScheduleTests(unittest.TestCase):
    def test_schedule_workspace_reads_persisted_app_state_without_saving(self) -> None:
        ...
        self.assertIn(schedule["evaluation"]["current_day"], {"Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"})
        self.assertIsInstance(schedule["evaluation"]["current_block_index"], int)
        self.assertIsInstance(schedule["evaluation"]["current_block_start"], str)
    ...
    def test_schedule_preview_and_save_use_backend_app_state_contract(self) -> None:
        ...
        self.assertIn("current_grid", preview.data)
        ...
        self.assertIn("current_grid", saved.data)
        self.assertFalse(saved.data["current_grid"]["Monday"][18])
        self.assertFalse(saved.data["current_grid"]["Tuesday"][0])
