...
class StatusServiceContractTests(unittest.TestCase):
    ...
    def test_build_current_work_cleans_noisy_movie_progress(self) -> None:
        ...
        self.assertEqual(current_work["queue_position_label"], "item 3 of 17")
        self.assertEqual(current_work["route_label"], "Remux route")
        self.assertEqual(current_work["percent_label"], "94%")

    def test_build_current_work_uses_saved_movie_cleaning_policy(self) -> None:
        service = DummyStatusService()
        with tempfile.TemporaryDirectory() as raw_root:
            ...
            hoppers = service._build_current_work(
                {
            ...
            iron_lung = service._build_current_work(
                {
                    "CurrentFileDisplay": "Iron.Lung.2026.1080p.WEBRip.x265.6CH-SupaCvnt.mkv",
                    "CurrentMediaType": "movie",
                },
                resolved,
            )

        self.assertEqual(hoppers["item_label"], "Hoppers (2026)")
        self.assertEqual(iron_lung["item_label"], "Iron Lung (2026)")
