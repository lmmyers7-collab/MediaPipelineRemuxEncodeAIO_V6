...
class ApplicationFacadePendingPublishTests(unittest.TestCase):
    def test_pending_publish_preview_uses_existing_scan_service(self) -> None:
        ...
        self.assertEqual(preview["file_inventory"]["schema_version"], "desktop_pending_publish_file_inventory.v1")
        self.assertEqual(preview["file_inventory"]["total_count"], 1)
        self.assertEqual(preview["file_inventory"]["payload_like_count"], 1)
        self.assertEqual(preview["file_inventory"]["orphan_payload_count"], 1)
        self.assertEqual(preview["file_inventory"]["rows"][0]["path"], str(payload))
        self.assertEqual(preview["file_inventory"]["rows"][0]["role"], "orphan_payload")
        self.assertIn("directory listing only", "\n".join(preview["file_inventory"]["summary_lines"]))
        ...
        self.assertEqual(preview["drain_confidence"]["schema_version"], "desktop_pending_drain_confidence.v1")
        self.assertEqual(preview["drain_confidence"]["evidence_authority"], "backend")
        self.assertFalse(preview["drain_confidence"]["render_complete"])
        confidence_checks = {row["check"]: row for row in preview["drain_confidence"]["rows"]}
        self.assertEqual(confidence_checks["Current parked rows"]["confidence"], "review")
        self.assertEqual(confidence_checks["Blocker evidence"]["confidence"], "review")
        self.assertEqual(confidence_checks["Validation checklist"]["confidence"], "blocked")
        self.assertEqual(confidence_checks["Durable drain summary"]["confidence"], "unknown")
    ...
    def test_pending_publish_preview_classifies_unreadable_manifest_for_operator_review(self) -> None:
        ...
        self.assertEqual(preview["file_inventory"]["manifest_count"], 1)
        self.assertEqual(preview["file_inventory"]["rows"][0]["role"], "manifest")
    ...
    def test_publish_reconciliation_correlates_completed_pending_and_drain_summary(self) -> None:
        ...
        proof = preview["completed_pending_proof"]
        self.assertEqual(proof["schema_version"], "desktop_completed_pending_proof.v1")
        self.assertEqual(proof["evidence_authority"], "backend")
        proof_signals = {row["signal"] for row in proof["rows"]}
        self.assertIn("pending-destination-overlap", proof_signals)
        self.assertIn("completed-source-still-pending", proof_signals)
        self.assertIn("drain-summary-output-proof", proof_signals)
        self.assertIn("completed-missing-output-still-pending", proof_signals)
        self.assertIn("completed-missing-output-with-drain-proof", proof_signals)
        self.assertIn("completed-missing-output-no-pending-proof", proof_signals)
        self.assertIn("Completed-to-Pending output proof cross-check:", "\n".join(proof["summary_lines"]))
        self.assertIn("First action: missing completed outputs have no exact pending/drain proof.", "\n".join(proof["summary_lines"]))
        no_proof = [row for row in proof["rows"] if row["signal"] == "completed-missing-output-no-pending-proof"][0]
        self.assertIn("empty Pending Publish page is not proof", no_proof["safe_next_action"])
