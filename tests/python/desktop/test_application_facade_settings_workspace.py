...
class ApplicationFacadeSettingsWorkspaceTests(unittest.TestCase):
    def test_settings_workspace_redacts_tokens_and_reports_validation(self) -> None:
        ...
        scratch_reserve_field = next(field for field in settings["field_definitions"] if field["key"] == "MinFreeSpaceGB")
        self.assertEqual(scratch_reserve_field["label"], "Scratch Free Space Reserve (GB)")
        self.assertEqual(scratch_reserve_field["advanced_visibility"], "standard")
        self.assertEqual(scratch_reserve_field["scope"], "global_only")
        self.assertEqual(scratch_reserve_field["default"], 50)
        output_reserve_field = next(field for field in settings["field_definitions"] if field["key"] == "OutsourceMinFreeSpaceGB")
        self.assertEqual(output_reserve_field["label"], "Output Free Space Reserve (GB)")
        self.assertEqual(output_reserve_field["advanced_visibility"], "standard")
        self.assertEqual(output_reserve_field["scope"], "global_only")
        self.assertEqual(output_reserve_field["default"], 50)
