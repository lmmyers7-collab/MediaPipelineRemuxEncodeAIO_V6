...
class LocalApiStaticFilesPolicyTests(unittest.TestCase):
    ...
    def test_render_index_expands_webview_page_partials(self) -> None:
        ...
        self.assertEqual(body.count('id="settings-save-review-dialog"'), 1)
        self.assertIn('id="settings-audio-builder-status"', body)
        self.assertIn("Prepares audio builder values for Save Settings", body)
        self.assertIn("Save Settings still runs backend validation", body)
    ...
    def test_render_static_includes_rejects_unsafe_paths(self) -> None:
        with tempfile.TemporaryDirectory() as raw_root:
            ...
            with self.assertRaises(ValueError):
                render_static_includes("<!-- mp-include: assets/mediapipeline.core.html -->", root)
