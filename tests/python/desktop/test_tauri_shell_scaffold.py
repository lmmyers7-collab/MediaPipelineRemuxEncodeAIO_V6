...
class TauriShellScaffoldTests(unittest.TestCase):
    ...
    def test_tauri_shell_rejects_second_instance_and_exposes_read_only_lifecycle_banner(self) -> None:
        ...
        self.assertIn("$expectedPermissions", audit)
        self.assertIn("allow-open-pipeline-log-window", audit)
    ...
    def test_tauri_shell_validates_backend_served_webview_before_opening_window(self) -> None:
        ...
        self.assertIn('"/assets/settings/finalLibraryPromotion.js"', source)
        self.assertIn('"/assets/settings/patchOverview.js"', source)
        self.assertIn('"/assets/settings/patchInteractions.js"', source)
        self.assertIn('"/assets/settings/patchReadiness.js"', source)
        ...
        self.assertIn('"/assets/launch/preflight/pilotReadiness.js"', source)
        ...
        self.assertIn('"/assets/launch/commandOrchestration.js"', source)
        self.assertIn('"/assets/launch/rerunEvidence.js"', source)
        self.assertIn('"/assets/launch/rerunPresentation.js"', source)
        self.assertIn('"/assets/launch/rerunOrchestration.js"', source)
        self.assertIn('"/assets/launch/rerunFacade.js"', source)
        ...
        self.assertIn('"/assets/pendingPublish/confidence/postDrainTrust.js"', source)
    ...
    def test_tauri_shell_requests_graceful_backend_shutdown_before_kill(self) -> None:
        ...
        self.assertIn("Authorization: Bearer {token}", source)
        self.assertIn("resolve_shutdown_with_child(", source)
        self.assertIn("ShutdownRequestState::Acknowledged", source)
        self.assertIn("ShutdownRequestState::TransportFailed", source)
        self.assertIn("if resolution.release_ownership", source)
        self.assertIn("terminate_tree_and_verify", source)
        self.assertIn("backend shutdown request failed", source)
        self.assertIn("child wait API failed", source)
        self.assertIn("ownership was retained", source)
        self.assertIn("descendant exit remains unverified", source)
    ...
    def test_tauri_shell_uses_safe_only_shutdown_for_unconfirmed_exit_paths(self) -> None:
        ...
        self.assertIn("RunEvent::ExitRequested { api, .. } =>", source)
        self.assertIn("api.prevent_exit()", source)
        self.assertIn("BackendShutdownOutcome::Blocked | BackendShutdownOutcome::Failed", source)
        self.assertIn("RunEvent::Exit =>", source)
        self.assertIn("shutdown_backend_state(app_handle, BackendShutdownMode::SafeOnly)", source)
        self.assertIn("CloseRequestDecision::AllowConfirmedForce => {", source)
        self.assertIn("BackendShutdownMode::ConfirmedForceActiveWork", source)
    ...
    def test_tauri_webview_uia_probe_does_not_launch_media(self) -> None:
        ...
        self.assertIn("[string[]]$ExpectedHomeNames = @()", source)
        self.assertIn("[string[]]$RejectedHomeNames = @()", source)
        self.assertIn("[switch]$HomeOnly", source)
        self.assertIn("Missing expected Home name", source)
        self.assertIn("Rejected raw Home name", source)
        self.assertIn("Accepted workload", source)
