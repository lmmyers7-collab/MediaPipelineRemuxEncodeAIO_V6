    ...
    return browser_cdp_runner_prelude() + textwrap.dedent(
        ...
        function completedPendingProofScript(data) {
          ...
          (async () => {
            ...
            const getCompletedPendingProofRows = () => {
              const completedView = window.mediaPipelineCompletedView || {};
              return typeof completedView.getLastCompletedPendingProofRows === "function" ? completedView.getLastCompletedPendingProofRows() : [];
            };
            await waitFor(
            ...
            const currentCompletedRows = window.mediaPipelineCompletedView.completedCurrentRows(payload.completed.rows || []);
            ...
            window.mediaPipelineCompletedView.renderCompleted(sameLeafCompleted);
            window.selectPendingRow(selectedPending);
            requireText("pending-detail", [
              "Completed Manifest correlation for selected pending row:",
            ...
            window.mediaPipelineCompletedView.renderCompleted(missingDrainedCompleted);
            ...
            const drainedPlacementLabels = (missingDrainedCompleted.rows || []).map((row) => window.mediaPipelineCompletedView.completedOutputPlacement(row, getCompletedPendingProofRows()).label);
            ...
            window.mediaPipelineCompletedView.renderCompleted(brokenCompleted);
            ...
            window.mediaPipelineCompletedView.renderCompletedRealMediaProof(brokenCompleted, brokenCompleted.rows, getCompletedPendingProofRows(), { rows: [], count: 0 });
            window.mediaPipelineCompletedView.renderCompletedFinalTrust(brokenCompleted, brokenCompleted.rows, getCompletedPendingProofRows(), { rows: [], count: 0 });
