...
def _node_runner_source() -> str:
    return textwrap.dedent(
        ...
        const priorityRows = [0, 1, 2].map((index) => Object.assign({}, payload.queue.rows[0], {
          row_key: `priority-smoke-${index + 1}`,
          global_order: index + 1,
          queue_index: index + 1,
          display_name: `Priority Smoke ${index + 1}`,
          relative_path: `Priority/Smoke ${index + 1}.mkv`,
          source_path: `C:/Source/Priority/Smoke ${index + 1}.mkv`,
        ...
        if (!selectedPriorityNames.includes("Priority Smoke 1") || !selectedPriorityNames.includes("Priority Smoke 2")) {
          throw new Error(`unexpected selected priority rows: ${selectedPriorityNames}`);
        }
        const priorityItems = context.mediaPipelineQueueView.queuePriorityItemsForSelected("low", "multi-select smoke");
        if (priorityItems.length !== 2) {
          throw new Error(`expected two priority payload items, got ${priorityItems.length}`);
        }
        ...
        requireText("pending-diagnostics-guidance", [
          ...
          "Selected-row diagnostic order: inspect backend-selected row targets first, then use Run Logs only when the row conflicts with disk state.",
        ]);
...
class WebViewRowDetailSmoke(unittest.TestCase):
