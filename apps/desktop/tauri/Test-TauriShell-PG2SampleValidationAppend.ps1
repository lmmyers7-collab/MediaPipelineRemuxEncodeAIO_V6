...
function Wait-ProcessIdsGone {
    param([int[]]$ProcessIds, [int]$TimeoutSeconds, [string]$Label)
...
function Get-BackendUrlForProcessId {
    param([Parameter(Mandatory)][int]$ProcessId, [int]$TimeoutSeconds = 20)
    $deadline = (Get-Date).AddSeconds($TimeoutSeconds)
    do {
        $ports = @()
        try {
...
function Invoke-BackendJson {
    param([Parameter(Mandatory)][string]$Method, [Parameter(Mandatory)][string]$Uri, [Parameter(Mandatory)][string]$Token)
    Invoke-RestMethod -Method $Method -Uri $Uri -Headers @{ Authorization = "Bearer $Token" } -TimeoutSec 30
}

function Get-MatchingSampleValidationRecord {
    param(
        [Parameter(Mandatory)][string]$Path,
        [Parameter(Mandatory)][string]$Source,
        [Parameter(Mandatory)][string]$NoteMarker
    )
    if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) { return $null }
    $lines = @(Get-Content -LiteralPath $Path -Tail 120 -ErrorAction SilentlyContinue)
    for ($idx = $lines.Count - 1; $idx -ge 0; $idx--) {
        try {
            $record = $lines[$idx] | ConvertFrom-Json
        } catch {
            continue
        }
        if ([string]$record.source_path -eq $Source -and [string]$record.shell -eq 'tauri' -and [string]$record.operator_notes -like "*$NoteMarker*") {
            return $record
        }
    }
    return $null
...
function Wait-SampleValidationRecord {
    param(
        [Parameter(Mandatory)][string]$Path,
        [Parameter(Mandatory)][string]$Source,
        [Parameter(Mandatory)][string]$NoteMarker,
    ...
    $deadline = (Get-Date).AddSeconds($TimeoutSeconds)
    do {
        $record = Get-MatchingSampleValidationRecord -Path $Path -Source $Source -NoteMarker $NoteMarker
        if ($record) { return $record }
        Start-Sleep -Seconds 2
    } while ((Get-Date) -lt $deadline)
    throw "Timed out waiting for Tauri Sample Validation record for $Source with marker $NoteMarker."
}
...
$logRoot = Join-Path ([System.IO.Path]::GetTempPath()) 'mediapipeline-tauri-shell-pg2-sample-validation'
New-Item -ItemType Directory -Path $logRoot -Force | Out-Null
$stamp = Get-Date -Format 'yyyyMMdd_HHmmss'
$noteMarker = "pg2-tauri-webview-append-$stamp"
...
$previousAutomation = $env:MEDIA_PIPELINE_TAURI_TEST_AUTOMATION
$previousSource = $env:MEDIA_PIPELINE_TAURI_TEST_AUTOLAUNCH_SINGLE_FILE
$previousDecision = $env:MEDIA_PIPELINE_TAURI_TEST_SAMPLE_VALIDATION_DECISION
...
$env:MEDIA_PIPELINE_TAURI_TEST_AUTOMATION = $previousAutomation
$env:MEDIA_PIPELINE_TAURI_TEST_AUTOLAUNCH_SINGLE_FILE = $previousSource
$env:MEDIA_PIPELINE_TAURI_TEST_SAMPLE_VALIDATION_DECISION = $previousDecision
...
try {
    ...
    if (-not $visibleWindowProcess) { throw "Tauri window '$WindowTitle' was not detected within $TimeoutSeconds seconds." }
    if (-not $windowProcess) { throw "New Tauri shell process was not detected within $TimeoutSeconds seconds." }
    if ($newBackendIds.Count -eq 0) { throw "New Python local API backend process was not detected within $TimeoutSeconds seconds." }
    ...
    $record = Wait-SampleValidationRecord -Path $SampleValidationLog -Source $resolvedSource -NoteMarker $noteMarker -TimeoutSeconds $AppendTimeoutSeconds
    $commands = Invoke-BackendJson -Method GET -Uri "$backendUrl/api/commands?limit=10" -Token $token
    $closeReadiness = Invoke-BackendJson -Method GET -Uri "$backendUrl/api/backend/close-readiness" -Token $token
    ...
    if (-not $devProcess.WaitForExit($CloseTimeoutSeconds * 1000)) {
        throw "Tauri dev process did not exit within $CloseTimeoutSeconds seconds after close."
    }
    ...
    if ($remainingShellIds.Count -gt 0 -or $remainingBackendIds.Count -gt 0) {
        throw "PG-2 Sample Validation cleanup left processes running. shell=$($remainingShellIds -join ', ') backend=$($remainingBackendIds -join ', ')"
    }
    ...
    [ordered]@{
        ...
        shell_surface = [string]$bootstrap.shellSurface
        sample_validation_log = $SampleValidationLog
        sample_validation_record_id = [string]$record.record_id
        sample_validation_shell = [string]$record.shell
        sample_validation_decision = [string]$record.operator_decision
        sample_validation_category = [string]$record.sample_category
    ...
    $env:MEDIA_PIPELINE_TAURI_TEST_SAMPLE_VALIDATION_NOTES = $previousNotes
    $env:MEDIA_PIPELINE_TAURI_TEST_SAMPLE_VALIDATION_OUTPUT = $previousOutput
    if ($devProcess -and -not $devProcess.HasExited) { Stop-ProcessTree -Process $devProcess }
    foreach ($shellPid in $newShellIds) { Stop-ProcessIdTree -ProcessId $shellPid }
    foreach ($backendPid in $newBackendIds) { Stop-ProcessIdTree -ProcessId $backendPid }
}
