...
function Resolve-TauriPython {
    ...
    foreach ($candidate in $candidates) {
        ...
        try {
            Test-LocalApiBackendModule -PythonPath $candidate -DesktopRoot $DesktopRoot -ProjectRoot $ProjectRoot
            $script:TauriPythonPreflightFailures = $failures
            return [string]$candidate
        } catch {
...
function Test-LocalApiBackendModule {
    ...
    try {
        ...
        $helpOutput = & $PythonPath -m mediapipeline.desktop.local_api_main --help 2>&1 | Out-String
        if ($LASTEXITCODE -ne 0 -or $helpOutput -notmatch 'Start the MediaPipeline local backend API') {
            $detail = ($helpOutput.Trim() -split "`r?`n" | Select-Object -Last 30) -join [Environment]::NewLine
            if ([string]::IsNullOrWhiteSpace($detail)) {
                $detail = '(no output)'
            }
            throw "python -m mediapipeline.desktop.local_api_main --help failed for $PythonPath (exit $LASTEXITCODE). Details:$([Environment]::NewLine)$detail"
        }
    } finally {
        Pop-Location
...
if (-not $python) {
    $failureText = ''
    if ($script:TauriPythonPreflightFailures) {
        $failureText = "$([Environment]::NewLine)Python candidates tried:$([Environment]::NewLine)- " + ($script:TauriPythonPreflightFailures -join "$([Environment]::NewLine)- ")
    }
    throw "A usable backend Python runtime was not found. Expected apps\desktop\runtime\Python\python.exe, ops\pipeline\runtime\Python\python.exe, or python on PATH.$failureText"
}
Test-LocalApiBackendModule -PythonPath $python -DesktopRoot $desktopRoot -ProjectRoot $projectRoot
...
Write-Host 'MediaPipelineRemuxEncodeAIO Tauri/WebView2 Shell' -ForegroundColor Cyan
Write-Host 'Current promoted workspace. Keep the external rollback workspace available if needed.' -ForegroundColor Yellow
