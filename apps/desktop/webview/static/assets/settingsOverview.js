(function () {
  ...
  function settingsOverviewNumber(config, key, fallback = 0) {
    const value = rawConfigValue(config, key, fallback);
    const number = Number(value);
    return Number.isFinite(number) ? number : fallback;
  }

  function settingsOverviewRouteMaxHeightFromUpperTolerance(baseHeight, tolerancePercent) {
    return Math.round(Number(baseHeight) * (1 + (Number(tolerancePercent) / 100)));
  }

  function settingsOverviewRouteMinHeightFromLowerTolerance(baseHeight, tolerancePercent) {
    return Math.round(Number(baseHeight) * (1 - (Number(tolerancePercent) / 100)));
  }
  ...
  function settingsOverviewRouteHeightBoundaries(config) {
    return {
      route1080pMaxHeight: settingsOverviewRouteMaxHeightFromUpperTolerance(
        1080,
        settingsOverviewNumber(config, "Route1080pUpperHeightTolerancePercent", 11.111111)
      ),
      route1440pMinHeight: settingsOverviewRouteMinHeightFromLowerTolerance(
        1440,
        settingsOverviewNumber(config, "Route1440pLowerHeightTolerancePercent", 16.597222)
      ),
      route1440pMaxHeight: settingsOverviewRouteMaxHeightFromUpperTolerance(
        1440,
        settingsOverviewNumber(config, "Route1440pUpperHeightTolerancePercent", 24.930556)
      ),
      route4kMinHeight: settingsOverviewRouteMinHeightFromLowerTolerance(
        2160,
        settingsOverviewNumber(config, "Route4KLowerHeightTolerancePercent", 16.666667)
      ),
    };
  }
  ...
  function settingsOperatorTrustLines(settings) {
    ...
    const routeBoundaries = settingsOverviewRouteHeightBoundaries(config);
    const lines = [
      "Saved settings trust checklist:",
