(function () {
  const launchCompactGateModule = window.__launchCompactGateModule || {};
  delete window.__launchCompactGateModule;
  if (typeof launchCompactGateModule.createLaunchCompactGateModule !== "function") {
    throw new Error("launch/scope/compactGate.js must load before launchView.scope.js");
  }
  ...
  function renderLaunchStartDecisionSummary(request = collectPipelineStartRequest(), payload = launchSettingsIntentPayload()) {
    ...
    const {
    ...
    } = launchCompactGateModule.createLaunchCompactGateModule({
      activateLaunchTab,
      appendCells,
      byId,
      clearRows,
      ...
      scopeApi: {
        launchStartDecisionAdvisoryPosture,
