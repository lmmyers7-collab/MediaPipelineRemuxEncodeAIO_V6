...
Policy precedence: AGENTS.md > SESSION.md > CLAUDE.md > inline comments
...
rules, source-of-truth documents, boundaries, and high-risk areas. SESSION.md owns the current branch/session
scope and overrides this file where they disagree.

If this file conflicts with AGENTS.md, AGENTS.md wins. If SESSION.md conflicts with this file on scope,
SESSION.md wins. Operator chat can narrow a task, approve extra paths for the current turn, or choose among
...
minimum startup needed to avoid violating AGENTS.md and SESSION.md, then proceed with a named caveat.

1. Read **AGENTS.md** — §§1–3 and §7 in full; skim the rest. §7 changes most often; never skim it.
2. Read **PROJECT_INDEX.md**. If missing, ask whether to regenerate via `scripts/dev/refresh_index.py`,
   proceed degraded with a named caveat, or abort. Do not reconstruct it by reading source files.
3. Read **SESSION.md**. If absent, read-only answers, repository discovery, and tiny single-file typo/comment
   fixes may proceed degraded; any non-trivial edit, refactor, migration, validation-sensitive task, or
   multi-file task requires a SESSION.md or explicit operator approval first.
...
5. Do **not** read unless the task explicitly requires it: `ARCHITECTURAL_OVERHAUL_PLAN.md`,
   `Docs/CURRENT_PROJECT_STATE.md`, `OPEN_WORK_CHECKLIST.md`, `AI_HANDOFF.md`, `MONOLITH_SPLIT_PLAN.md`, or
...
PROJECT_INDEX.md, SESSION.md, or Docs/audit/latest.md permits degraded mode only as described above or if the
operator confirms.
...
- Never edit a file not listed in SESSION.md without explicit current-turn operator approval naming the file,
...
- Operator approval can expand the current turn's editable scope only where AGENTS.md allows; it does not
  rewrite AGENTS.md, SESSION.md, or this file.
- Never touch AGENTS.md §7 high-risk areas unless SESSION.md names them explicitly and the operator has
...
- Do not create status, review, audit, handoff, or planning Markdown files as a workaround. Put notes in
  SESSION.md or an approved SSOT document.
...
`engine/`, or `app/`. If the next edit would touch a directory outside the current SESSION.md domain set, stop
...
Migration exception: when SESSION.md explicitly names V6 migration work, `DesktopApp/` ↔ `app/` and
`Pipeline/Modules/` ↔ `engine/` are each one domain pair for that session. Outside migration sessions, they
...
direct fix for a specific pasted error. Even then, stay within SESSION.md scope.
...
expected diff is under 100 lines; target is not AGENTS.md §7; target is under `app/`, `engine/`, `summaries/`,
`Docs/`, or `SmokeTests/`; prompt does not require secrets, credentials, private media contents, or
...
Cached artifacts: use `summaries/<mirror-of-source-path>.md`, PROJECT_INDEX.md, and Docs/audit/latest.md as
...
- One edit domain per session. Do not mix `Pipeline/` PowerShell work with `ui_web/` JavaScript work unless
  SESSION.md explicitly scopes both.
...
- Compact after planning, after a green test run, and after completing a sub-task. Re-read SESSION.md after
  every compact.
...
- Do not opportunistically refactor. Leave adjacent cleanup for a new SESSION.md unless required for the
  approved task.
...
- Use `DesktopApp\Runtime\Python\python.exe`, never system Python, unless AGENTS.md specifies a different
  project-owned tool.
...
the operator signals interruption/usage-limit/compact, or SESSION.md exit criteria are met.

Write the summary into SESSION.md under `## Handoff <YYYY-MM-DD>` unless AGENTS.md says otherwise. SESSION.md
is always in scope for current-session handoff notes. If SESSION.md is absent, provide the handoff in chat and
do not create SESSION.md unless asked.
