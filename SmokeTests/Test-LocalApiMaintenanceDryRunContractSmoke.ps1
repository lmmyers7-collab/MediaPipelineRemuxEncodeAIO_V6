...
Write-Host 'Boundary: executes only /api/maintenance/release-dry-run and /api/maintenance/completed-backfill-dry-run backend dry-run POST routes.'
Write-Host 'Boundary: validates token enforcement, command history, release dry-run no-manifest/no-zip evidence, and completed-manifest backfill dry-run no-manifest-write evidence.'
...
Write-Host 'Boundary: does not create a release package, write a release manifest, write a zip package, or rewrite completed manifests.'
Write-Host "Python: $python"

Push-Location -LiteralPath $projectRoot
try {
    $env:PYTHONDONTWRITEBYTECODE = '1'
    & $python -m unittest DesktopApp.tests.test_local_api_maintenance_dry_run_contract_smoke -q
