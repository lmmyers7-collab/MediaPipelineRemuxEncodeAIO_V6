...
def sample_validation_cutover_gate_payload(
    ...
    if blocked_count:
        ...
        safe_next_action = "WebView has current accepted sample evidence for a limited operator trial; keep the external rollback workspace and validate more media-policy changes before cutover."
    summary_lines = [
...
def sample_validation_pilot_runbook_payload(
    ...
    rows = [
        ...
        row(
            ...
            "Launch",
            execution_status("Backend Launch Boundary", "Start only through backend-owned Launch", "manual"),
        ...
        row(
            ...
            f"diagnostics/log stage={execution_status('Post-run Diagnostics Proof', 'Read Diagnostics and run logs for the same sample', 'unknown')}",
    ...
    if blocked_count:
        ...
        safe_next_action = "Run or record the next representative sample while keeping external rollback/manual review as fallback."
    ...
    markdown_lines = [
        ...
        "- Use external rollback/manual review as fallback until representative real-media samples have current evidence.",
        "",
        "## Checklist",
    ]
    for item in rows:
...
def sample_validation_sample_set_guide_payload(
    ...
    if blocked_count:
        ...
        safe_next_action = "Representative real-media sample categories have current evidence; keep the external rollback workspace and rerun samples after media-policy changes."
    summary_lines = [
...
from mediapipeline.core.sample_validation.pilot_plan_projection import *  # noqa: F403
