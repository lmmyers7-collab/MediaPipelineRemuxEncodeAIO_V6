...
from mediapipeline.core.kernel.config_keys import KEY_DEFERRED_PUBLISH
...
def sample_validation_policy_alignment_payload(resolved: ResolvedPaths) -> dict[str, Any]:
    ...
    media_readiness = _build_media_policy_readiness(config)
    readiness_rows = {
...
def _build_media_policy_readiness(config: dict[str, Any]) -> dict[str, Any]:
    from mediapipeline.core.config.settings_facade import build_media_policy_readiness

    return build_media_policy_readiness(config)
