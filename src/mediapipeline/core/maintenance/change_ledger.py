...
from mediapipeline.tools.change_control import packet_coverage
...
def _python_group(path: str) -> str:
    ...
    if not parts:
        return "other_python"
    if parts[:3] == ["src", "mediapipeline", "core"] and len(parts) > 3:
        return f"src/mediapipeline/core/{parts[3]}"
    if parts[:3] == ["src", "mediapipeline", "desktop"]:
        return "src/mediapipeline/desktop" if len(parts) < 4 else f"src/mediapipeline/desktop/{parts[3]}"
    if parts[:3] == ["src", "mediapipeline", "contracts"]:
        return "src/mediapipeline/contracts"
    if parts[:3] == ["src", "mediapipeline", "pipeline"]:
        return "src/mediapipeline/pipeline"
    if parts[:3] == ["src", "mediapipeline", "tools"]:
        return "src/mediapipeline/tools" if len(parts) < 4 else f"src/mediapipeline/tools/{parts[3]}"
    if parts[:2] == ["ops", "scripts"] and len(parts) > 2:
        return f"ops/scripts/{parts[2]}"
    if parts[:2] == ["ops", "pipeline"] and len(parts) > 2:
        return f"ops/pipeline/{parts[2]}"
    if parts[:2] == ["tests", "python"] and len(parts) > 2:
        return f"tests/python/{parts[2]}"
