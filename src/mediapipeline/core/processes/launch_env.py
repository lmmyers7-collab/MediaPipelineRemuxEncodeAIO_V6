...
def iter_bundled_launch_dirs(app_root: Path, workspace_root: Path) -> list[Path]:
    candidates = [
        ...
        workspace_root / "apps" / "desktop" / "runtime" / "Python",
        workspace_root / "ops" / "pipeline" / "tools" / "ffmpeg" / "bin",
        workspace_root / "ops" / "pipeline" / "tools" / "MKVToolNix",
        workspace_root / "ops" / "pipeline" / "runtime" / "PowerShell-7.6.0-win-x64",
    ]
    seen: set[str] = set()
    resolved: list[Path] = []
