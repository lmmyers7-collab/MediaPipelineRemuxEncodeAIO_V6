...
#     "position": <optional manual-order number>
#   }
#
# Bulk request body:
#   {
#     "items": [
#       {"path": "...", "level": "high", "reason": "...", "position": 1},
#       ...
#     ]
#   }
#
...
def _priority_position_from_request(value: Any) -> tuple[float | None, str]:
    if value in (None, ""):
        return None, ""
    try:
        position = float(value)
    except (TypeError, ValueError):
        return None, "Manual order position must be a finite number."
    ...
    def _queue_priority_payload(self, request: dict[str, Any]) -> dict[str, Any]:
        ...
        if items_raw is not None:
            ...
            for item in items_raw:
                ...
                if "reason" in item:
                    payload_item["reason"] = str(item.get("reason", "")).strip()
                if "position" in item:
                    position, position_error = _priority_position_from_request(item.get("position"))
                    if position_error:
                        return _priority_error_payload(position_error)
                    payload_item["position"] = position
        ...
        position = None
        position_was_supplied = "position" in request
        ...
        if position_was_supplied:
            position, position_error = _priority_position_from_request(request.get("position"))
            if position_error:
                return _priority_error_payload(position_error)

        try:
            if position_was_supplied:
                manifest = set_manifest_entry(manifest_path, source_path or "", level_raw, reason, position=position)
            else:
