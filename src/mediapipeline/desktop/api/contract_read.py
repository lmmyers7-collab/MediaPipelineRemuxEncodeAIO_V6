...
LOCAL_API_STATUS_READ_ROUTE_CONTRACT: tuple[dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "purpose": "Read backend-authored launch preflight checks for pipeline, audit, or CSV rerun. Blank Run Once pipeline scope adds a normal_queue_scope check: only a fresh, matching snapshot with zero runnable rows blocks as no_runnable_work; missing, stale, mismatched, or scanning evidence stays advisory. Pipeline preflight refreshes the backend-owned encoder capability diagnostic JSON only when refresh_encoder_capability_report=true; it does not reserve locks, launch work, write control flags, mutate config, or touch media files.",
    },
    {
        "method": "GET",
        "path": "/api/commands",
