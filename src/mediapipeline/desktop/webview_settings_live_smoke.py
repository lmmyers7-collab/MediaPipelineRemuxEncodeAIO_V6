...
def _validate_static_assets(base_url: str) -> None:
    ...
    for asset_path in (
        ...
        child_status, child_js, _child_type = _get_text(f"{base_url}/{asset_path}")
        _require_status(child_status, f"/{asset_path}")
        launch_parts.append(child_js)
    launch_status, launch_parent_js, _launch_type = _get_text(f"{base_url}/assets/launchView.risk.js")
    _require_status(launch_status, "/assets/launchView.risk.js")
    launch_js = "\n".join([*launch_parts, launch_parent_js])
    for fragment, label in (
