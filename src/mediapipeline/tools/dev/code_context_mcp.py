...
def create_mcp_server(service: CodeContextService | None = None) -> FastMCP:
    ...
    def repo_context(task: str, budget: int = 2000, include_history: bool = False) -> dict[str, Any]:
        """Get a token-bounded implementation slice for a repository task."""
        return context_service.repo_context(task, budget=budget, include_history=include_history)
