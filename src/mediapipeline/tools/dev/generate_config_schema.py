"""Generate the config JSON Schema from `mediapipeline.contracts.config`.

Use `--check` in CI/pre-commit to fail when `src/mediapipeline/contracts/schemas/config.v1.schema.json`
is stale.
"""
...
def render_schema() -> str:
    ...
    schema["$id"] = "https://local.mediapipeline/src/mediapipeline/contracts/schemas/config.v1.schema.json"
