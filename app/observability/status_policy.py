...
from app.telemetry.gpu_usage import gpu_encoder_usage_payload
from mediapipeline_desktop_app.models import Snapshot, TelemetrySnapshot
...
def pipeline_progress_bars(progress: Mapping[str, Any], *, pipeline_state: str, stale: bool) -> list[dict[str, Any]]:
    ...
    if push_state or sidecar_state or stage_lower in {"push", "sidecar", "retry_pending_push"}:
        publish_percent = percent
        if publish_percent is None and push_state.lower() == "copying" and copy_percent is not None:
            publish_percent = min(94.0, round(copy_percent * 0.94, 1))
        if publish_percent is None:
            publish_percent = float_from_mapping({"publish_percent": {
                "copied_pending_reveal": 95,
                "revealing": 99,
                "complete": 100,
                "deferred": 100,
            }.get(push_state.lower(), "")}, "publish_percent")
        ...
        bars.append(
            progress_bar(
                ...
                detail=" | ".join(part for part in (push_state, sidecar_state, route, file_display) if part),
                source="pipeline_progress.json",
                updated_at=text_from_mapping(progress, "LastUpdate", "UpdatedAt", "updated_at"),
                stale=stale,
            )
