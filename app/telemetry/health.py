...
from app.kernel.config_keys import (
...
from app.storage.db import STATE_DB_FILENAME
...
CANONICAL_LAYOUT_PATHS = (
    ("scripts/dev/start-local-api.bat", "canonical Local API launcher"),
    ("scripts/dev/start-tauri-preview.bat", "canonical Tauri preview launcher"),
    ("scripts/dev/start-api-and-browser.bat", "canonical API/browser launcher"),
    ("scripts/dev/run.bat", "canonical operator launcher"),
    ("scripts/verify-env.bat", "canonical environment verifier"),
    ("scripts/verify-env.ps1", "canonical environment verifier"),
    ("DesktopApp/Runtime/Python/python.exe", "bundled Python runtime"),
    ("Pipeline/PowerShell-7.6.0-win-x64/pwsh.exe", "bundled PowerShell runtime"),
    ("DesktopApp/mediapipeline_desktop_app/ui_web/static/index.html", "backend-served WebView index"),
)
...
def api_contract_health_rows() -> list[HealthRow]:
    ...
    status_note = "/api/status is not an active V6 route; /api/health and /api/snapshot are the current status surfaces."
...
def bundle_layout_health_rows(workspace_root: Path, app_root: Path) -> list[HealthRow]:
    ...
    pipeline_modules = workspace_root / "Pipeline" / "Modules"
    try:
        dotted_modules = list(pipeline_modules.glob("*.*.ps1")) if pipeline_modules.is_dir() else []
    except OSError:
        dotted_modules = []
    if missing:
        return [("Bundle layout", False, "Missing expected V6 layout path(s): " + "; ".join(missing[:6]))]
    if reintroduced:
        return [("Bundle layout", False, "Removed root launcher shim(s) reintroduced: " + ", ".join(reintroduced))]
    if dotted_modules:
        return [("Bundle layout", False, "Dotted Pipeline/Modules PowerShell file(s) present: " + ", ".join(path.name for path in dotted_modules[:6]))]
