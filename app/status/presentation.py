...
from app.rename.movie import clean_pipeline_movie_name
from app.rename.tv import clean_pipeline_tv_name_part, extract_confident_tv_episode_title
...
def current_work_item_label(progress: dict[str, Any]) -> str:
    ...
    if media_type == "movie":
        return clean_pipeline_movie_name(_strip_release_group_suffix(leaf)) or Path(leaf).stem
    if media_type == "tv":
...
def build_current_work(progress: dict[str, Any] | None) -> dict[str, str]:
    payload = progress if isinstance(progress, dict) else {}
    return {
        "schema_version": CURRENT_WORK_SCHEMA_VERSION,
        "item_label": current_work_item_label(payload),
