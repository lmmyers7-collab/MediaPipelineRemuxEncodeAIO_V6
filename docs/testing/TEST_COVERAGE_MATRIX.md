...
| Coverage type | Files / Wrappers |
|---|---|
| Python unit tests | `test_facade_queue_policy.py`, `test_service_queue_snapshot.py`, `test_service_queue_preview_builder.py`, `test_service_queue_dry_run.py`, `test_service_queue_dry_run_runner.py`, `test_application_facade_queue.py` (facade queue preview snapshot-read behavior, stale evidence, blocked-vs-excluded rows, runtime outcome correlation, CSV rerun manifest fallback status projection, and row-key open allowlists), `test_application_facade_local_api_queue.py` (queue state route contract, source-root scope, and command history), `test_process_rerun_results.py` (CSV rerun first-class Queue read-model status mapping and action evidence), `test_api_contract_payload.py` (route effect schema, effectful POST command-result history contract, and CSV rerun auto-return destination defaults) |
...
| Coverage type | Files / Wrappers |
|---|---|
| Python unit tests | `test_stage_runner.py` (scratch-only dispatcher rename dry-run/execute, strict confirmation, source/scratch boundary rejection, stale-fingerprint rejection, command/operation journals, duplicate replay, undo evidence, and rollback), `test_stage_contracts.py` (backend registration and strict DTO), `test_facade_rename_policy.py`, `test_application_facade_rename.py` (facade rename preview/apply command behavior, confirmation, selected-source scope, multi-select handling, and apply-lock guard), `test_application_facade_local_api_rename.py` (native rename browse command-result payload with injected picker, Local API absent/false `confirm_apply` rejection without rename mutation, undo confirmation, media-root injection, and preview route policy), `test_api_path_dialogs.py` (Windows PowerShell dialog host selection, encoded-command invocation, payload parsing, and host-aware failure messaging), `test_service_rename_movie.py`, `test_service_rename_tv.py`, `test_service_rename_tv_folder.py`, `test_service_rename_plan_policy.py`, `test_service_rename_planner.py` (service-layer duplicate-destination blocking for every colliding row), `test_service_rename_apply.py`, `test_service_rename_preview.py`, `test_service_rename_discovery.py`, `test_service_rename_preview_runner.py`, `test_service_rename_apply_runner.py` |
...
**Gaps**: `rename.apply` is never invoked by any smoke (by design — it is a filesystem mutation); the scratch-only dispatcher stage is covered with temporary fixture files rather than representative media because it changes names only and does not inspect container streams; the real native Windows dialog is not opened by automated smokes because it would block headless runs; TV folder rename not exercised in smokes.
...
**Gaps**: WebView smokes do not submit real `pipeline/start`, `pipeline/control`, `rerun/control`, or `rerun/continue` commands by design. PowerShell end-to-end smoke exercises `-Once` and `-DrainPendingPushes`; `validate`, `continuous`, `audit/start`, `rerun/start`, `rerun/continue`, and process-control route submissions are not smoke-exercised. Launch history rendering has read-only browser coverage.
...
| PowerShell focused checks | `ops/pipeline/tests/Unit/Invoke-PortablePathChecks.ps1` (audit/legacy GUI scripts parse and do not default audit roots to local operator UNC paths); `ops/pipeline/tests/Unit/Invoke-RerunSourceIdentityChecks.ps1` (CSV rerun source identity remains deterministic and sample-hash-sensitive when `ffprobe` is unavailable); `ops/pipeline/tests/Unit/Invoke-RerunCooperativeStopChecks.ps1` (CSV rerun stop marker placement, stale marker guards, post-chunk manifest stop evidence, and continuation guidance); `ops/pipeline/tests/Unit/Invoke-RerunAutoDestinationChecks.ps1` (auto-return clean replacement, remaining-issue Pending Publish review routing, and source preservation with a mocked nested pipeline) |
...
**Gaps**: Report-open diagnostics targets are not individually exercised; `audit/start` remains intentionally unexercised by smokes because it is a backend-owned process-launch mutation. CSV rerun start is covered from Queue, not Reports.
