...
**Module**: `queueView.js`
**API routes**: `GET /api/queue`, `GET/POST /api/queue/priority`, `GET/POST /api/queue/strategy`, `GET/POST /api/queue/file-overrides`, `POST /api/queue/file-overrides/route-preview`, `POST /api/queue/file-overrides/series-preview`, `POST /api/queue/file-overrides/series-apply`, `POST /api/queue/file-overrides/series-clear-preview`, `POST /api/queue/file-overrides/series-clear-apply`, `POST /api/queue/open`, `POST /api/rerun/preview`, `GET /api/rerun/results`, `POST /api/rerun/start`, `POST /api/rerun/control`, `POST /api/rerun/continue`, `POST /api/rerun/open`, `POST /api/rerun/promote-dry-run`, `POST /api/rerun/promote`

| Coverage type | Files / Wrappers |
|---|---|
| Python unit tests | `test_facade_queue_policy.py`, `test_service_queue_snapshot.py`, `test_service_queue_preview_builder.py`, `test_service_queue_dry_run.py`, `test_service_queue_dry_run_runner.py`, `test_application_facade_queue.py` (facade queue preview snapshot-read behavior, stale evidence, blocked-vs-excluded rows, runtime outcome correlation, CSV rerun manifest fallback status projection, and row-key open allowlists), `test_application_facade_local_api_queue.py` (queue state route contract, source-root scope, and command history), `test_rerun_lifecycle.py` (durable enrollment, strict exit proof, spawn ambiguity, and restart correlation), `test_process_rerun_results.py` (exact v2 manifest-path binding, copied-manifest degradation, logical recovery generations, canonical selector idempotence, waiting-child correlation, and Queue action evidence), `test_api_contract_payload.py` (route effect schema, effectful POST command-result history contract, and CSV rerun auto-return destination defaults) |
...
| Coverage type | Files / Wrappers |
|---|---|
| Python unit tests | `test_facade_process_pipeline_policy.py`, `test_application_facade_process_launch.py` (pipeline/audit/rerun handoff, strict failed-before-manifest supersession, verified-versus-ambiguous cleanup, duplicate rejection, launch-lock and schedule behavior, original-source policy rejection, and backend-authored readiness), `test_service_process_spawn_runner.py` and `test_service_process_kill.py` (verified watcher cleanup, watcher-start failure, degraded process-tree retention, and normal kill success), `test_application_facade_local_api_http.py` (Local API exposure of nested `operator_readiness`), `test_application_facade_web_static_launch.py` (state-aware Launch command button wiring, active CSV rerun Stop After Current copy, hidden-until-active emergency topbar Force Stop, destructive/long-running action tooltips, and sanitized command-failure detail hooks), `test_application_facade_web_static_queue.py` (Queue CSV Rerun tab state table, `/api/rerun/*` route ownership, and no `apiPost("/api/pipeline/start")` from rerun JS), `test_webview_launch_command_buttons_smoke.py` (active CSV rerun disables Pause/Rescan while keeping Stop After Current and Force Stop available), `test_facade_process_control_policy.py`, `test_application_facade_process_control.py` (facade pipeline control flag contract and backend control-lock fail-closed behavior), `test_facade_process_guard_policy.py`, `test_application_facade_queue.py` (CSV rerun rows remain visible when normal queue rows are empty), `test_facade_process_rerun_policy.py`, `test_service_process_readiness.py`, `test_service_process_launch_runner.py`, `test_service_process_launch_plans.py`, `test_service_process_launch_env.py`, `test_process_rerun_results.py` (rerun control marker, stopped-manifest result evidence, pending-only continuation CSV, first-class Queue state row mapping, and promote action evidence), `test_api_contract_payload.py` (CSV rerun auto-return route defaults, replacement confirmation, and strict rerun control/continue contracts) |
...
is authoritative for the current 24-wrapper inventory; this table summarizes
