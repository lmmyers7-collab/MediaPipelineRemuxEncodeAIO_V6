...
`test_stage_contracts.py`, `test_stage_runner.py`, and `test_stage_entrypoint.py` guard the Python/PowerShell stage boundary. They now prove `ingest` is the only enabled mutation-capable stage, `execute` requires strict boolean `confirm_ingest=true`, string confirmations fail before dispatch, command-journal summaries carry bounded stage evidence, dry-run ingest writes nothing, execute ingest copies only to a guarded scratch-root child path, writes rollback/recovery evidence, refuses source-inside-scratch boundaries, and leaves the source hash unchanged. These tests use temporary representative files only; they do not prove FFmpeg, subtitle, audio, publish, drain, or real-media output quality.
