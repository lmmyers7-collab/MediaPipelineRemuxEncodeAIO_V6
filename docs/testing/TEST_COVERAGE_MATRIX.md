...
| **Browser-backed smokes** (`Test-WebViewBrowser*.ps1`, Chrome/Edge) | Real browser render, interactive clicks, filter behavior, mutation-boundary enforcement, and hash-based no-mutation assertions for temporary fixtures; selected scenarios also exercise isolated temp-config persistence, production lifecycle reconciliation APIs, or a temp Local API launch boundary with a synchronized fake runner | Real media processing, production PowerShell/FFmpeg pipeline execution, output-quality proof, production settings/state mutation, or native Tauri dialog behavior |
...
## Backend Queue Run Once Monitor Coverage

The standard `mode=once` / Backend Queue workflow uses the versioned
`pipeline_run_monitor.v1` durable engine record and the read-only
`desktop_run_monitor.v1` projection from `GET /api/run-monitor`. Queue remains
planned-intent authority; fresh exact run/job engine evidence owns current
claims; Completed, Pending Publish, and failure artifacts own terminal proof.
The WebView does not reconstruct current work from Queue render order, filename
similarity, route text, logs, cumulative counters, or an uncorrelated event
tail.

| Coverage tier | Files / proof |
|---|---|
| Contract and projection | `tests/python/core/contract/test_run_monitor_contract.py`, `tests/python/desktop/test_service_run_monitor.py` (immutable exact membership beyond display limits, stable identities, run-wide positions, stage and collection enums, planned/executed/final routes, per-track evidence, truthful progress, all workers, freshness suppression, terminal references, restart/retention, Stop After Current, partial/corrupt compatibility) |
| Local API and launch | `test_application_facade_local_api_http.py`, `test_application_facade_local_api_process_commands.py`, `test_application_facade_local_api_workflow.py`, `test_application_facade_process_launch.py`, `test_api_contract_payload.py`, `test_api_route_inventory.py` (authenticated read route, optional exact run ID, launch run/fingerprint handoff, unavailable evidence, command correlation, route inventory) |
| PowerShell engine | `Invoke-RunMonitorStateChecks.ps1`, `Invoke-EncodeRuntimeRouteEvidenceChecks.ps1`, `Invoke-LocalWorkerSlotChecks.ps1`, `Invoke-FFmpegProgressChecks.ps1`, `Invoke-PathBoundaryGuardChecks.ps1`, `Invoke-PendingPublishSafetyChecks.ps1`, `Invoke-PipelineQueueEngineChecks.ps1` (seed/fingerprint correlation, versioned planned rename and force-rename evidence, accepted-membership digest recomputation, raw/unversioned/tampered-name rejection, no-lost-update state, explicit stage transitions, exact hardware/CPU route evidence, all-worker projection, scratch/publish outcomes, terminal-artifact reconciliation) |
| Static WebView | `tests/webview/test_webview_run_monitor_static.py`, `test_webview_navigation_static.py`, `test_webview_css_design_tokens.py` (one primary Current Work surface, backend-only current claims, outcome-first progressive-disclosure regions, full evidence retained under the existing Advanced gate, retired competing panels, planned Queue labels, focus/navigation contract, roving Queue/current-work selection, responsive per-cell track labels, dynamic worker/proof focus keys, plural active-file semantics, reflow, reduced motion, non-color cues) |
| Deterministic browser states | `tests/webview/test_webview_browser_run_monitor_smoke.py` (empty, starting, scanning, known/unknown copy, probe active/N/A, route pending, remux/hardware/CPU work, multi-track/no-audio, determinate/indeterminate OCR, mux, verification, sidecars, publish/park, handoff, failure/review/blocked/skipped/stopped, pause/stop/stopping, stale/contradictory evidence, completion/fresh idle, compact normal-mode outcome/route/stage/track/output rendering, Advanced-mode restoration of exact IDs/bytes/paths/timestamps/provenance/full ledger, verified backend-planned rename filenames with raw source identity retained as secondary accessible evidence, contradictory name provenance treated as legacy/unknown, a folded first-20 filename preview, complete selectable expansion, exact selection/focus retention across fold and same-run refresh, safe folded-disclosure focus reset on different-run replacement, rows/workers/terminal proof, two distinct active files, explicit owner/next-action guidance for terminal rows, semantic clean-name announcements, computed AA contrast, keyboard, narrow labels, and viewport-equivalent narrow/200%/400% reflow layouts) |
| Standard journey | `tests/webview/test_webview_browser_queue_launch_completed_smoke.py` and `Test-WebViewBrowserQueueLaunchCompletedSmoke.ps1` pass the real backend Queue plan, blank Single File, accepted run/Queue-plan and membership fingerprints, folded clean-name preview/opened exact selection, launch, starting/scanning, two workers, file A terminal/file B current, Completed plus review proof, run completion, fresh idle, reload persistence, clean-name announcements, source-hash preservation, and unrelated-event suppression. |

### Original finding-to-regression map

| # | Corrective implementation | Regression evidence |
|---|---|---|
| 1 | Encode/remux activity comes only from the canonical active stage, never route text. | `Invoke-RunMonitorStateChecks.ps1`; `test_webview_run_monitor_static.py`; browser canonical-stage fixtures |
| 2 | Missing audio evidence is `awaiting_evidence`/unknown, never “pending.” | `test_run_monitor_contract.py`; `test_webview_browser_run_monitor_smoke.py` |
| 3 | Missing subtitle evidence is `awaiting_evidence`/unknown, never “pending.” | `test_run_monitor_contract.py`; `test_webview_browser_run_monitor_smoke.py` |
| 4 | Copy-to-scratch closes only from exact copy evidence, and reuse requires freshly recomputed matching source/scratch SHA-256 evidence. | `Invoke-ScratchCopyIdentityChecks.ps1`; `Invoke-PathBoundaryGuardChecks.ps1`; `Invoke-RunMonitorStateChecks.ps1` |
| 5 | Terminal evidence requires exact run/job-correlated artifacts; event tails are non-authoritative. | `Invoke-RunMonitorStateChecks.ps1`; `test_service_run_monitor.py`; standard Backend Queue browser journey with a recent unrelated-run `job_completed` event |
| 6 | One projection freshness decision suppresses stale current items, stages, routes, workers, and percentages. | `test_service_run_monitor.py`; browser stale/contradictory states |
| 7 | `encode_verify` is canonical Verification, not Encoding. | `Invoke-FFmpegProgressChecks.ps1`; browser Verification state |
| 8 | `remux_verify` is canonical Verification, not Remuxing. | `Invoke-FFmpegProgressChecks.ps1`; browser Verification state |
| 9 | Completed track work cannot override a later file stage. | `Invoke-RunMonitorStateChecks.ps1`; canonical-stage browser fixtures |
| 10 | Selection and worker correlation use exact job IDs; duplicate leaf names remain distinct. | contract duplicate/identity tests; both Run Once browser suites |
| 11 | Accepted-workload rows/counts are run-scoped and immutable; cumulative “Next 5” reconstruction is retired. | 507-item contract test; static retired-surface test |
| 12 | The projection and master list include every active worker. | `test_service_run_monitor.py`; `Invoke-LocalWorkerSlotChecks.ps1`; browser two-worker assertions |
| 13 | Queue labels predictive fields “Planned route” and “Planned reason.” | `test_webview_run_monitor_static.py`; browser route-separation assertions |
| 14 | Stage/File/ETA focus the selected Current Work detail; Telemetry remains a separate hardware link. | navigation static test; browser exact focus assertions |
| 15 | Existing backend-owned Stop After Current is present on Current Work. | control/flag unit tests; browser stop-state assertions |
| 16 | Exact job selection and focused row survive refresh. | browser selection/focus handoff assertions |
| 17 | Shared navigation lands on the destination heading or exact selected-file/proof target. | navigation static test; both Run Once browser suites |
| 18 | Current Work uses one semantic transition announcer rather than polling panel live regions. | static live-region count; browser assertions for new current work, failure/review, parked output, Stop After Current, and run completion |
| 19 | A deterministic browser suite executes the standard Backend Queue Run Once transitions through terminal reload and fresh idle. | `test_webview_browser_queue_launch_completed_smoke.py` |
| 20 | The Queue → Launch → terminal-proof regression submits Backend Queue with a blank Single File field and passes with exact accepted fingerprint/membership seeding. | Backend Queue journey plus `test_webview_browser_launch_queue_readiness_smoke.py` |

The synthetic/browser tiers do not prove FFmpeg/container correctness, media
policy execution, playback, OCR quality, hardware fallback, final placement,
or source/scratch behavior against representative media. Because this change
touches high-risk stage evidence, a representative real-media run remains the
release-readiness gate for this change packet and was not run without explicit
authorization.

The automated responsive smoke uses viewport-equivalent widths for the 200%
and 400% reflow cases. Actual browser zoom and screen-reader operation remain
manual accessibility checks in
`docs/operator/WEBVIEW_MANUAL_OPERATOR_TEST_SCRIPT.md`; no automated result is
presented as proof that those manual checks were executed.
...
| `ops/pipeline/tests/Unit/Invoke-AuditCommandSupportChecks.ps1` | Temporary-layout regression for entrypoint-relative bundled `ffprobe` discovery after the audit helper-module move; no operator media scan or live audit start. The active reliability wrapper runs it by default. |
...
`test_stage_contracts.py`, `test_stage_runner.py`, `test_stage_entrypoint.py`, and `test_stage_subtitle_convert.py` guard the Python/PowerShell stage boundary. They prove guarded PowerShell `ingest` plus guarded Python scratch-only `rename` and standalone ASS/SSA `subtitle-convert` are the enabled mutation-capable stages. Ingest execute requires strict boolean `confirm_ingest=true`, copies only to a guarded scratch-root child, writes rollback/recovery evidence, refuses source-inside-scratch boundaries, and leaves the source hash unchanged. Rename execute requires strict boolean `confirm_apply=true`, a UUID operation ID, matching content-bound dry-run fingerprint, command/operation journals, disjoint source/scratch roots, no overwrite, and a scratch-local undo record; duplicate replay and terminal-evidence rollback are covered. Subtitle-convert requires the corresponding strict confirmation/identity/fingerprint/journal gates, creates only one non-overwriting sibling SRT, preserves the ASS/SSA hash, routes no-dialogue/decode uncertainty to review, and rolls output back when terminal evidence fails. Contract tests also require every disabled mutation stage to expose a non-empty machine-readable blocker and required-validation list, while enabled stages may not retain a blocker. These tests use temporary byte and representative ASS fixtures only; they do not prove TX3G extraction, BDPGS OCR, embedded-stream policy, language routing, FFmpeg, audio, publish, drain, production naming/subtitle integration, or real-media output quality.
...
`test_application_facade_web_static.py`, `test_application_facade_queue.py`, `test_application_facade_completed.py`, `test_application_facade_web_static_completed.py`, `test_rerun_csv_preview.py`, and `test_application_facade_process_launch.py` now cover the current backend-served UI/facade equivalents of the 2026-05-20 UI-improvement backlog closure. Static WebView coverage pins state-aware Launch controls, hidden-until-active emergency Force Stop posture, destructive/long-running action tooltips, telemetry render failure diagnostics, status-chip keyboard/tooltip behavior, and structured backend error routing. Service/facade coverage pins backend scan-detail progress text, Completed refresh/backfill evidence, CSV rerun preview/preflight behavior, and CSV rerun terminal activity evidence.
...
The process lifecycle Protocol cleanup is covered by `test_service_process_control_runner.py`, `test_service_process_active_job_runner.py`, `test_service_process_launch_runner.py`, `test_service_process_runtime_runner.py`, `test_service_process_spawn_runner.py`, `test_service_process_kill.py`, `test_application_facade_process_launch.py`, `test_application_facade_process_control.py`, and `test_application_facade_close_readiness.py`. These tests prove the typed process runner boundaries still handle control flags, ActiveJobs records, launch plans, spawn/readiness cleanup, degraded taskkill evidence, lease/registry retention when process-tree exit is unproved, completion/heartbeat watcher-start failure, duplicate-launch rejection, command control, and close-readiness evidence. They do not launch real media processing.
...
Preserved remediation evidence: [Layout Manager Edge Cases](../archive/remediation-changelog/entries-0751-1000.md#2026-05-20---layout-manager-edge-cases).

```
Task ID: Layout manager edge cases
Files inspected: apps/desktop/webview/static/assets/app.js; apps/desktop/webview/static/assets/styles.layout-manager.css; tests/python/desktop/test_application_facade_local_api.py; tests/webview/test_webview_css_design_tokens.py; tests/webview/test_webview_browser_layout_manager_smoke.py; archived reconciliation evidence; docs/archive/remediation-changelog/entries-0751-1000.md
...
Preserved remediation evidence: [UI Improvement Backlog Closure](../archive/remediation-changelog/entries-0751-1000.md#2026-05-20---ui-improvement-backlog-closure).

```
Task ID: UI improvement backlog closure
Files inspected: apps/desktop/webview/static/assets/app.js; launchView.js; telemetryView.js; domHelpers.js; styles.layout.css; styles.controls.css; backend queue/completed/rerun/rename service and facade tests; archived reconciliation evidence; docs/archive/remediation-changelog/entries-0751-1000.md
Files changed: docs/testing/TEST_COVERAGE_MATRIX.md
Validation: node --check app.js launchView.js telemetryView.js domHelpers.js; apps\desktop\runtime\Python\python.exe -m unittest tests.python.desktop.test_application_facade_web_static tests.webview.test_webview_css_design_tokens tests.python.desktop.test_application_facade_queue tests.python.desktop.test_application_facade_completed tests.python.desktop.test_rerun_csv_preview -q
...
**Module**: `runMonitorView.js`, `crossPageContextView.js`, `crossPageContextView.conflict.js`, `crossPageContextView.sample.js`, `crossPageContextView.settings.js`, `crossPageContextView.sampleValidation.js`
**API routes**: `GET /api/run-monitor`, `GET /api/snapshot`, `GET /api/telemetry`, `GET /api/sample-validation`, `GET /api/commands`; existing `POST /api/pipeline/control` owns Stop After Current

| Coverage type | Files / Wrappers |
|---|---|
| Python unit tests | `test_run_monitor_contract.py`, `test_service_run_monitor.py`, `test_application_facade_local_api_http.py`, `test_api_contract_payload.py`, `test_api_route_inventory.py`, `test_facade_status_policy.py`, `test_service_status_progress.py`, `test_sample_validation_api.py`, `test_service_status_events.py`, `test_service_status_summary.py`, `test_application_facade_snapshot.py` (versioned monitor route/projection plus legacy snapshot telemetry), `test_application_facade_web_static.py` and `test_webview_run_monitor_static.py` (one backend-owned Current Work surface, retired competing panels, focus/live-region/route-label boundaries) |
...
| Browser-backed smokes | `Test-WebViewBrowserQueueLaunchCompletedSmoke.ps1` (standard Backend Queue journey), `test_webview_browser_run_monitor_smoke.py` (deterministic Current Work state/accessibility/responsive matrix, computed AA contrast, visible stacked track labels, plural active files, exact worker/terminal-proof focus retention), `Test-WebViewBrowserHighRiskSmoke.ps1` (blocked/warning row guidance), `Test-WebViewBrowserDiagnosticsHandoffSmoke.ps1` (row selection and combined row review plans across tables), `Test-WebViewBrowserSampleValidationSmoke.ps1` (Home Sample Validation evidence), `Test-WebViewBrowserHomeLiveStateSmoke.ps1` (legacy supporting telemetry/worksheet regressions), `Test-WebViewBrowserLaunchQueueReadinessSmoke.ps1` (launch/readiness and separate Single File regression) |

**Gaps**: Browser fixtures prove rendering and interaction against backend-shaped evidence but do not execute the PowerShell media engine, FFmpeg, ffprobe, MKVToolNix, OCR, real publication, or representative media. The real-media gate above remains required before release readiness is declared.

---

### Telemetry / supporting progress
...
**Gaps**: This page is secondary hardware/supporting telemetry. It is not current-stage authority and may not repopulate Current Work from logs, FFmpeg parsing, or stale legacy progress. Stale-progress timing threshold calculation remains unit-level/service-policy coverage.
...
**Module**: `queueView.js`
**API routes**: `GET /api/queue`, `POST /api/queue/scan`, `GET/POST /api/queue/priority`, `GET/POST /api/queue/strategy`, `GET/POST /api/queue/file-overrides`, `POST /api/queue/file-overrides/route-preview`, `POST /api/queue/file-overrides/series-preview`, `POST /api/queue/file-overrides/series-apply`, `POST /api/queue/file-overrides/series-clear-preview`, `POST /api/queue/file-overrides/series-clear-apply`, `POST /api/queue/open`, `POST /api/rerun/preview`, `GET /api/rerun/results`, `POST /api/rerun/start`, `POST /api/rerun/control`, `POST /api/rerun/continue`, `POST /api/rerun/open`, `POST /api/rerun/promote-dry-run`, `POST /api/rerun/promote`

| Coverage type | Files / Wrappers |
|---|---|
| Python unit tests | `test_facade_queue_policy.py`, `test_service_queue_snapshot.py`, `test_service_queue_preview_builder.py`, `test_service_queue_dry_run.py`, `test_service_queue_dry_run_runner.py` (unique temporary output, atomic promotion, request/input provenance, cached fallback labeling, and cleanup), `test_application_facade_queue.py` (read-only snapshot behavior, stale/fallback/input evidence, blocked-vs-excluded rows, runtime correlation, and row-key open allowlists), `test_application_facade_local_api_queue.py` (queue state route contract, source-root scope, and command history), `test_application_facade_launch_preflight.py` (advisory snapshot age plus hard provenance/scope/input/membership/plan gates), `test_application_facade_process_launch.py` (accepted Queue-run identity and durable duplicate guard), `test_application_facade_local_api_process_commands.py` (accepted/running/rejected and terminal command-ID correlation), `test_rerun_lifecycle.py`, `test_process_rerun_results.py`, `test_api_contract_payload.py` |
...
| Browser-backed smokes | `Test-WebViewBrowserHighRiskSmoke.ps1` (blocked row risk signals), `Test-WebViewBrowserDiagnosticsHandoffSmoke.ps1` (row click, combined review plan, filter warnings, clear-filter buttons), `Test-WebViewBrowserLargeTableSmoke.ps1` (250/260 render cap, filter scope evidence, Launch Decision daily-use handoff/filter evidence, Backend Launch Scope Preview boundary, selected-row at-a-glance summary), `Test-WebViewBrowserLaunchQueueReadinessSmoke.ps1` (Queue/Launch/Schedule readiness with explicit verified planned-name evidence), `Test-WebViewBrowserQueueLaunchCompletedSmoke.ps1` (two distinct raw release-name sources plus blocked/excluded rows; `mode=once` with blank Single File; accepted fingerprints/run identity; folded verified clean names and expanded selection; real Run Monitor starting/scanning/two-worker/handoff/review/completion states; durable duplicate/lifecycle rejection; focus, keyboard, non-color, clean-name live-announcement behavior; fresh idle; terminal reload; safe close-readiness; unchanged source hashes) |

**Gaps**: The disposable-root safe-operator browser smoke exercises all three Queue row-open targets through `POST /api/queue/open`, plus all displayed Completed and Pending row-open targets. Priority marker display remains not browser-smoke-tested. Queue priority/strategy/file-override route contracts, source-scope rejection, and command journaling are covered by Local API unit tests, not by the Queue → Launch → Completed browser flow. That flow uses a synchronized fake runner and temporary evidence; it does not prove PowerShell/FFmpeg execution or real-media routing/output quality.

**Pipeline regression coverage**: Queue-plan legacy priority compatibility, ordered queue schema keys, pending-manifest source-path matching, full-plan fingerprinting, active/dry fingerprint mismatch exit-before-dispatch, matching-plan continuation, and local worker-slot queue-engine dispatch/fail-closed behavior are covered by `ops/pipeline/tests/Invoke-ReliabilityRegressionChecks.ps1`, `ops/pipeline/tests/Invoke-EndToEndSmokeChecks.ps1`, `ops/pipeline/tests/Unit/Invoke-ContractSchemaChecks.ps1`, `ops/pipeline/tests/Unit/Invoke-PendingPublishSafetyChecks.ps1`, and `ops/pipeline/tests/Unit/Invoke-PipelineQueueEngineChecks.ps1`.
...
| Browser-backed smokes | `Test-WebViewBrowserHighRiskSmoke.ps1` (broken row guidance), `Test-WebViewBrowserDiagnosticsHandoffSmoke.ps1` (row click, combined review plan, filter warnings), `Test-WebViewBrowserLargeTableSmoke.ps1` (250/260 cap, output acceptance daily-use handoff/filter scope, selected-row at-a-glance summary), `Test-WebViewBrowserCompletedPendingProofSmoke.ps1` (proof ladder, saved-policy reconciliation, Sample Validation handoff, overlap proof, publish reconciliation, missing-output blocker, same-leaf duplicate-title), `Test-WebViewBrowserQueueLaunchCompletedSmoke.ps1` (Backend Queue accepted-run monitor handoff to temporary Completed Manifest/fake output/sidecar plus review failure artifact, durable command evidence, visible Completed deep link, terminal reload, and source-hash preservation) |

**Gaps**: Size-growth outlier rendering not smoke-exercised; route-agreement check rendering has partial coverage only via `RealMediaEvidence`. The new flow proves Completed state persistence, not container validity, stream policy, playback, hash verification, or output quality.

**Runtime state hygiene**: current Completed facade/static coverage verifies the `LocalBase\State\App` projection path, while `ops/pipeline/tests/Unit/Invoke-RuntimeStateHygieneChecks.ps1` prevents the legacy `DesktopApp\encode_speed_history.json` app-root runtime state from returning and guards the inventory/ignore/test coverage for this state file.
...
**Module**: `renameView.js`, `renameLabels.js`, `renameHistoryView.js`, and `assets/rename/*.js`
**API routes**: `POST /api/rename/browse` (native path picker, non-mutating), `POST /api/rename/preview`, `POST /api/rename/filter-cases`, `POST /api/rename/apply`, `POST /api/rename/undo` (apply/undo are guarded and never exercised by smokes)

| Coverage type | Files / Wrappers |
|---|---|
| Python unit tests | The 20 `test_*rename*.py` suites plus `test_api_path_dialogs.py`, `test_stage_contracts.py`, and `test_stage_runner.py`; detailed ownership and the auditable count are in `docs/inventories/RENAME_SAFETY_TEST_INVENTORY.md`. Coverage includes cleaning filters, revisions, bare anime episodes, canonical ranges and semantic overlaps, ordinal seasons/specials, numeric-title false positives, preview/apply parity, strict API confirmations, active-work blocking, path authority, on-disk media-plus-sidecar apply/undo, bad-case corpus output, WebView contracts, and scratch-stage journals/rollback. |
| PowerShell focused checks | `ops/pipeline/tests/Unit/Invoke-NamingSupportChecks.ps1` (movie/TV cleaning parity, numeric and metadata-like titles, uploader revisions, multi-episode ranges, ordinal seasons/specials, exact SubsPlease destination planning, sidecars, and identity keys); `ops/pipeline/tests/Unit/Invoke-PipelineQueueEngineChecks.ps1` (parse-once Queue rows, exact SubsPlease `12v2` parsing/order, and unreliable-row blocking) |
...
**Gaps / limits**: `rename.apply` and `rename.undo` are never invoked by smokes (by design — they mutate the filesystem), but both are exercised with isolated temporary media and sidecars in unit/integration tests. The scratch dispatcher uses minimal fixtures rather than representative media because rename does not inspect container streams. Automated smokes do not open the blocking native Windows dialog, TV folder rename is not exercised in smokes, and coordinator/worker path rewriting remains unverified for rename.
...
**API routes**: `GET /api/launch/preflight`, `POST /api/pipeline/start`, `POST /api/pipeline/control`, `POST /api/audit/start`

| Coverage type | Files / Wrappers |
|---|---|
| Python unit tests | `test_facade_process_pipeline_policy.py`, `test_application_facade_process_launch.py` (pipeline/audit/rerun handoff, strict failed-before-manifest supersession, verified-versus-ambiguous cleanup, normal-Queue dry-run provenance/input/plan gate, duplicate rejection, launch-lock and schedule behavior, and original-source policy rejection), `test_application_facade_launch_preflight.py` (backend-authored pipeline/audit/rerun readiness, Queue-plan provenance, path health, schedule/lock blockers, and encoder capability evidence), `test_application_facade_network_lifecycle.py` (Network-role launch rejection plus coordinator/worker lifecycle provider, strict-journal, duplicate, concurrency, and redaction behavior), `test_application_facade_network_rerun_launch.py` (Network rerun dry-run/start, claim-enabled state, claim/done/release handoff, stale fingerprint rejection, and journal cleanup), `test_application_facade_local_api_process_commands.py` (start-running and observer-recorded terminal command evidence), `test_service_process_spawn_runner.py` and `test_service_process_kill.py` (verified watcher cleanup, watcher-start failure, degraded process-tree retention, and normal kill success), `test_application_facade_local_api_http.py`, `test_application_facade_web_static_launch.py`, `test_application_facade_web_static_queue.py`, `test_webview_launch_command_buttons_smoke.py`, `test_facade_process_control_policy.py`, `test_application_facade_process_control.py`, `test_facade_process_guard_policy.py`, `test_application_facade_queue.py`, `test_facade_process_rerun_policy.py`, `test_service_process_readiness.py`, `test_service_process_launch_runner.py`, `test_service_process_launch_plans.py`, `test_service_process_launch_env.py`, `test_process_rerun_results.py`, `test_api_contract_payload.py` |
...
| Browser-backed smokes | `Test-WebViewBrowserSettingsLaunchSmoke.ps1` (Settings-to-Launch intent, Settings Effective Policy Trust, selectable Launch Risk Handoff proof-chain detail, staged subtitle/audio rows, filter-scope evidence for blocked rows, Preview Patch called, Save Patch not posted), `Test-WebViewBrowserLaunchQueueReadinessSmoke.ps1` (Launch readiness prefers backend `operator_readiness` from `GET /api/launch/preflight`, Launch timing trust, Launch Scope Reconciliation, Launch Start Decision Summary, Launch Real-Media Sample Proof Handoff including generated worksheet, Sample Validation record selected-sample match evidence, saved-policy-vs-Queue-route evidence, Launch Sample Execution Checklist, Launch Pilot Run Readiness, backend preflight rendering, launch history/review correlation, and no POST routes during readiness rendering), `Test-WebViewBrowserQueueLaunchCompletedSmoke.ps1` (fresh authoritative Backend Queue plan accepted for `mode=once` with blank Single File, run/fingerprint correlation, second start durably rejected, real Run Monitor transition sequence, synchronized fake-runner terminal evidence, and safe close-readiness/fresh idle after reload) |

**Gaps**: The Queue → Launch → Completed smoke submits `pipeline/start` only against a temporary backend whose synchronized fake runner launches no PowerShell child, FFmpeg, or media tool; it is not a real pipeline start. PowerShell end-to-end smoke exercises `-Once` and `-DrainPendingPushes`; `validate`, `continuous`, `audit/start`, `rerun/start`, `rerun/continue`, `pipeline/control`, and rerun-control route submissions are not browser-smoke-exercised. Launch history rendering has browser coverage.
...
**API routes**: `GET /api/failures`, `GET /api/audit-results`, `POST /api/audit/start` (not exercised), `POST /api/audit/export-rerun-csv`
...
| PowerShell focused checks | `ops/pipeline/tests/Unit/Invoke-PortablePathChecks.ps1` (audit/legacy GUI scripts parse and do not default audit roots to local operator UNC paths); `ops/pipeline/tests/Unit/Invoke-AuditCommandSupportChecks.ps1` (bundled `ffprobe` discovery from a generated temporary promoted-entrypoint layout); `ops/pipeline/tests/Unit/Invoke-RerunSourceIdentityChecks.ps1` (CSV rerun source identity remains deterministic and sample-hash-sensitive when `ffprobe` is unavailable); `ops/pipeline/tests/Unit/Invoke-RerunCooperativeStopChecks.ps1` (CSV rerun stop marker placement, stale marker guards, post-chunk manifest stop evidence, and continuation guidance); `ops/pipeline/tests/Unit/Invoke-RerunRecoveryChecks.ps1` (typed initial/mid-copy source loss including thrown/missing-landed failures, durable landed-scratch identity proof, mutating Robocopy flag rejection, mandatory drive/UNC full-chain and dangling-reparse guards, ambiguity-preserving cleanup, restart/correlation ordering, and PlanOnly no-write behavior); `ops/pipeline/tests/Unit/Invoke-RerunNestedConfigChecks.ps1` plus `Invoke-RerunPlanOnlyChecks.ps1` (live Movies/TV staging-root creation without PlanOnly artifacts); `ops/pipeline/tests/Unit/Invoke-RerunAutoDestinationChecks.ps1` (auto-return clean replacement, remaining-issue Pending Publish review routing, and source preservation with a mocked nested pipeline) |
| Non-browser smokes | None |
| Browser-backed smokes | `Test-WebViewBrowserMaintenanceReportsSmoke.ps1` (failure/audit triage, grouped failure resolution, More actions, marker clear preview/confirm, evidence archive preview/confirm with reason/fingerprint, selected-row evidence, cleanup-route allowlist) |

**Gaps**: Report-open diagnostics targets are not individually exercised;
`audit/start` remains intentionally unexercised by browser smokes because it is
a backend-owned process-launch mutation. Bundled-tool resolution is retained
by the temporary-layout PowerShell check, but a full isolated synthetic audit
workflow is still separate. CSV rerun start is covered from Queue, not Reports.
Automated recovery uses isolated fixtures and mocked copy/process boundaries;
representative real-media validation and an operator-controlled storage
disconnect/reconnect run remain required release gates.
...
| Browser-backed smokes | `Test-WebViewBrowserMaintenanceReportsSmoke.ps1` (Maintenance health/readiness, release dry-run result rendering, completed-manifest backfill dry-run result rendering, dry-run history, Reports failure-marker clear preview/confirm, evidence archive preview/confirm, cleanup-route allowlist); `Test-WebViewBrowserMaintenanceChangeLedgerSmoke.ps1` (Change Ledger summary/table/detail/hygiene, filters, empty state, read-only `GET /api/maintenance/change-ledger`, no media/queue/settings/pending-publish/rename mutation posts) |
...
| Coverage type | Files / Wrappers |
|---|---|
| Python unit tests | `test_facade_diagnostics_policy.py`, `test_facade_diagnostics_open_policy.py`, `test_application_facade_diagnostics.py` (diagnostics open/tail allowlists, read-only state-summary artifact evidence, and diagnostics path lookup failure logging), `test_application_facade_local_api_diagnostics.py` (backend diagnostics tail/state-summary route handling), `test_application_facade_web_static_diagnostics_reports.py` (WebView advisory fallback static contract) |
...
| Browser-backed smokes | `Test-WebViewBrowserSettingsLaunchSmoke.ps1` (staged settings, Settings Effective Policy Trust, Settings-to-Launch intent, Launch Risk Handoff proof-chain detail, filter-scope evidence, Preview Patch called, Save Patch not posted), `Test-WebViewBrowserSettingsFieldMatrixSmoke.ps1` (metadata-derived accounting for all 204 backend fields, activation of all ten tabs, all 155 structured bindings and 77 eligible Library overrides exercised, representative field equivalence classes, one temp-config save/reload from a two-key candidate plus six deterministic rename-filter compatibility keys, and unchanged media sentinels) |

**Gaps**: All ten Settings tabs are activated, every backend field is metadata-accounted, all structured builder bindings and eligible Library overrides are exercised, and unstructured/direct-only fields use representative equivalence classes rather than one interaction per field. High-risk field risk-overlay rendering remains only partially covered by the Settings/Launch scenario. Native Settings folder Browse is route/static/live-DOM verified but not clicked in browser smoke because it opens the OS dialog. The field-matrix save/reload is isolated to temporary config and does not prove production configuration migration or media-policy outcomes.
...
| Queue | Good | 2 wrappers | 5 workflows | Good; includes file overrides and isolated Queue-to-Completed handoff |
...
| Launch | Good | 2 wrappers | 3 workflows | Good for UI/start guards; one once-mode start uses a synchronized fake runner, not the production PowerShell/FFmpeg pipeline |
| Audit / Reports | Good | **None** | 1 wrapper | Partial |
| Schedule | Good | 1 wrapper | 1 wrapper | Good |
| Network | Good | None | 1 wrapper | Partial |
| Maintenance | Good | 1 wrapper | 2 workflows | Good for dry-run and evidence flows; real repair deferred |
| Diagnostics | Good | 1 wrapper | 2 workflows | Good for handoff plus temporary-state lifecycle reconciliation; native abnormal-exit UX remains separate |
| Settings | Excellent | 3 wrappers | 3 workflows | Good; includes Library Profiles save/reload and the metadata-derived field matrix against temporary configs |

Browser workflow attribution is intentionally non-exclusive: cross-page
wrappers can contribute to more than one row. The generated smoke-wrapper map
is authoritative for the current 25 canonical browser wrappers; this table summarizes
page-level behavioral coverage rather than duplicating that registry.
...
| Gap | Risk | Mitigation |
|---|---|---|
| Production PowerShell/FFmpeg launch execution for `once`/`continuous`/`validate` | Medium | The isolated Queue/Launch/Completed browser workflow exercises one once-mode `/api/pipeline/start`, duplicate rejection, and fake completion; production child execution and the other modes still require pipeline/real-media validation |
...
| Real-media route/remux/encode proof | High | Requires `docs/sample-validation/REAL_MEDIA_PILOT_CHECKLIST.md`; smokes use generated fixtures only |
| Stuck/orphan production-pipeline recovery and native Tauri UX | High | The lifecycle-reconciliation browser smoke covers a real temporary helper PID, abnormal exit, stale fingerprint rejection, exactly-once archive, reload, and safe close-readiness; `Invoke-AdversarialForceKillEncodeChecks.ps1` proves a force-killed encode partial is not accepted, while real pipeline-child reconciliation in the native shell still needs broader runtime coverage |
...
Files inspected: tests/webview/test_webview_browser_*.py, tests/webview/webview_browser_smoke_support.py
Files changed: docs/testing/TEST_COVERAGE_MATRIX.md
...
## Freshness Review - 2026-07-02 (Queue Current Coverage)

The old desktop-shell Queue controller split has been superseded by backend-served facade/WebView coverage. Current Queue behavior is covered by `test_application_facade_queue.py`, `test_application_facade_local_api_queue.py`, `test_process_rerun_results.py`, and WebView static/smoke tests: queue snapshot reads, first-class CSV rerun queue-state projection from `/api/rerun/results`, CSV rerun manifest fallback when normal queue rows are empty, priority/file override contracts, stale evidence, runtime outcome correlation, and row-key open allowlists.

```text
Task ID: Queue current coverage refresh
Files inspected: tests/python/desktop/test_application_facade_queue.py; tests/python/desktop/test_application_facade_local_api_queue.py
Files changed: docs/testing/TEST_COVERAGE_MATRIX.md
Validation: apps\desktop\runtime\Python\python.exe -m unittest tests.python.desktop.test_application_facade_queue tests.python.desktop.test_application_facade_local_api_queue -q
```
...
The 2026-06-24 follow-up split retired `test_application_facade_local_api.py` to a no-test orientation module. Local API server coverage now lives in focused `test_application_facade_local_api_*.py` modules for HTTP/auth, diagnostics, network, repair, settings, lifecycle, process commands, queue, rename, and workflow contracts. Backend-served WebView/Tauri static coverage now lives in focused `test_application_facade_web_static_*.py` modules for shell, completed, queue, pending publish, settings, launch, rename, diagnostics/reports, and cross-page contracts. `test_application_facade.py` is now a shared fixture/helper orientation module for split application-facade tests and intentionally owns no tests.
...
## Freshness Review - 2026-07-02 (Completed Current Coverage)

The old desktop-shell Completed controller split has been superseded by backend-served facade/WebView coverage. Current Completed behavior is covered by `test_application_facade_completed.py`, `test_application_facade_web_static_completed.py`, `test_facade_completed_policy.py`, and completed service tests: completed-row projection, proof/validation state, route/size/runtime classification, missing-output handling, open allowlists, and static Completed page rendering.

```
Task ID: Completed current coverage refresh
Files inspected: tests/python/desktop/test_application_facade_completed.py; tests/python/desktop/test_application_facade_web_static_completed.py; tests/python/desktop/test_facade_completed_policy.py
Files changed: docs/testing/TEST_COVERAGE_MATRIX.md
Validation: apps\desktop\runtime\Python\python.exe -m unittest tests.python.desktop.test_application_facade_completed tests.python.desktop.test_application_facade_web_static_completed tests.python.desktop.test_facade_completed_policy -q
```
...
## Freshness Review — 2026-07-02 (CSV Rerun Current Coverage)

Legacy desktop-shell controller split references are historical. Current CSV rerun coverage lives in `test_rerun_csv_preview.py`, `test_application_facade_process_launch.py`, `test_application_facade_queue.py`, `test_application_facade_web_static_queue.py`, `test_application_facade_web_static_launch.py`, `test_webview_browser_launch_queue_readiness_smoke.py`, `test_webview_launch_command_buttons_smoke.py`, `test_process_rerun_results.py`, `ops/pipeline/tests/Unit/Invoke-RerunCooperativeStopChecks.ps1`, and `ops/pipeline/tests/Unit/Invoke-RerunAutoDestinationChecks.ps1`: disabled/blocked rows, unsafe policy overrides, duplicate rows, path/existence/extension blockers, backend process-launch/preflight readiness, Queue CSV Rerun tab ownership, Launch tab removal, first-class Queue CSV rerun status mapping, empty-normal-queue visibility, action routing through `/api/rerun/*`, manifest row operator status, terminal stdout activity detection, cooperative stop marker handling, pending-only continuation CSV materialization, active-rerun controls, rerun result promotion, and auto-return clean replacement versus remaining-issue Pending Publish review routing.

```
Task ID: CSV rerun current coverage refresh
Files inspected: tests/python/desktop/test_rerun_csv_preview.py; tests/python/desktop/test_application_facade_process_launch.py; tests/python/desktop/test_application_facade_queue.py; tests/python/desktop/test_application_facade_web_static.py; tests/python/desktop/test_process_rerun_results.py
Files changed: docs/testing/TEST_COVERAGE_MATRIX.md
Validation: apps\desktop\runtime\Python\python.exe -m unittest tests.python.desktop.test_rerun_csv_preview tests.python.desktop.test_application_facade_process_launch tests.python.desktop.test_application_facade_queue tests.python.desktop.test_application_facade_web_static tests.python.desktop.test_process_rerun_results -q
```
...
Files inspected: src/mediapipeline/desktop/api/static_files.py; src/mediapipeline/desktop/api/static_files_policy.py; apps/desktop/webview/static/index.html; apps/desktop/webview/static/partials/page-home.html; tests/python/desktop/test_api_static_files_policy.py; tests/webview/test_webview_browser_home_live_state_smoke.py; tests/webview/test_webview_browser_sample_validation_smoke.py; tests/webview/test_webview_command_evidence_smoke.py; tests/webview/test_webview_real_media_smoke.py
Files changed: docs/testing/TEST_COVERAGE_MATRIX.md
...
Files inspected: src/mediapipeline/desktop/api/static_files.py; src/mediapipeline/desktop/api/static_files_policy.py; apps/desktop/webview/static/index.html; apps/desktop/webview/static/partials/page-settings.html; tests/python/desktop/test_api_static_files_policy.py; tests/webview/test_webview_browser_settings_launch_smoke.py; tests/webview/test_webview_browser_launch_queue_readiness_smoke.py; tests/webview/test_webview_browser_sample_validation_smoke.py
Files changed: docs/testing/TEST_COVERAGE_MATRIX.md
Validation: apps\desktop\runtime\Python\python.exe -m unittest tests.python.desktop.test_api_static_files_policy tests.webview.test_webview_css_design_tokens tests.webview.test_webview_navigation_static tests.webview.test_webview_inventory_docs tests.webview.test_webview_frontend_mutation_boundary tests.webview.test_webview_network_read_only_boundary -q
...
Files inspected: src/mediapipeline/desktop/api/static_files.py; src/mediapipeline/desktop/api/static_files_policy.py; apps/desktop/webview/static/index.html; apps/desktop/webview/static/partials/page-diagnostics.html; tests/python/desktop/test_api_static_files_policy.py; tests/webview/test_webview_browser_diagnostics_handoff_smoke.py; tests/webview/test_webview_browser_lifecycle_smoke.py; tests/webview/test_webview_command_evidence_smoke.py
Files changed: docs/testing/TEST_COVERAGE_MATRIX.md
Validation: apps\desktop\runtime\Python\python.exe -m unittest tests.python.desktop.test_api_static_files_policy tests.webview.test_webview_css_design_tokens tests.webview.test_webview_navigation_static tests.webview.test_webview_inventory_docs tests.webview.test_webview_frontend_mutation_boundary tests.webview.test_webview_network_read_only_boundary -q
...
Files inspected: src/mediapipeline/desktop/api/static_files.py; src/mediapipeline/desktop/api/static_files_policy.py; apps/desktop/webview/static/index.html; apps/desktop/webview/static/partials/page-pending.html; tests/python/desktop/test_api_static_files_policy.py; tests/webview/test_webview_browser_pending_drain_guard_smoke.py; tests/webview/test_webview_browser_completed_pending_proof_smoke.py; tests/webview/test_webview_browser_diagnostics_handoff_smoke.py; tests/webview/test_webview_row_detail_smoke.py
Files changed: docs/testing/TEST_COVERAGE_MATRIX.md
Validation: apps\desktop\runtime\Python\python.exe -m unittest tests.python.desktop.test_api_static_files_policy tests.webview.test_webview_css_design_tokens tests.webview.test_webview_navigation_static tests.webview.test_webview_inventory_docs tests.webview.test_webview_frontend_mutation_boundary tests.webview.test_webview_network_read_only_boundary -q
...
Files inspected: src/mediapipeline/desktop/api/static_files.py; src/mediapipeline/desktop/api/static_files_policy.py; apps/desktop/webview/static/index.html; apps/desktop/webview/static/partials/page-queue.html; tests/python/desktop/test_api_static_files_policy.py; tests/webview/test_webview_browser_large_table_smoke.py; tests/webview/test_webview_browser_launch_queue_readiness_smoke.py; tests/webview/test_webview_browser_diagnostics_handoff_smoke.py; tests/webview/test_webview_row_detail_smoke.py
Files changed: docs/testing/TEST_COVERAGE_MATRIX.md
Validation: apps\desktop\runtime\Python\python.exe -m unittest tests.python.desktop.test_api_static_files_policy tests.webview.test_webview_css_design_tokens tests.webview.test_webview_navigation_static tests.webview.test_webview_inventory_docs tests.webview.test_webview_frontend_mutation_boundary tests.webview.test_webview_network_read_only_boundary -q
...
Files inspected: src/mediapipeline/desktop/api/static_files.py; src/mediapipeline/desktop/api/static_files_policy.py; apps/desktop/webview/static/index.html; apps/desktop/webview/static/partials/page-completed.html; tests/python/desktop/test_api_static_files_policy.py; tests/webview/test_webview_css_design_tokens.py; tests/webview/test_webview_browser_completed_pending_proof_smoke.py; tests/webview/test_webview_row_detail_smoke.py
Files changed: docs/testing/TEST_COVERAGE_MATRIX.md
Validation: apps\desktop\runtime\Python\python.exe -m unittest tests.python.desktop.test_api_static_files_policy tests.webview.test_webview_css_design_tokens tests.webview.test_webview_navigation_static tests.webview.test_webview_inventory_docs tests.webview.test_webview_frontend_mutation_boundary tests.webview.test_webview_network_read_only_boundary -q
...
Files inspected: src/mediapipeline/desktop/api/static_files.py; src/mediapipeline/desktop/api/static_files_policy.py; apps/desktop/webview/static/index.html; apps/desktop/webview/static/partials/page-launch.html; tests/python/desktop/test_api_static_files_policy.py; tests/python/desktop/test_application_facade.py; tests/webview/test_webview_real_media_smoke.py
Files changed: docs/testing/TEST_COVERAGE_MATRIX.md
...
Open questions: None.
Risk: Low — WebView markup include/static-test only; `launchView*.js`, `/api/launch/preflight`, `/api/pipeline/start`, `/api/pipeline/control`, `/api/audit/start`, backend launch policy, command preconditions, and process semantics are unchanged. CSV rerun operator controls now live under Queue while still using `/api/rerun/*`.
```
...
Files inspected: src/mediapipeline/desktop/api/static_files.py; src/mediapipeline/desktop/api/static_files_policy.py; apps/desktop/webview/static/index.html; apps/desktop/webview/static/partials/page-network.html; apps/desktop/webview/static/partials/page-maintenance.html; tests/webview/test_webview_network_read_only_boundary.py
Files changed: docs/testing/TEST_COVERAGE_MATRIX.md
Validation: apps\desktop\runtime\Python\python.exe -m unittest tests.python.desktop.test_api_static_files_policy tests.python.desktop.test_application_facade.ApplicationFacadeTests.test_local_api_static_file_helpers_render_bootstrap_and_assets tests.python.desktop.test_application_facade.LocalApiServerTests.test_local_api_serves_read_only_web_prototype tests.webview.test_webview_navigation_static tests.webview.test_webview_inventory_docs tests.webview.test_webview_frontend_mutation_boundary tests.webview.test_webview_network_read_only_boundary -q
