...
- `tests/python/desktop/test_repair_reconcile_apply.py` verifies confirmed apply fingerprint gating, backup/write behavior, strict payload validation, command journaling, no frontend path acceptance, Local API Completed manifest and sidecar metadata apply evidence that source/output/sidecar hashes remain unchanged, and Local API Pending repair evidence that manifest repair and orphan-payload apply write only backend-authorized manifest state while source/output/payload hashes and the pending drain summary remain unchanged.
...
The selected-row Pending Publish manifest repair and orphan-payload reconcile controls are covered by static mutation-boundary tests plus the Pending Publish browser smoke's bounded request-body assertions and no-media-mutation snapshot. Pending manifest repair and orphan-payload apply now have automated Local API temp-fixture evidence: the apply routes are journaled as repair commands, write only backend-authorized manifest state, leave source/output/payload hashes and the pending drain summary unchanged, and record no drain command. MP-CHANGE-2026-0622-005 adds generated-runtime bridge evidence for orphan-payload repair: the Local API apply creates only the backend-authorized payload-adjacent manifest, leaves source/output/payload unchanged before drain, records only the repair command, and `MediaPipeline.ps1 -DrainPendingPushes` then consumes that manifest, publishes the final output bytes, removes the parked payload/manifest, writes a successful drain summary, and leaves the source unchanged. The Completed selected-row manifest reconciliation and sidecar metadata repair controls are covered by static mutation-boundary tests plus the Completed/Pending proof browser smoke's no-media-mutation snapshot and bounded request-body assertions; Completed manifest and sidecar metadata apply now have automated Local API evidence that the routes are journaled as repair commands, write only their selected manifest/sidecar state with backup evidence, leave source/output/sidecar hashes unchanged, and record no drain command.
