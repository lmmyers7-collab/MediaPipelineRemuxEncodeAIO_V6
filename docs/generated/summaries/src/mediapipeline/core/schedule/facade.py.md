---
file: src/mediapipeline/core/schedule/facade.py
pipeline_stage: orchestration
token_priority: medium
owner_domain: schedule
last_modified: 2026-06-04
last_reviewed: 2026-06-04
sha256: b5dd6cce87d93bd07eb57e8cca353592e74d077b7e5d9ec6ccc4635dc07707f7
---
# `src/mediapipeline/core/schedule/facade.py`

**Purpose:** Schedule workspace facade adapter.

**Classes:** `ScheduleFacadeMixin`
**In-repo imports:** `mediapipeline.core.schedule.policy`, `mediapipeline.desktop.application.dto_base`, `mediapipeline.desktop.application.dto_commands`, `mediapipeline.desktop.application.dto_workspaces`, `mediapipeline.desktop.application.schedule_stop_watcher`

_Edit the source, not this file. Regenerate with `apps/desktop/runtime/Python/python.exe ops/scripts/dev/run-python-tool.py mediapipeline.tools.dev.refresh_summaries --paths src/mediapipeline/core/schedule/facade.py`._
