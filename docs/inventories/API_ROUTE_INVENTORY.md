...
| `GET /api/snapshot` | `none` | `desktop_app_snapshot.v1` | Home, all pages (poll) | Yes | `test_facade_status_policy.py`, `test_status_service.py`, `test_application_facade_snapshot.py`, `test_status_rerun_completion.py`; includes read-only long-run reliability counters, additive `current_work` active-work summary fields, and backend-manifest `csv_rerun_summary` terminal totals/identity so Home does not infer CSV completion from stale progress |
| `GET /api/run-monitor` | `none` | `desktop_run_monitor.v1` | Home Current Work | Yes | `test_service_run_monitor.py`, `test_application_facade_local_api_http.py`, `test_api_contract_payload.py`; optional exact `run_id` selects durable run history, while omission follows the validated latest pointer; invalid or duplicate identities are rejected and missing/partial/stale/contradictory evidence returns explicit unavailable/unknown/stale state without legacy current-work fallback. For legacy terminal items only, the backend may project a cleaned effective `display_name` from exact same-item terminal output plus an exact job-correlated Completed or Pending Publish reference; it returns the immutable stored label as `accepted_display_name` and the authority as `display_name_basis`. No frontend cleaning or current-Queue inference is permitted. |
...
| `GET /api/backend/recovery-status` | `none` | `desktop_lifecycle_recovery.v1` | Diagnostics / startup recovery evidence | Yes | `test_lifecycle_recovery.py`, `test_api_contract_payload.py` |
...
| `GET /api/launch/preflight` | `none` | `desktop_launch_preflight.v1` + nested `desktop_launch_readiness.v1` | Launch | Yes | `test_service_process_readiness.py`, `test_facade_process_pipeline_policy.py`, `test_application_facade_process_launch.py`, `test_application_facade_local_api_http.py`; blank Run Once adds `normal_queue_scope`: snapshot age/mtime/clock skew are advisory because runtime rebuilds and fingerprint-verifies the queue, while missing/invalid/provenance/scope/input/membership/pending-safety evidence and a validated zero-runnable plan remain backend blockers |
...
| `GET /api/rerun/results` | `none` | `desktop_rerun_results.v1` + nested `desktop_rerun_queue_state.v1` | Queue | Yes - bounded recent history reports loaded/discovered counts and scan warnings separately from backend-selected `current_local`/`current_network`; local current activity requires an exactly correlated live process, Network current activity requires an exact fresh persisted coordinator claim, and recovery actions remain backend-authored | `test_api_contract_payload.py`, `test_application_facade_process_launch.py`, `test_process_rerun_results.py`, `test_application_facade_web_static_queue.py` |
...
| `GET /api/completed` | `none` | `desktop_completed_preview.v1`, including bounded `parse_health` and omitted-row evidence | Completed | Yes | `test_facade_completed_policy.py`, `test_service_completed_manifest.py`, `test_repair_reconcile_dry_run.py` |
...
| `GET /api/metrics` | `none` | `desktop_metrics.v1`, including authoritative-history, cache parse-health, availability, and completeness evidence | Metrics | Yes | `test_metrics_feature.py` |
...
| `GET /api/failures` | `none` | `desktop_failure_preview.v1` + backend-authored full-set counts, hidden-blocker count, completeness/availability, `resolution_summary` / `resolution_groups`, and capped visible row `evidence_details` | Reports, Diagnostics | Yes | `test_facade_failures_policy.py`, `test_application_facade_local_api_workflow.py`, `test_service_failure_markers.py` |
...
| `POST /api/diagnostics/encoder-capabilities/refresh` | `diagnostics-artifact-write` | empty request only | Launch, Settings | Low — writes only bounded backend diagnostic evidence; no media launch or source mutation | `test_application_facade_process_launch.py`, `test_api_contract_payload.py` |
...
| `POST /api/rename/apply` | `filesystem-mutation` | `paths`, `selected_sources`, `preview_fingerprint`, `confirm_apply`, `allow_outside_configured_roots` | Rename | **High** — renames files on disk only after the backend recomputes and matches the fresh preview fingerprint and final configured-root authority | `test_application_facade_rename.py`, `test_service_rename_apply.py` |
...
| Route | Effect | Key Request Keys / Allowed Values | Frontend Caller | Mutation Risk | Backend Test Coverage |
|---|---|---|---|---|---|
| `POST /api/pipeline/control` | `control-flag-write` | `action`: `pause`, `stop`, `rescan`, `kill`; `expected_run_id` for Current Work stop; `confirm_force_stop` | Current Work, Launch | Medium — writes control flags; standard Run Once Backend Queue stop is accepted only when the displayed `expected_run_id` exactly matches one contract-backed active launch under the process-control lock; Continuous and Single File retain PID/launch correlation; `kill` requires literal `confirm_force_stop: true` and exact killed-process proof before Run Monitor takeover | `test_facade_process_control_policy.py`, `test_application_facade_process_control.py`, `test_application_facade_local_api_http.py`, `test_service_process_control_flags.py`, `test_service_process_kill.py`, `test_webview_browser_run_monitor_smoke.py` |
...
| `POST /api/pipeline/start` | `process-launch` | required typed `mode`: `once`/`continuous`/`validate`/`drain_pending_pushes`; `sleep_seconds`, `show_config`, `show_console`, `single_file`, `schedule_override` | Launch | **High** — spawns pipeline process; blank `once` scope requires a current backend dry-run and passes its plan fingerprint into an active-engine pre-dispatch parity check; durable lifecycle lease blocks duplicates; terminal command evidence is correlated by command ID | `test_facade_process_pipeline_policy.py`, `test_application_facade_process_launch.py`, `test_application_facade_local_api_process_commands.py`, `test_facade_process_guard_policy.py` |
...
| `POST /api/rerun/network-preview` | `read-only-preview` | `csv_path`, `execution_mode`, `destination_mode`, `collision_policy`, `window_size`, `confirm_replace_final`, `confirm_source_overwrite`, `scope` filters | Queue | None — models future network CSV rerun row claims from the backend CSV preview, including claimable/skipped/blocked/duplicate rows, source mapping readiness, `NetworkRerunHandoffRoot` readiness, destination policy risk, and explicit `writes_network_state=false` / `touches_media=false` evidence; it does not create handoff folders, start workers, or write batch state | `test_rerun_csv_preview.py`, `test_api_contract_payload.py`, `test_api_command_contracts.py` |
| `POST /api/rerun/network/start-dry-run` | `none` | `csv_path`, `execution_mode`, `destination_mode`, `collision_policy`, `window_size`, `confirm_replace_final`, `confirm_source_overwrite`, `scope` filters, `minimum_worker_count`, `reason` | Queue | None — validates a future network CSV rerun batch start without writing state: NetworkRole=coordinator, coordinator lifecycle readiness, active-work and duplicate-batch guards, destination/collision preview safety, `NetworkRerunHandoffRoot` readiness, remote-worker UNC compatibility, worker availability evidence, exact state file paths the confirmed start would write, rollback expectations, and no-touch media evidence | `test_application_facade_process_launch.py`, `test_api_contract_payload.py`, `test_api_command_contracts.py` |
| `POST /api/rerun/network/start` | `network-state-write` | `csv_path`, `dry_run_fingerprint`, `confirm_start` literal boolean `true`, network preview fields | Queue | High — after matching backend dry-run proof, strict confirmation, and a transient coordinator handoff-root create/read/list/delete probe, writes exactly one coordinator-owned `State\Rerun\Network\network-rerun-*.json` batch file plus strict command journal evidence with planned per-row handoff folders; rows become additive `csv_rerun_row` claim candidates for the coordinator, while the route itself does not directly launch workers, stage sources, mutate normal queue state, apply destination policy, mutate Pending Publish, replace final output, or touch source media | `test_application_facade_process_launch.py`, `test_application_facade_close_readiness.py`, `test_network_rerun_claims.py`, `test_api_contract_payload.py`, `test_api_command_contracts.py` |
| `POST /api/rerun/network/retry` | `network-state-write` | `batch_id`, `row_key`, `request_id`, nonblank `reason`, `confirm_retry` literal boolean `true` | Queue | High — reopens exactly one terminal `retry_exhausted` source-access row only after fresh coordinator source verification, no active claim, no output-ownership ambiguity, strict confirmation, idempotent request evidence, and the configured retry bound; preserves prior failure history and writes Network batch state plus strict command journal evidence without touching source media or output files | `test_network_rerun_claims.py`, `test_api_contract_payload.py`, `test_api_command_contracts.py`, `test_process_rerun_results.py` |
...
| `POST /api/backend/lifecycle/reconcile-dry-run` | `none` | `reason` | Diagnostics / Audit recovery | None — correlates the terminal failed-recovery chain, requires every recorded PID to be conclusively dead, and previews the exact archive transaction without writing lifecycle evidence or touching media files | `test_lifecycle_reconciliation_api.py`, `test_lifecycle_reconciliation.py` |
| `POST /api/backend/lifecycle/reconcile` | `lifecycle-evidence-reconciliation` | `confirm_apply`, `dry_run_fingerprint`, `reason` | Diagnostics / Audit recovery | **High** — after a matching dry-run fingerprint and literal confirmation, archives only the correlated failed lifecycle evidence through a guarded transaction with strict durable command evidence; it never touches media files | `test_lifecycle_reconciliation_api.py`, `test_lifecycle_reconciliation.py` |
...
| `diagnostics-artifact-write` | 2 | `POST /api/diagnostics/encoder-capabilities/refresh`, `POST /api/maintenance/support-export` |
...
| `network-state-write` | 2 | `POST /api/rerun/network/start`, `POST /api/rerun/network/retry` |
| `lifecycle-evidence-reconciliation` | 1 | `POST /api/backend/lifecycle/reconcile` |
...
| WebView smokes (25 canonical browser wrappers) | Integration rendering and mutation-boundary verification |
...
`/api/contract` exposes `stage_dispatcher_contract` as an ownership statement,
not a stage-execution API. There are no `/api/stage/*` routes. The Python
dispatcher supervises only enabled PowerShell stage contracts; it must not
accept frontend-selected mutation paths or replace `MediaPipeline.ps1` as the
owner of media, publish/drain, or rename behavior.
