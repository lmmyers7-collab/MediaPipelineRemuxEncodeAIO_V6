...
| `GET /api/snapshot` | Yes | `desktop_app_snapshot.v1` | Home, all pages (refresh) | Core pipeline/audit status payload plus read-only long-run reliability counters and backend-owned `current_work` active-work summary fields for current item, current/next stage, latest evidence/event, and missing/blocked evidence |
| `GET /api/run-monitor` | Yes | `desktop_run_monitor.v1` | Home Current Work | Backend owns accepted Run Once membership, exact run/job correlation, stage/track evidence, all-worker projection, freshness suppression, terminal references, and projection-only legacy terminal display-name reconciliation. One optional exact `run_id` selects history; omission uses the validated latest pointer. Durable accepted labels and membership are never rewritten. The API preserves the stored label as `accepted_display_name` and may use `display_name_basis=terminal_output` only when exact same-item terminal output and an exact job-correlated Completed or Pending Publish reference prove the filename. The frontend may select and render rows but cannot clean or infer names/current state from Queue order, the current Queue snapshot, filename similarity, routes, logs, or legacy progress. |
...
| `GET /api/rerun/results` | Yes | `desktop_rerun_results.v1` + nested `desktop_rerun_queue_state.v1` | Queue | Reads bounded recent local/Network CSV rerun history with discovered/loaded counts and per-file scan warnings, plus direct current local live-process correlation and fresh exact persisted Network claim evidence. Includes first-class Queue-facing status, recovery, reducer, destination, review, and import/scoped CSV evidence; no file movement or media mutation |
...
| `GET /api/completed` | Yes | `desktop_completed_preview.v1` | Completed | From local completed-jobs manifest with bounded parse-health and omitted-row evidence; no output-share scan |
...
| `GET /api/metrics` | Yes | `desktop_metrics.v1` | Metrics | Separates authoritative completed-manifest history from generated/cache discovery evidence and reports parse health, completeness, and availability; no launch, drain, promote, config save, queue mutation, or media touch |
...
| `GET /api/failures` | Yes | `desktop_failure_preview.v1` | Reports, Diagnostics | Query params: `source` (`latest_json` or `markers`), `limit`; full-set counts, hidden blockers, completeness, and availability are independent of capped visible rows |
...
| `GET /api/rename/clean-filename-preview` | Yes | `desktop_rename_clean_filename_preview.v1` | Rename | Read-only clean-filename preview and optional workbench comparison/suggestion payload. Workbench comparisons use the synthetic, no-media-probe production PowerShell destination plan as actual authority, bind the applied staged/saved cleaning-policy fingerprint, and fail closed when production or policy evidence is unavailable; accepts query fields and writes nothing. |
...
Allowed targets for `/api/diagnostics/open`: `run_logs`, `pipeline_log`, `cluster_log`, `config`, `config_folder`, `workspace`, `state`, `pending_publish`, `failed_reports`, `failed_markers`, `audit_reports`, `queue_snapshot`, `active_jobs`, `completed_manifest`, `latest_failure_report`, `latest_failure_json`, `latest_audit_csv`, `latest_priority_csv`, `last_stdout_log`, `last_stderr_log`, `sample_validation_log`.
...
| `POST /api/rename/apply` | `filesystem-mutation` | `paths`, `selected_sources`, `preview_fingerprint`, `confirm_apply`, `allow_outside_configured_roots` | **High** — filesystem rename after fresh backend fingerprint and final-root validation | Rename |
...
| `POST /api/audit/export-rerun-csv` | `report-file-write` | `row_keys`, `priority_only`, `limit` | Medium — writes a backend-owned rerun CSV artifact plus Queue CSV Rerun handoff metadata only; does not launch rerun work | Reports |
...
files. `audit/export-rerun-csv` may return a `desktop_audit_to_queue_rerun_handoff.v1`
...
| Route | Effect | Key Request Keys | Mutation Risk | Frontend Caller |
|---|---|---|---|---|
| `POST /api/pipeline/control` | `control-flag-write` | `action` (`pause`, `stop`, `rescan`, `kill`), `expected_run_id`, `confirm_force_stop` | Medium — Current Work sends its exact run ID for standard Run Once Backend Queue stop; the backend re-correlates run/PID/launch/scope under lock and rejects stale, blank, or mismatched identity before writing a flag. Continuous and Single File use exact PID/launch correlation. `kill` still requires literal confirmation and verified captured-tree exit before terminalizing a Run Monitor. | Current Work, Launch |
...
| `POST /api/pipeline/start` | `process-launch` | required typed `mode`, `sleep_seconds`, `show_config`, `show_console`, `single_file`, `schedule_override` | **High** — blank `once` requires a completed current dry-run; the gate is repeated inside the durable lease and active PowerShell discovery must match the accepted plan fingerprint before dispatch; pending-drain eligibility remains backend-owned | Launch |
...
| `POST /api/rerun/preview` | `read-only-preview` | `csv_path`, `execution_mode`, `destination_mode`, `collision_policy`, `window_size`, `confirm_replace_final`, `confirm_source_overwrite`, `scope` filters | None — backend parses and summarizes CSV rerun rows, rule decisions, import/scoped CSV candidates, lifecycle warnings, filter options, tiles, and scoped counts without launch or media mutation | Queue |
| `POST /api/rerun/network-preview` | `read-only-preview` | `csv_path`, `execution_mode`, `destination_mode`, `collision_policy`, `window_size`, `confirm_replace_final`, `confirm_source_overwrite`, `scope` filters | None — backend models future network CSV rerun row claims, source mapping readiness, `NetworkRerunHandoffRoot` readiness, duplicate/blocked/skipped rows, and destination policy risk without launch, handoff folder creation, worker claim, network state write, queue mutation, or media touch | Queue |
| `POST /api/rerun/network/start-dry-run` | `none` | `csv_path`, `execution_mode`, `destination_mode`, `collision_policy`, `window_size`, `confirm_replace_final`, `confirm_source_overwrite`, `scope` filters, `minimum_worker_count`, `reason` | None — backend validates the future network CSV rerun batch start contract, coordinator role/lifecycle readiness, active-work and duplicate-batch guards, `NetworkRerunHandoffRoot` readiness, remote-worker UNC compatibility, worker availability evidence, exact confirmed-start state file path, rollback expectations, and no-touch media evidence without writing network state or worker claims | Queue |
| `POST /api/rerun/network/start` | `network-state-write` | `csv_path`, `dry_run_fingerprint`, `confirm_start` | High — after matching backend dry-run proof, strict confirmation, and a transient coordinator handoff-root create/read/list/delete probe, writes only coordinator-owned `State\Rerun\Network\network-rerun-*.json` batch state plus command journal evidence; rows carry planned `<NetworkRerunHandoffRoot>/<batch_id>/<row_key>/` folders and become additive `csv_rerun_row` claim candidates, but the route itself does not directly launch workers, mutate normal queue state, apply destination policy, mutate pending publish, replace final output, or touch source media | Queue |
| `POST /api/rerun/network/retry` | `network-state-write` | `batch_id`, `row_key`, `request_id`, `reason`, `confirm_retry` | High — strict-confirmed, idempotent coordinator mutation reopens only an exact source-access `retry_exhausted` row after fresh source verification; active claims, output ambiguity/existence, review-only failures, and duplicate conflicts are rejected while failure history is preserved and media files remain untouched | Queue |
| `POST /api/rerun/start` | `process-launch` | `csv_path`, `dry_run`, `plan_only`, `execution_mode`, `destination_mode`, `collision_policy`, `window_size`, `confirm_replace_final`, `confirm_source_overwrite`, `scope` filters | **High** — spawns backend-owned CSV rerun v2; Queue posts the default normal-remediation mode `auto_replace_clean_else_pending_review`, scoped starts write under `State\Rerun\ScopedCsv`, backend rule-blocked rows cannot launch, WebView derives final-replacement confirmation from destination/collision selection, backend/PowerShell require strict boolean confirmation, clean/problem destination decisions stay backend-owned, and source-path overwrite additionally requires `confirm_source_overwrite` | Queue |
| `POST /api/rerun/control` | `process-control` | `action` (`stop_after_current`), `confirm_stop` | Medium — writes only the backend-owned rerun stop marker under `State\Rerun\Control`; PowerShell owns manifest mutation after the current row/window completes | Queue |
| `POST /api/rerun/continue` | `process-launch` | `manifest_key`, `confirm_continue` | **High** — writes a pending-only scoped CSV under `State\Rerun\ScopedCsv` and launches through existing CSV rerun locks; failed/review rows are left untouched | Queue |
| `POST /api/rerun/open` | `shell-open` | `target`, `row_key`, `csv_key` | Low — opens only backend-derived rerun review outputs, manifests, import/scoped CSVs, or folders; arbitrary filesystem paths are rejected | Queue |
| `POST /api/rerun/promote-dry-run` | `read-only-preview` | `row_key` | None — computes dry-run evidence for promoting an existing rerun review output into Pending Publish without moving files or writing manifests | Queue |
| `POST /api/rerun/promote` | `pending-manifest-write` | `row_key`, `dry_run_fingerprint`, `confirm_promote` | Medium — after matching dry-run evidence and strict confirmation, writes a `pending_move` manifest, moves an existing rerun review output into Pending Publish, updates the manifest to `parked`, and preserves known sidecar evidence; it does not directly publish or touch source media | Queue |
| `POST /api/backend/lifecycle/reconcile-dry-run` | `none` | `reason` | None — backend correlates terminal failed-recovery evidence and proves every recorded PID is dead without writing lifecycle evidence or touching media files | Diagnostics / Audit recovery |
| `POST /api/backend/lifecycle/reconcile` | `lifecycle-evidence-reconciliation` | `confirm_apply`, `dry_run_fingerprint`, `reason` | **High** — strict-confirmed, fingerprint-gated backend transaction archives only correlated failed lifecycle evidence and records strict durable command evidence; media files are never touched | Diagnostics / Audit recovery |
...
`pipeline/start` allowed modes: `once`, `continuous`, `validate`, `drain_pending_pushes`. `schedule_override` allowed values: `""`, `run_once`, `ignore`. `rerun/preview` backs auto-preview for the Queue CSV Rerun tab and is no-write. It also applies backend-owned CSV rerun rule decisions; bad download/source-integrity rows, manual-review rows, and missing rule-input rows are blocked before launch. `rerun/results` backs the Queue CSV rerun state table with backend-owned row statuses, rule decisions, and evidence while explicitly marking those rows as `uses_pipeline_start=false`. `rerun/start` remains CSV-rerun-specific; CSV rerun rows are not converted into normal queue rows and are not submitted to `/api/pipeline/start`. The Queue UI keeps final replacement controls visible and posts the default normal-remediation selection (`destination_mode=auto_replace_clean_else_pending_review`, `collision_policy=replace_final`, `confirm_replace_final=true`) while backend/PowerShell decide whether clean outputs replace the final target or issue-bearing outputs move to Pending Publish/review. Legacy `stage_mode`/`original_mode`/`return_mode` values are compatibility aliases only. `rerun/control` requires strict `confirm_stop: true` and writes only the backend-owned stop marker. `rerun/continue` requires strict `confirm_continue: true`, accepts only stopped-after-current manifests, and launches pending rows only. Source-path overwrite is opt-in; destination and collision determine verified-output placement. Final replacement remains strict-confirmation gated: the WebView derives `confirm_replace_final: true` from `destination_mode=auto_replace_clean_else_pending_review`, `destination_mode=publish_replace_final`, or `collision_policy=replace_final`; source-path overwrite additionally requires `confirm_source_overwrite: true`, and backend/PowerShell reject missing or non-boolean confirmation.
...
| `diagnostics-artifact-write` | `POST /api/maintenance/support-export`, `POST /api/diagnostics/encoder-capabilities/refresh` | Backend-owned bounded diagnostic artifacts only: redacted support exports under per-user AppData DiagnosticsExports and encoder-capability probe evidence under the configured diagnostics state root; neither route mutates media |
...
| `report-file-write` | `POST /api/audit/export-rerun-csv` | Writes a backend-owned report CSV artifact only and returns Queue handoff metadata |
...
| `process-control` | `POST /api/audit/stop`, `POST /api/rerun/control` | Confirmation-gated audit-only process cleanup or cooperative rerun stop marker write |
...
| `process-launch` | `POST /api/pipeline/start`, `POST /api/audit/start`, `POST /api/rerun/start`, `POST /api/rerun/continue` | Spawns backend processes |
| `network-state-write` | `POST /api/rerun/network/start`, `POST /api/rerun/network/retry` | Writes coordinator-owned Network CSV rerun batch state only after matching proof/eligibility, strict confirmation, duplicate guards, and command journal evidence |
| `lifecycle-evidence-reconciliation` | `POST /api/backend/lifecycle/reconcile` | Archives only a correlated terminal failed-recovery chain after a matching dry-run fingerprint, literal confirmation, and strict durable command evidence |
...
- **Pipeline/audit/rerun start, rerun continue, and process stop/control**: backend owns process launch/control, locks, command journal, audit-only cleanup scope, rerun stop markers, scoped continuation CSVs, and progress state; the frontend cannot exec or kill processes directly.
