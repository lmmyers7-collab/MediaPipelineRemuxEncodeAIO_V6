...
Total routes: 171 (54 read, 117 command).
...
| Route | Effect | Key Request Keys | Mutation Risk | Frontend Caller |
|---|---|---|---|---|
| `POST /api/pipeline/control` | `control-flag-write` | `action` (`pause`, `stop`, `rescan`, `kill`), `confirm_force_stop` | Medium — `kill` requires literal `confirm_force_stop: true`; other actions retain compatibility | Launch |
...
| `POST /api/pipeline/start` | `process-launch` | required typed `mode`, `sleep_seconds`, `show_config`, `show_console`, `single_file`, `schedule_override` | **High** — spawns pipeline process; absent mode is rejected and pending-drain eligibility is decided by backend preflight/drain logic | Launch |
