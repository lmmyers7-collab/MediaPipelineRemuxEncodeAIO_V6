...
- **288 reachable JS files** recursively inventoried under `assets/`: **83 root-level** and **205 nested**
- **56 files** contain **58 `window.mediaPipeline*` namespace assignments**
- **235 files** contain **532 flat `window.*` assignments**
...
- **2 HTML surfaces** reference every JS asset: `main` 287, `assets/pipelineLogWindow.html` 2; **1 script** is shared across surfaces
...
| `crossPageContextView.js` | `main` | mediaPipelineLastCrossPageContext, mediaPipelineCrossPageContextView | 37 |
...
| `diagnosticsView.js` | `main` | mediaPipelineDiagnosticsView | 57 |
...
| `runMonitor/formatters.js` | `main` | - | 1 |
| `runMonitor/interaction.js` | `main` | - | 1 |
| `runMonitor/normalization.js` | `main` | - | 1 |
| `runMonitor/rendering.js` | `main` | - | 1 |
...
| Criterion | Status |
| --- | --- |
| All recursively discovered JS assets inventoried | Pass - 288 files |
...
| Flat assignments identified per file | Pass - 532 assignments |
...
Generated by `apps/desktop/runtime/Python/python.exe ops/scripts/dev/run-python-tool.py mediapipeline.tools.dev.generate_webview_inventory_docs` from every script referenced by the main and auxiliary HTML surfaces. Counts are assignment occurrences: **58 namespace** and **532 flat**.
