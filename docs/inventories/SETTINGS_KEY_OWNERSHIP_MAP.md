...
| `VideoCodec` | **High** | Encoder: `hevc_nvenc`, `hevc_qsv`, `hevc_amf`, `libx265`, `h264_nvenc`, `h264_qsv`, `h264_amf`, `libx264`, `av1_nvenc`, `av1_qsv`, `av1_amf`, or `libaom-av1`. Unsupported, HDR-unsafe, or inactive descriptor choices fail closed before FFmpeg planning. | Yes — active video codec row | `test_settings_risk_policy_rules.py`, `test_service_config_option_policy.py` |
| `EncoderBackend` | Medium | Dormant global backend preference: `auto`, `nvenc`, `qsv`, `amf`, or `cpu`. Default `auto` preserves literal `VideoCodec` behavior; this key currently feeds saved settings plus capability diagnostics only and does not activate new hardware families. | Yes — video detail settings builder; capability dump evidence only | `test_service_config_option_policy.py`, `test_webview_handbrake_settings_ui.py`, `Invoke-ConfigKeyRegistryChecks.ps1` |
...
| `AutonomyPendingTotalReviewBytes` | Medium | Backend-only diagnostics pending-publish byte threshold for review evidence (default 100 GiB). | Diagnostics/Home read-only evidence only | `test_autonomy_health.py` |
| `AutonomyPendingTotalBlockBytes` | **High** | Backend-only diagnostics pending-publish byte threshold for blocking new autonomous work (default 250 GiB). | Diagnostics/Home read-only evidence only | `test_autonomy_health.py` |
