...
function ConvertTo-StreamSummary {
    ...
    $language = if ($null -eq $tags) { '' } else { [string](Get-ObjectValue -Object $tags -Name 'language' -Default '') }
    $title = if ($null -eq $tags) { '' } else { [string](Get-ObjectValue -Object $tags -Name 'title' -Default '') }
    $isDefault = if ($null -eq $disposition) { $false } else { ((ConvertTo-IntValue (Get-ObjectValue -Object $disposition -Name 'default' -Default 0)) -eq 1) }
    $isForced = if ($null -eq $disposition) { $false } else { ((ConvertTo-IntValue (Get-ObjectValue -Object $disposition -Name 'forced' -Default 0)) -eq 1) }
