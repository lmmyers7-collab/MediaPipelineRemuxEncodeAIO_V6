...
$outputPathText = @(
    (Get-Content -LiteralPath (Join-Path $repoRoot 'ops\pipeline\engine\paths\effective_settings.ps1') -Raw)
    (Get-Content -LiteralPath (Join-Path $repoRoot 'ops\pipeline\engine\paths\output_evidence.ps1') -Raw)
) -join "`n"
...
Assert-True ($routingText -match 'function Get-MediaRouteRuleOutcomeEvidence' -and $routingText -match 'function New-MediaRouteExplanation' -and $routingText -match 'route_explanation' -and $routingText -match 'runtime_effective_settings_ref' -and $routingText -match 'library_effective_settings_ref' -and $routingText -match 'oversized_encode_remux_fallback') 'Expected route metadata helper to expose runtime provenance, library-only references, route explanations, and fallback-remux advisory evidence.'
