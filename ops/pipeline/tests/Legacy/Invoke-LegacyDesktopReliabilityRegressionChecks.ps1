...
Assert-True ($serviceQueueText -match 'mediapipeline\.core\.queue\.priority_markers' -and $serviceQueuePriorityText -match 'def get_source_priority_info' -and $serviceQueuePriorityText -notmatch 'def apply_priority_marker|def touch_priority_target|os\.utime|\.rename\(' -and $serviceQueueText -notmatch 'def apply_priority_marker|def touch_priority_target|def format_priority_leaf_name') "Queue priority compatibility must remain read-only; active priority authority is manifest-backed and source rename/retimestamp helpers must not exist."
