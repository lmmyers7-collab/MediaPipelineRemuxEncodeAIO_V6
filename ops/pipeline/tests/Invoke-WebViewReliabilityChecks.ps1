...
Assert-True ($staticFilesText -match 'resolve_asset_path' -and $staticFilesText -match 'render_static_includes' -and $staticFilesPolicyText -match 'http-only-cookie' -and $staticFilesPolicyText -match 'tauri-initialization-script' -and $indexText -match '__MEDIA_PIPELINE_BOOTSTRAP__' -and $indexText -match 'media-pipeline-bootstrap' -and $httpHelpersText -match 'MediaPipelineAuth' -and $tauriLibText -match 'tauri_bootstrap_initialization_script\(\s*backend\.token\(\)\s*,\s*backend\.startup_warnings\(\)\s*,\s*&allowed_origin\s*,?\s*\)' -and $tauriLibText -match '\.initialization_script\(initialization_script\)' -and $tauriBackendContractText -match 'web_shell::validate_web_shell\(backend_url, token\)' -and $tauriBackendWebShellText -match 'Backend WebView index leaked the bearer token') "Static WebView assets must be rendered through backend helpers while browser mode uses an HttpOnly cookie and Tauri injects the token outside public index HTML."
