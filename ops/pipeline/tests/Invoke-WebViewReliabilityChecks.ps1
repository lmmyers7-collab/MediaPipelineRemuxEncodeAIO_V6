...
Assert-True ($releaseVerifierText -match 'audit_checks emit release-self-test' -and $releaseVerifierText -match '\$auditCheckManifest\.checks' -and $releaseVerifierText -match 'Invoke-PythonModuleCheck' -and $releaseVerifierText -match '\$check\.source_tree_only' -and $releaseVerifierText -match '\$inReleasePackage') "Release self-test must run package-safe generated-artifact, active-doc, schema, and dependency guardrails."
