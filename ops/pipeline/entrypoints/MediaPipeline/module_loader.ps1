...
#   - RunMonitorContract.ps1 and RunMonitorPersistence.ps1 load before
#     RunMonitorState.ps1 so schema validation and atomic storage remain
#     separate from runtime state orchestration.
...
$engineModulePaths = @{
    ...
    'ToolLogLifecycle.ps1'       = Join-Path $repoRootForModules 'ops\pipeline\engine\process\tool_log_lifecycle.ps1'
    ...
    'RunMonitorContract.ps1'     = Join-Path $repoRootForModules 'ops\pipeline\engine\status\run_monitor_contract.ps1'
    'RunMonitorPersistence.ps1'  = Join-Path $repoRootForModules 'ops\pipeline\engine\status\run_monitor_persistence.ps1'
    'RunMonitorState.ps1'        = Join-Path $repoRootForModules 'ops\pipeline\engine\status\run_monitor_state.ps1'
    ...
    'MediaTrackVerification.ps1' = Join-Path $repoRootForModules 'ops\pipeline\engine\verify\media_track_verification.ps1'
