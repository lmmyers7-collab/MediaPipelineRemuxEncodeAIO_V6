...
function Test-IsBundledPythonPath {
    ...
    foreach ($relative in @(
            ...
            'ops\pipeline\runtime\Python'
        )) {
        $root = [System.IO.Path]::GetFullPath((Join-Path $script:Root $relative)).TrimEnd('\')
...
$script:Root = [System.IO.Path]::GetFullPath((Join-Path $scriptRoot '..\..\..'))
$desktopRoot = Join-Path $script:Root 'apps\desktop'
$pipelineRoot = Join-Path $script:Root 'ops\pipeline'
$script:PipelineRoot = $pipelineRoot

$desktopPackage = Join-Path $script:Root 'src\mediapipeline\desktop'
$pipelineSetup = Join-Path $pipelineRoot 'entrypoints\Setup-MediaPipeline.ps1'
$pipelineRun = Join-Path $script:Root 'ops\scripts\dev\run.bat'
$pipelineScript = Join-Path $pipelineRoot 'entrypoints\MediaPipeline.ps1'
$userConfigRoot = if ($env:LOCALAPPDATA) { Join-Path $env:LOCALAPPDATA 'MediaPipelineRemuxEncodeAIO' } else { '' }
$pipelineConfigCandidates = @(
    if ($userConfigRoot) { Join-Path $userConfigRoot 'MediaPipeline_config.psd1' }
    if ($userConfigRoot) { Join-Path $userConfigRoot 'MediaPipeline_config_chatgpt.psd1' }
    Join-Path $pipelineRoot 'config\MediaPipeline_config.psd1'
    Join-Path $pipelineRoot 'config\MediaPipeline_config_chatgpt.psd1'
) | Where-Object { -not [string]::IsNullOrWhiteSpace($_) }
$pipelineConfig = $pipelineConfigCandidates | Where-Object { Test-Path -LiteralPath $_ } | Select-Object -First 1
if (-not $pipelineConfig) {
    $pipelineConfig = $pipelineConfigCandidates | Select-Object -First 1
}
$auditScript = Join-Path $pipelineRoot 'entrypoints\Audit-MediaLibrary.ps1'
$subtitleHelper = Join-Path $script:Root 'src\mediapipeline\pipeline\ass_to_srt_cli.py'

$bundledPythonGui = Resolve-CommandPath -Candidates @() -RelativePreferred @('apps\desktop\runtime\Python\pythonw.exe','apps\desktop\runtime\Python\python.exe','ops\pipeline\runtime\Python\python.exe') -RejectWindowsApps
$bundledPythonCli = Resolve-CommandPath -Candidates @() -RelativePreferred @('apps\desktop\runtime\Python\python.exe','ops\pipeline\runtime\Python\python.exe') -RejectWindowsApps
$pythonGui = Resolve-CommandPath -Candidates @('pythonw','pythonw.exe','python','python.exe') -RelativePreferred @('apps\desktop\runtime\Python\pythonw.exe','apps\desktop\runtime\Python\python.exe','ops\pipeline\runtime\Python\python.exe') -RejectWindowsApps
$pythonCli = Resolve-CommandPath -Candidates @('python','python.exe') -RelativePreferred @('apps\desktop\runtime\Python\python.exe','ops\pipeline\runtime\Python\python.exe') -RejectWindowsApps
$pwsh = Resolve-CommandPath -Candidates @('pwsh','pwsh.exe') -RelativePreferred @('ops\pipeline\runtime\PowerShell-7.6.0-win-x64\pwsh.exe')
$powershellBootstrap = Resolve-CommandPath -Candidates @('powershell','powershell.exe')
$ffmpeg = Resolve-CommandPath -Candidates @('ffmpeg','ffmpeg.exe') -RelativePreferred @('ops\pipeline\tools\ffmpeg\bin\ffmpeg.exe')
$ffprobe = Resolve-CommandPath -Candidates @('ffprobe','ffprobe.exe') -RelativePreferred @('ops\pipeline\tools\ffmpeg\bin\ffprobe.exe')
$mkvmerge = Resolve-CommandPath -Candidates @('mkvmerge','mkvmerge.exe') -RelativePreferred @('ops\pipeline\tools\MKVToolNix\mkvmerge.exe')
...
if (Test-Path -LiteralPath $pipelineConfig) {
    try {
        ...
        $configImportError = $_.Exception.Message
    }
}
$allowSystemTools = Get-ConfigBoolValue -Config $configData -Key 'AllowSystemTools' -Default $false
$pythonPackageCheckHost = if ($bundledPythonCli) {
    $bundledPythonCli
} elseif ($allowSystemTools) {
    $pythonCli
} else {
    $null
}
...
foreach ($pair in @(
    @{ Label = 'Desktop app folder'; Path = $desktopRoot },
    @{ Label = 'Pipeline ops folder'; Path = $pipelineRoot },
...
if ($bundledPythonGui) {
    Write-Ok "Bundled Python host: $bundledPythonGui"
} elseif ($pythonGui) {
    Write-Warn "Bundled Python host not found. Development fallback detected: $pythonGui"
} else {
    Write-Warn "Bundled Python host not found. Preferred: apps\\desktop\\runtime\\Python\\pythonw.exe or ops\\pipeline\\runtime\\Python\\python.exe. Windows Store aliases are ignored."
}
if ($bundledPythonCli) {
    Write-Ok "Pipeline Python runtime: $bundledPythonCli"
} elseif ($allowSystemTools -and $pythonCli) {
    Write-Warn "Pipeline Python runtime missing; AllowSystemTools=true permits system fallback: $pythonCli"
} elseif ($pythonCli) {
    Write-Fail "Pipeline Python runtime missing and active config has AllowSystemTools=false. Drop python.exe under ops\\pipeline\\runtime\\Python or apps\\desktop\\runtime\\Python."
    Write-Warn "Ignored system Python fallback while AllowSystemTools=false: $pythonCli"
    $failed = $true
} else {
    Write-Fail "Pipeline Python runtime missing. The subtitle helper requires python.exe under ops\\pipeline\\runtime\\Python or apps\\desktop\\runtime\\Python. Windows Store aliases are ignored."
    $failed = $true
}
if ($pwsh) {
...
if ($pythonPackageCheckHost) {
    Write-Host "Checking packages with: $pythonPackageCheckHost" -ForegroundColor DarkGray
    $result = & $pythonPackageCheckHost -c "import importlib.util; names=['pysubs2','psutil','zeroconf']; [print(name + '=' + str(bool(importlib.util.find_spec(name)))) for name in names]" 2>$null
    ...
    Write-Warn 'Skipping Python package check because no Python host is eligible for pipeline execution under the active config.'
}
...
if (Test-Path -LiteralPath $pipelineConfig) {
    ...
    if ($configImportError) {
        Write-Fail "Config could not be imported: $configImportError"
        $failed = $true
    } else {
        Write-Ok "AllowSystemTools: $allowSystemTools"
    }
    if ($pwsh -and (Test-Path -LiteralPath $pipelineSetup)) {
        ...
        $prepend = @(
            (Join-Path $script:Root 'apps\desktop\runtime\Python'),
            (Join-Path $script:Root 'ops\pipeline\runtime\Python'),
            (Join-Path $script:Root 'ops\pipeline\tools\ffmpeg\bin'),
            (Join-Path $script:Root 'ops\pipeline\tools\MKVToolNix'),
            (Join-Path $script:Root 'ops\pipeline\runtime\PowerShell-7.6.0-win-x64')
...
if ($failed) {
    ...
    Write-Host '  2. Run .\ops\scripts\dev\setup.bat' -ForegroundColor Yellow
    Write-Host '  3. Re-run .\ops\scripts\dev\verify-env.bat' -ForegroundColor Yellow
    exit 1
}
