...
- `Verify-MediaPipelineRemuxEncodeAIO-Environment.bat` to check whether this folder is runnable on the current machine.
...
The PowerShell backend keeps stable launcher/script names, but reusable logic lives in `Pipeline\Modules\`:

- subtitle policy and conversion: `Subtitles.ps1` plus `Subtitles.*.ps1`
- audit progress, probe cache, issue policy, scanning, and reports: `Audit.*.ps1`
- routing, encode policy, native tool execution, media probing, audio, naming, folder policies, sidecars, and pending publish: dedicated module files

`Subtitles.ps1` remains a supported facade because the pipeline and regression tests use its public entrypoints. Migration-only comments and obsolete scaffolding have been removed; retained wrappers are treated as compatibility APIs until call sites and docs are deliberately migrated.

## Recommended First-Run Order

1. Run `Verify-MediaPipelineRemuxEncodeAIO-Environment.bat`
2. Fix any missing dependency/runtime issues it reports
3. Run `Setup-MediaPipelineRemuxEncodeAIO.bat`
4. Run `Start-MediaPipelineRemuxEncodeAIO-ApiAndBrowser.bat`
...
- **Release Package** wraps `Build-MediaPipelineRemuxEncodeAIO-Release.ps1`. Use **Plan Only** for a dry-run package plan, then **Build Package** when the destination/options look right. Defaults match the new-user release policy: live config, generated state/logs, optional tool bulk, and dev-only clutter are stripped unless explicitly included.
...
- root setup/run/verify/desktop launchers and the release builder
...
- `Verify-MediaPipelineRemuxEncodeAIO-Environment.ps1` reports OCR readiness and optional network dependency status separately from standalone readiness.
