...
- `mediapipeline.desktop\local_api_main.py`
  - local API entry point
- `mediapipeline.desktop\api\`
...
- `mediapipeline.desktop\views\rename.py`
...
- Folder-level `mediapipeline.folder.json` sidecars can override audio, subtitle, and routing policy for a show/season folder. Use `ops\pipeline\config\src\mediapipeline\contracts\schemas\media_pipeline_folder_policy.example.json` as the operator template, then validate the folder with an approved backend/service workflow before running a batch.
