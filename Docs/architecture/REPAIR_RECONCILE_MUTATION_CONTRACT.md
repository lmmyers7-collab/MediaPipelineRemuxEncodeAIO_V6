...
- `tests/python/desktop/test_repair_reconcile_apply.py` verifies confirmed apply fingerprint gating, backup/write behavior, strict payload validation, command journaling, no frontend path acceptance, and Local API orphan-payload apply evidence that a complete backend proposal writes only the missing manifest while source/output/payload hashes and the pending drain summary remain unchanged.
...
The selected-row Pending Publish manifest repair and orphan-payload reconcile controls are covered by static mutation-boundary tests plus the Pending Publish browser smoke's bounded request-body assertions and no-media-mutation snapshot. Orphan-payload apply now has automated Local API temp-fixture evidence for complete backend proposal handling: the apply route is journaled as `pending_publish.reconcile_orphan_payloads`, writes only the payload-adjacent manifest, leaves source/output/payload hashes and the pending drain summary unchanged, and records no drain command. Daily-driver use still needs operator/runtime deferred publish/drain validation against representative parked payload evidence before trusting the repair surface. The Completed selected-row manifest reconciliation and sidecar metadata repair controls are covered by static mutation-boundary tests plus the Completed/Pending proof browser smoke's no-media-mutation snapshot and bounded request-body assertions.
