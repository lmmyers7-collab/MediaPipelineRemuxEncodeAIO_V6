...
- `tests/python/desktop/test_repair_reconcile_apply.py` verifies confirmed apply fingerprint gating, backup/write behavior, strict payload validation, command journaling, no frontend path acceptance, and Local API pending repair evidence that manifest repair and orphan-payload apply write only backend-authorized manifest state while source/output/payload hashes and the pending drain summary remain unchanged.
