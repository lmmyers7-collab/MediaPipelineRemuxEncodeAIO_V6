...
The selected-row Pending Publish manifest repair and orphan-payload reconcile controls are covered by static mutation-boundary tests plus the Pending Publish browser smoke's bounded request-body assertions and no-media-mutation snapshot. Pending manifest repair and orphan-payload apply now have automated Local API temp-fixture evidence: the apply routes are journaled as repair commands, write only backend-authorized manifest state, leave source/output/payload hashes and the pending drain summary unchanged, and record no drain command. Daily-driver use still needs operator/runtime deferred publish/drain validation against representative parked payload evidence before trusting the repair surface. The Completed selected-row manifest reconciliation and sidecar metadata repair controls are covered by static mutation-boundary tests plus the Completed/Pending proof browser smoke's no-media-mutation snapshot and bounded request-body assertions; Completed manifest and sidecar metadata apply now have automated Local API evidence that the routes are journaled as repair commands, write only their selected manifest/sidecar state with backup evidence, leave source/output/sidecar hashes unchanged, and record no drain command.
