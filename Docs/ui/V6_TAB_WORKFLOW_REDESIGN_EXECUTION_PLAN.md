...
| 13 | Settings | Profiles editable; manual routing unlock clarity; `CPU Max Threads 0` as Unlimited; selectors with descriptions/defaults for flags/codecs/subtitles; subtitle subtab; aggressive parsing controls; remove Open Workers. | Split into backend-metadata-driven display sections, keep advanced controls display-only, use backend Preview/Save Patch for persistence, and keep Library Profiles override groups stable. | Existing settings workspace/preview/save routes; backend metadata, validation, preview, and save remain authoritative. | Completed 2026-05-31 |
...
| 2026-05-31 | Settings metadata/grouping and Library Profiles polish | `settingsView.js`, `settingsLibraries.js`, `settingsMetadata.js`, `page-settings.html`, WebView settings tests, backend metadata/config tests | Phase 8/9 focused settings-library WebView tests, config/library/promotion parity tests, and PowerShell registry/routing/preflight checks | Passed; Settings uses backend metadata for labels/help/options/defaults/advanced display where available, HandBrake-style sections are display-only, Library Profiles preserve inherited/explicit/reset semantics, Preview/Save Patch stays backend-owned, persisted V6 keys and override groups are unchanged, and no media/runtime behavior changed. |
