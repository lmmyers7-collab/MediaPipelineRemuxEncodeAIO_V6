...
For changes to `DesktopApp\mediapipeline_desktop_app\ui_web\static\` (JS, HTML, CSS).
...
### Browser-backed smokes (Chrome or Edge required; skip cleanly if absent)
...
For changes to `DesktopApp\mediapipeline_desktop_app\api\` or facade layer.

Use the bundled interpreter from the repository root. Do not treat system Python failures such as missing `pytest` as V5 test failures unless the task explicitly validates a clean system-Python install.
...
& $py -m unittest DesktopApp.tests.test_api_http_helpers -q
& $py -m unittest DesktopApp.tests.test_backend_bootstrap -q
& $py -m unittest DesktopApp.tests.test_config_keys -q
.\Pipeline\PowerShell-7.6.0-win-x64\pwsh.exe -NoProfile -ExecutionPolicy Bypass `
  -File .\Pipeline\Tests\Unit\Invoke-ConfigKeyRegistryChecks.ps1
.\Pipeline\PowerShell-7.6.0-win-x64\pwsh.exe -NoProfile -ExecutionPolicy Bypass `
  -File .\Pipeline\Tests\Unit\Invoke-FailureCodeRegistryChecks.ps1
...
$py = "DesktopApp\Runtime\Python\python.exe"
# Settings
& $py -m unittest DesktopApp.tests.test_facade_settings_policy -q
& $py -m unittest DesktopApp.tests.test_facade_settings_patch_policy -q
& $py -m unittest DesktopApp.tests.test_service_config_save_runner -q

# Rename
& $py -m unittest DesktopApp.tests.test_service_rename_apply -q
& $py -m unittest DesktopApp.tests.test_service_rename_apply_runner -q
& $py -m unittest DesktopApp.tests.test_service_rename_plan_policy -q

# Pending publish
& $py -m unittest DesktopApp.tests.test_service_pending_publish_manifest -q
& $py -m unittest DesktopApp.tests.test_service_pending_publish_manifest_rows -q
& $py -m unittest DesktopApp.tests.test_pending_publish_service -q
```
...
What these prove: service and facade logic for command routing, guard logic. What they do not prove: actual config write safety on real config files; real rename on production media.
...
```powershell
.\Pipeline\PowerShell-7.6.0-win-x64\pwsh.exe -NoProfile -ExecutionPolicy Bypass `
  -File .\Pipeline\Tests\Invoke-UnitChecks.ps1
```

### Active V6 reliability regression wrapper

```powershell
.\Pipeline\PowerShell-7.6.0-win-x64\pwsh.exe -NoProfile -ExecutionPolicy Bypass `
  -File .\Pipeline\Tests\Invoke-ReliabilityRegressionChecks.ps1
```

This wrapper runs the V6 WebView/backend reliability gate and focused backend safety checks by default. Archived legacy desktop-shell checks require the explicit `-RunLegacyDesktopChecks` switch.
...
```powershell
.\Pipeline\PowerShell-7.6.0-win-x64\pwsh.exe -NoProfile -ExecutionPolicy Bypass `
  -File .\Pipeline\Tests\Invoke-AdversarialForceKillEncodeChecks.ps1
```
...
```powershell
.\Pipeline\PowerShell-7.6.0-win-x64\pwsh.exe -NoProfile -ExecutionPolicy Bypass `
  -File .\Pipeline\Tests\Invoke-ToolIntegrationChecks.ps1
```
...
For changes to `DesktopApp\tauri_shell\` (Rust, Cargo.toml, tauri.conf.json).
...
```powershell
python -m unittest DesktopApp.tests.test_tauri_pg1_close_adversarial_scaffold -v
```
...
```powershell
.\Pipeline\PowerShell-7.6.0-win-x64\pwsh.exe -NoLogo -NoProfile `
  -File .\DesktopApp\tauri_shell\Test-TauriShell-PG1ActiveClose.ps1 `
...
- Skipping browser smokes because Chrome is unavailable — note the skip in your test record and re-run on a machine with a browser before final acceptance.
