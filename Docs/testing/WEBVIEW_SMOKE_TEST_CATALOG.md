...
These are not WebView rendering smokes. The lifecycle smoke validates browser-free close-readiness and backend-shutdown Local API contracts against temporary token-protected backends. The Maintenance dry-run smoke validates browser-free ops/release/metadata/backfill dry-run POST contracts, token enforcement, command history, and no temp source/output media mutation. The sample-validation smoke validates browser-free preview/append/read/tail contracts, current-backend-evidence preview, diagnostics tail allowlist, and temp-only validation-log writes.
