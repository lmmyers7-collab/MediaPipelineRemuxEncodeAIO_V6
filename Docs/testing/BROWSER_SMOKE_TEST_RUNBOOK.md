...
| `Test-WebViewBrowserMaintenanceReportsSmoke.ps1` | `test_webview_browser_maintenance_reports_smoke` | Maintenance health/readiness, release dry-run result rendering, completed-manifest backfill dry-run result rendering, dry-run history, Reports failure/audit triage, selected-row details, read-only Launch/Diagnostics handoff navigation, and no mutation posts |
...
| `Test-WebViewBrowserLayoutManagerSmoke.ps1` | `test_webview_browser_layout_manager_smoke` | Customize mode exposes independent customize bars and draggable handles for representative page tabs, subtabs, and generated subsections, and inactive Settings-family subtabs are visible while customizing |
