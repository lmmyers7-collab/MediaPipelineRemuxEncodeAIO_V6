...
- Folder-level `mediapipeline.folder.json` sidecars can override audio, subtitle, and routing policy for a show/season folder. Use `Pipeline\Schemas\media_pipeline_folder_policy.example.json` as the operator template, then validate the folder with an approved backend/service workflow before running a batch.
