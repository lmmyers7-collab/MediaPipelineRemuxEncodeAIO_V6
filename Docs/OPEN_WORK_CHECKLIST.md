...
- [ ] **Dynamic HDR preservation** — Detection, warning, tool availability, config, sidecar evidence, a pure preserve/remux/review planner, and a pure encode-preservation decision resolver are implemented. FFmpeg/x265 argument wiring now accepts guarded Dolby Vision RPU and HDR10+ JSON artifact paths on CPU/libx265 HDR plans only, with x265 artifact-path resolution guarded to relative, colon-free paths. Backend extraction planning plus an inert execution helper cover fail-closed HEVC/RPU/HDR10+ command topology, command-result evidence, temp artifact checks, and RPU frame-count parsing without changing routing. Actual preservation remains open until `Do-Encode` extraction/injection wiring, policy routing, output verification, and representative real-media validation are complete.
