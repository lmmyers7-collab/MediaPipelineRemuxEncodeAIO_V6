...
- [ ] **Encoder breadth and AV1** — Config/schema choices include AV1/QSV/AMF/libaom families, and Phase 0 encode-argument snapshots plus the Phase 1 HEVC/NVENC/libx265 descriptor parity scaffold are in place. Dormant H.264/NVENC and libx264 descriptors are cataloged with fail-closed HDR guards but are not selected by the active resolver. New encoder families still need descriptor-backed activation, capability probing, fallback behavior, and real-media validation before they are considered daily-driver safe.
