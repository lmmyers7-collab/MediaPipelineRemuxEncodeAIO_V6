...
- [ ] **Dynamic HDR preservation** — Detection, warning, tool availability, config, sidecar evidence, a pure preserve/remux/review planner, and a pure encode-preservation decision resolver are implemented. `Do-Encode` now honors Dynamic HDR policy routing before FFmpeg attempt planning, including fail-closed review behavior and guarded preserve-or-remux fallback. FFmpeg/x265 argument wiring accepts guarded Dolby Vision RPU and HDR10+ JSON artifact paths on CPU/libx265 HDR plans only, with x265 artifact-path resolution guarded to relative, colon-free paths, and the native/FFmpeg runner can execute from an explicit working directory for those relative x265 artifacts. Backend extraction planning plus an inert execution helper cover fail-closed HEVC/RPU/HDR10+ command topology, command-result evidence, temp artifact checks, RPU frame-count parsing, and fail-closed MKV video track-id resolution from `mkvmerge -J` evidence. Actual preservation remains open until `Do-Encode` extraction/injection wiring, output verification, and representative real-media validation are complete.
- [ ] **Encoder breadth and AV1** — Config/schema choices include AV1/QSV/AMF/libaom families, and Phase 0 encode-argument snapshots plus the Phase 1 HEVC/NVENC/libx265 descriptor parity scaffold are in place. Dormant H.264/NVENC, libx264, AV1/NVENC, libaom AV1, QSV, and AMF descriptors are cataloged with fail-closed unsupported-HDR guards where needed, but non-HEVC choices are not selected by the active resolver. Descriptor-owned list/runtime capability-probe scaffolding now exists for the dormant catalog, a synthetic SDR runtime/topology matrix now executes CPU descriptor rows while hardware descriptor execution remains opt-in with explicit skip reporting, a pure descriptor selection resolver now returns primary/fallback descriptors plus trace evidence, and encode attempt plans now expose descriptor-selection evidence that is active only for current HEVC/libx265 parity paths. New encoder families still need descriptor-backed activation, fallback wiring, full hardware/HDR runtime matrix coverage, and real-media validation before they are considered daily-driver safe.
