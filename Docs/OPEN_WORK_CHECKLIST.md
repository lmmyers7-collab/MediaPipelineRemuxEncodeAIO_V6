...
- [ ] **Dynamic HDR preservation** — Detection, warning, tool availability, config, sidecar evidence, and a pure preserve/remux/review planner are implemented. FFmpeg/x265 argument wiring now accepts guarded Dolby Vision RPU and HDR10+ JSON artifact paths on CPU/libx265 HDR plans only. Backend extraction planning plus an inert execution helper now cover fail-closed HEVC/RPU/HDR10+ command topology, command-result evidence, temp artifact checks, and RPU frame-count parsing without changing routing. Actual preservation remains open until encode-route extraction/injection wiring, policy routing, output verification, and representative real-media validation are complete.
