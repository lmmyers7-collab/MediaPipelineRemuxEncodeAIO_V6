...
- [ ] **Dynamic HDR preservation** — Detection, warning, tool availability, config, sidecar evidence, a pure preserve/remux/review planner, and a pure encode-preservation decision resolver are implemented. `Do-Encode` now honors Dynamic HDR policy routing before FFmpeg attempt planning, including fail-closed review behavior and guarded preserve-or-remux fallback. FFmpeg/x265 argument wiring accepts guarded Dolby Vision RPU and HDR10+ JSON artifact paths on CPU/libx265 HDR plans only, with x265 artifact-path resolution guarded to relative, colon-free paths, and the native/FFmpeg runner can now execute from an explicit working directory for those relative x265 artifacts. Backend extraction planning plus an inert execution helper cover fail-closed HEVC/RPU/HDR10+ command topology, command-result evidence, temp artifact checks, and RPU frame-count parsing. Actual preservation remains open until `Do-Encode` extraction/injection wiring, output verification, and representative real-media validation are complete.
