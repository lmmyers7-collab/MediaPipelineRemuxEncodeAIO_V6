...
- [ ] **Dynamic HDR preservation** — Detection, warning, tool availability, config, sidecar evidence, and a pure preserve/remux/review planner are implemented. FFmpeg/x265 argument wiring now accepts guarded Dolby Vision RPU and HDR10+ JSON artifact paths on CPU/libx265 HDR plans only. A backend extraction command planner now builds fail-closed HEVC/RPU/HDR10+ command topology without running tools or changing routing. Actual preservation remains open until runtime extraction/injection execution, policy routing, output verification, and representative real-media validation are complete.
