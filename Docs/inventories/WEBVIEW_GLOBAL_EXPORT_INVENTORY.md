...
Inventories all `window.*` assignments in `apps/desktop/webview/static/assets/*.js`. Source: generated scan of `window.{name} =` assignments across all 70 JS files.

---

## Summary

- **70 JS files** total in `assets/`
- **32 files** export a primary namespace object (`window.mediaPipeline* = { ... }`)
...
| `launchView.js` | mediaPipelineLaunchView | 83 | Generated from current `window.* =` assignments |
...
| `reportsView.js` | mediaPipelineReportsView | 5 | Generated from current `window.* =` assignments |
...
| `settingsView.js` | mediaPipelineSettingsView | 92 | Generated from current `window.* =` assignments |
...
Several modules call other modules' flat exports via `typeof window.X === "function"` guards. This is the designed inter-module communication pattern (no module bundler; load-order safety via guards).
...
| `diagnosticsView.js` | `diagnosticsView.activejobs.js` (`__diagnosticsActiveJobsModule` split-child factory stash, consumed and deleted during load), `diagnosticsView.log.js` (`__diagnosticsLogModule` split-child factory stash, consumed and deleted during load), `diagnosticsView.investigation.js` (`__diagnosticsInvestigationModule` split-child factory stash, consumed and deleted during load), `diagnosticsBridge.js` (`diagnosticsBridgeActions`, `diagnosticsBridgeActionLabel`, `diagnosticsBridgeOrderedActions`), `diagnosticsTailView.js` (tail functions), `queueView.js` (`queueReviewRows`), `completedView.js` (`completedReviewRows`), `pendingPublishView.js` (`pendingReviewRows`), `crossPageContextView.js` (`crossPageConflictRows`), `commandHistory.js` (`commandHistoryIssueEntries`, `commandHistoryOwnerPage`, `commandHistorySuggestedAction`), `diagnosticsStateSummaryView.js` (`diagnosticsStateOperatorStatus`, `diagnosticsStateRecommendedFirstAction`) |
| `diagnosticsStateSummaryView.js` | `diagnosticsBridge.js` (`diagnosticsBridgeActionLabel`, `diagnosticsBridgeOrderedActions`), `diagnosticsTailView.js` (`requestDiagnosticsTail`, `requestDiagnosticsOpen`) |
...
| Criterion | Status |
|---|---|
| All 70 JS files inventoried | Pass |
| Namespace objects identified per file | Pass - 32 namespace-owning files; 38 no-namespace files listed in Summary |
...
## Machine-Generated Flat Export Manifest — 2026-06-05
