...
| `EncoderBackend` | Medium | Dormant global backend preference: `auto`, `nvenc`, `qsv`, `amf`, or `cpu`. Default `auto` preserves literal `VideoCodec` behavior; this key currently feeds capability diagnostics only and does not activate new hardware families. | Not directly surfaced; capability dump evidence only | `test_service_config_option_policy.py`, `Invoke-ConfigKeyRegistryChecks.ps1` |
