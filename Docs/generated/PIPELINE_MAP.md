...
Generated by `ops/scripts/dev/generate_pipeline_map.py` from `app/contracts/stages.py`. Do not hand-edit.
...
- **Domain:** app/orchestration/ (stage boundary), ops/pipeline/engine/storage/ (scratch copy)
...
- **Domain:** app/contracts/source_media*.py (source facts), ops/pipeline/engine/probe/ (ffprobe)
...
- **Domain:** app/processes/ and app/orchestration/ (runner/plans), ops/pipeline/engine/process/ and ops/pipeline/entrypoints/MediaPipeline/ (invocation)
...
- **Domain:** app/contracts/source_media*.py (subtitle facts), ops/pipeline/engine/subtitles/
...
- **Domain:** app/contracts/source_media*.py (audio facts), ops/pipeline/engine/audio/
...
- **Domain:** app/publish/, ops/pipeline/engine/publish/
...
- **Domain:** app/publish/ (drain), ops/pipeline/engine/publish/
...
- **Domain:** app/rename/, ops/pipeline/engine/naming/
- **Note:** Backend owns apply and undo behavior.
- **Note:** Every apply is expected to write an undo record.

## Schemas

`src/mediapipeline/contracts/schemas/stages.v1.schema.json` is generated from `app/contracts/stages.py`.

## Validation

- `app/contracts/stages.py::STAGE_REGISTRY` must match this generated table.
- `python ops/scripts/dev/generate_pipeline_map.py --check` fails when this file drifts.
- `python ops/scripts/dev/generate_stage_schema.py --check` fails when the schema drifts.
