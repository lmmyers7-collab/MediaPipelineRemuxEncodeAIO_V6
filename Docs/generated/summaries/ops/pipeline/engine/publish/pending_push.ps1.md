---
file: ops/pipeline/engine/publish/pending_push.ps1
pipeline_stage: publish
token_priority: high
owner_domain: publish
last_modified: 2026-06-08
last_reviewed: 2026-06-04
sha256: 16a2fc3def26e7b67fe898993c7c47d75c6b80d067ae8dfea55d2dbec28a11f6
---
# `ops/pipeline/engine/publish/pending_push.ps1`

**Purpose:** (no .SYNOPSIS block)

**Functions:** `Add-PendingDrainSummaryCount`, `Complete-PendingDrainSummary`, `Get-PendingDrainSummaryPath`, `Invoke-ParkPendingPush`, `Invoke-ParkPendingPushWithTx3gSidecars`, `Invoke-RetryPendingPushes`, `New-PendingDrainSummaryItem`, `Write-PendingDrainRuntimeProgress`, `Write-PendingDrainSummary`

_Edit the source, not this file. Regenerate with `apps/desktop/runtime/Python/python.exe ops/scripts/dev/run-python-tool.py mediapipeline.tools.dev.refresh_summaries --paths ops/pipeline/engine/publish/pending_push.ps1`._
