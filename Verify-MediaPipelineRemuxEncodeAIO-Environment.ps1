...
$pipelineRun = Join-Path $pipelineRoot 'Run-MediaPipelineRemuxEncodeAIO.bat'
...
foreach ($pair in @(
    ...
    @{ Label = 'Pipeline runner'; Path = $pipelineRun },
...
if (Test-Path -LiteralPath $pipelineConfig) {
    ...
    Write-Host "Next step: run Setup-MediaPipelineRemuxEncodeAIO.bat" -ForegroundColor DarkGray
}
...
if ($failed) {
    ...
    Write-Host '  2. Run Setup-MediaPipelineRemuxEncodeAIO.bat' -ForegroundColor Yellow
    Write-Host '  3. Re-run Verify-MediaPipelineRemuxEncodeAIO-Environment.bat' -ForegroundColor Yellow
    exit 1
}
