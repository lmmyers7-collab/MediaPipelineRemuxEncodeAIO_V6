...
$projectRoot = [System.IO.Path]::GetFullPath((Split-Path -Parent (Split-Path -Parent $scriptRoot)))
$templateCandidates = @(
...
$body = Set-TemplateLineValue -Text $body -Field 'V6 workspace path' -Value $projectRoot
...
Run these from the V6 workspace root as appropriate. These commands are validation gates only; they do not replace the real-media sample run evidence above.
