...
DEFAULT_MANIFEST_PATH = REPO_ROOT / "release" / "RELEASE_MANIFEST.json"
...
def validate_version_label(version: str) -> str:
    ...
    if not cleaned:
        raise SystemExit("--version must not be blank")
    if not VERSION_LABEL_RE.fullmatch(cleaned):
        raise SystemExit(
            "Invalid release version. Use only letters, numbers, dots, dashes, "
            "and underscores; path separators and spaces are not allowed."
        )
    return cleaned
...
def build_manifest(
    ...
    return {
        ...
        "build_id": _build_id(resolved_version, release_date, channel, output_path),
        ...
        "files_touched": _list_values(packets, "files_touched"),
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--version",
        default=None,
        help="Manifest version. Defaults to release/VERSION.",
    )
    parser.add_argument(
        "--channel",
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="Output path. Defaults to release/RELEASE_MANIFEST.json.",
    )
    parser.add_argument(
