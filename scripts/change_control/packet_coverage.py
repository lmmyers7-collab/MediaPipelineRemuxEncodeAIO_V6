...
COVERAGE_FAILURE_HINT = (
    ...
    "record_change_touch.py MP-CHANGE-YYYY-MMDD-### <path>"
)
...
def unreleased_packet_paths(root: Path) -> list[Path]:
    packet_dir = root / "changes" / "unreleased"
...
def _index_packet_paths(root: Path) -> tuple[list[str], str]:
    output, error = _run_git(root, ["ls-files", "--cached", "--", ":(glob)changes/unreleased/*.json"], require_git=True)
    if error:
        return [], error
...
def coverage_for_paths(
    *,
    ...
    covered = sorted(path for path in normalized_changed if path in touched)
    uncovered = sorted(path for path in normalized_changed if path not in touched)
