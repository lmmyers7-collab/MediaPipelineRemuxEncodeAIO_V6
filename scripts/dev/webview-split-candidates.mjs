...
const analyzerPath = join(repoRoot, "scripts", "dev", "analyze-webview-godfiles.mjs");
...
function testsForCandidate(sourceFile, topic) {
  const tests = [
    ...
    "DesktopApp.tests.test_webview_inventory_docs",
  ];
  if (sourceFile.endsWith("/app.js")) {
    tests.push("DesktopApp.tests.test_application_facade_web_static");
    if (topic.includes("layout") || topic.includes("home") || topic.includes("refresh")) {
      tests.push("DesktopApp.tests.test_webview_browser_layout_manager_smoke");
    }
  }
  if (sourceFile.endsWith("/settingsView.js")) {
    tests.push(
      "DesktopApp.tests.test_application_facade_settings_workspace",
      "DesktopApp.tests.test_webview_settings_libraries",
      "DesktopApp.tests.test_webview_settings_patch_smoke"
    );
  }
...
function buildManifest() {
  ...
  return {
    ...
    generated_by: "scripts/dev/webview-split-candidates.mjs",
    source_analyzer: "scripts/dev/analyze-webview-godfiles.mjs",
