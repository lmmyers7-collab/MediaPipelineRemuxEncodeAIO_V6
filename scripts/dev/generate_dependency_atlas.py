"""Generate a readable Python dependency atlas for V6 navigation.
...
DEFAULT_PACKAGE_ROOTS = (
    ...
    (
        ...
        REPO_ROOT / "DesktopApp" / "mediapipeline_desktop_app",
    ),
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate V6_dependency_atlas HTML, images, and CSV exports."
    )
    parser.add_argument(
...
def clean_outputs() -> None:
    ...
    for stale in (
        "V6_python_dependencies.png",
        "V6_python_dependencies.svg",
        "V6_dependency_domains.png",
        "V6_dependency_domains.svg",
    ):
...
def category_for_module(module_name: str) -> str:
    parts = module_name.split(".")
    if parts[0] == "app":
        return "app/" + (parts[1] if len(parts) > 1 else "root")
    if parts[0] == "mediapipeline_desktop_app":
        return "desktop/" + (parts[1] if len(parts) > 1 else "root")
    return parts[0]
...
def module_label(module_name: str, index: ModuleIndex) -> str:
    ...
    if module_name == "app" or module_name.startswith("app."):
        return path.relative_to(REPO_ROOT / "app").as_posix()
    return path.relative_to(REPO_ROOT / "DesktopApp" / "mediapipeline_desktop_app").as_posix()
...
def render_overview_dot(data: AtlasData, min_edge_count: int, min_files: int) -> str:
    ...
    lines = [
        ...
        f'  label="V6 Python dependency atlas: major package/domain edges '
        f'({min_edge_count}+ source modules)"; labelloc=t; fontsize=22; fontname="Segoe UI";',
    ]

    for category in sorted(major_nodes):
        fill = "#dbeafe" if category.startswith("app/") else "#dcfce7"
        label = (
...
def render_detail_dot(data: AtlasData, category: str) -> tuple[str, int, int]:
    ...
    for external_category in sorted(external_categories):
        fill = "#f8fafc" if external_category.startswith("app/") else "#f0fdf4"
        lines.append(
...
def write_detail_images(data: AtlasData, dot_path: Path) -> list[DetailRecord]:
    ...
    for category in sorted(
        detail_categories,
        key=lambda item: (not item.startswith("app/"), -len(data.category_files[item]), item),
    ):
        source_dot_text, node_count, edge_count = render_detail_dot(data, category)
        file_slug = slug(category)
        source_dot = ASSET_ROOT / f"V6_dependency_detail_{file_slug}.dot"
        png_path = ASSET_ROOT / f"V6_dependency_detail_{file_slug}.png"
        svg_path = ASSET_ROOT / f"V6_dependency_detail_{file_slug}.svg"
...
def render_html(
    ...
    lines = [
        "<!doctype html>",
        '<html lang="en"><head><meta charset="utf-8"><title>V6 Dependency Atlas</title>',
        "<style>"
        ...
        "</style>",
        "</head><body>",
        "<h1>V6 Dependency Atlas</h1>",
        '<p class="meta">Generated from Python AST imports in <code>app/</code> and '
        '<code>DesktopApp/mediapipeline_desktop_app/</code>. Counts are source-module '
        ...
        '<span><i class="swatch" style="background:#dcfce7"></i>desktop domain</span>'
        ...
        '<img src="V6_dependency_atlas.svg" alt="V6 dependency overview">',
