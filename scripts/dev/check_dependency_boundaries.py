"""Check Python app dependency boundary rules."""
...
def module_name_for_path(path: Path, app_root: Path) -> str:
...
def iter_app_modules(root: Path = REPO_ROOT) -> dict[str, Path]:
    root = root.resolve()
    app_root = root / "app"
    modules: dict[str, Path] = {}
    for path in sorted(app_root.rglob("*.py")):
        modules[module_name_for_path(path, app_root)] = path
    return modules
...
def collect_app_imports(root: Path = REPO_ROOT) -> tuple[list[ImportEdge], list[ParseError]]:
    root = root.resolve()
    modules = iter_app_modules(root)
...
def analyze(root: Path = REPO_ROOT) -> DependencyReport:
    root = root.resolve()
    app_imports, parse_errors = collect_app_imports(root)
    ...
    forbidden_config_imports = [
        edge
        for edge in app_imports
        if package_name(edge.source_module) == "app.config"
    ...
    return DependencyReport(
        ...
        direct_app_shared_imports=filter_target(app_imports, "app.shared"),
        direct_app_shared_utils_imports=filter_target(app_imports, "app.shared.utils"),
        direct_app_shared_constants_imports=filter_target(app_imports, "app.shared.constants"),
        direct_app_shared_protocols_imports=filter_target(app_imports, "app.shared.protocols"),
        forbidden_config_imports=forbidden_config_imports,
        forbidden_api_ui_imports=filter_package_edge(app_imports, "app.api", "app.ui"),
        forbidden_observability_status_imports=filter_package_edge(
            app_imports, "app.observability", "app.status"
        ),
        forbidden_telemetry_observability_imports=filter_package_edge(
            app_imports, "app.telemetry", "app.observability"
        ),
        forbidden_telemetry_status_imports=filter_package_edge(app_imports, "app.telemetry", "app.status"),
...
def config_rule_id(edge: ImportEdge) -> str:
    target_package = package_name(edge.target_module)
    if target_package == "app.orchestration":
        return "NO_CONFIG_TO_ORCHESTRATION"
    if target_package == "app.decide":
...
def hard_rule_findings(report: DependencyReport) -> list[RuleFinding]:
    ...
    findings.extend(
        RuleFinding(
            ...
            edge.source_module,
            edge.target_module,
            f"Direct app.shared import at {edge.source_path}:{edge.line}.",
        )
    ...
    findings.extend(
        RuleFinding(
            ...
            edge.source_module,
            edge.target_module,
            f"app.config must stay low-level; import found at {edge.source_path}:{edge.line}.",
        )
    ...
    findings.extend(
        RuleFinding(
            ...
            edge.source_module,
            edge.target_module,
            f"app.api must not import app.ui; import found at {edge.source_path}:{edge.line}.",
        )
    ...
    findings.extend(
        RuleFinding(
            ...
            edge.source_module,
            edge.target_module,
            f"app.observability must not import app.status; import found at {edge.source_path}:{edge.line}.",
        )
    ...
    findings.extend(
        RuleFinding(
            ...
            edge.source_module,
            edge.target_module,
            f"app.telemetry must not import app.observability; import found at {edge.source_path}:{edge.line}.",
        )
    ...
    findings.extend(
        RuleFinding(
            ...
            edge.source_module,
            edge.target_module,
            f"app.telemetry must not import app.status; import found at {edge.source_path}:{edge.line}.",
        )
...
def warning_rule_findings(report: DependencyReport) -> list[RuleFinding]:
    ...
    findings.extend(
        ...
        for edge in report.direct_app_shared_constants_imports
        if package_name(edge.source_module) != "app.shared"
    )
    findings.extend(
        RuleFinding(
            "NO_BARREL_INIT_REEXPORTS",
...
def render_report(
    ...
    lines = [
        ...
        "",
        f"Internal app imports: {len(report.app_imports)}",
    ...
    sections = [
        ("Direct app.shared imports", report.direct_app_shared_imports),
        ("Direct app.shared.utils imports", report.direct_app_shared_utils_imports),
        ("Direct app.shared.constants imports", report.direct_app_shared_constants_imports),
        ("Direct app.shared.protocols imports", report.direct_app_shared_protocols_imports),
        ("Forbidden app.config higher-level imports", report.forbidden_config_imports),
        ("Forbidden app.api to app.ui imports", report.forbidden_api_ui_imports),
        ("Forbidden app.observability to app.status imports", report.forbidden_observability_status_imports),
        (
            "Forbidden app.telemetry to app.observability imports",
            report.forbidden_telemetry_observability_imports,
        ),
        ("Forbidden app.telemetry to app.status imports", report.forbidden_telemetry_status_imports),
    ]
    ...
    lines.append("")
    lines.extend(render_edge_section("Internal App Import Edges", report.app_imports, root, max_internal_imports))
