"""Generate the stage JSON Schema from `app.contracts.stages`.

Use `--check` in CI/pre-commit to fail when `schemas/stages.v1.schema.json`
is stale.
"""
...
def render_schema() -> str:
    from app.contracts.stages import STAGE_SCHEMA_VERSION, StageContractSchema

    schema = StageContractSchema.model_json_schema()
    schema["$schema"] = "https://json-schema.org/draft/2020-12/schema"
    schema["$id"] = "https://local.mediapipeline/schemas/stages.v1.schema.json"
...
def check_current(path: Path, expected: str) -> int:
    ...
    print("  python scripts/dev/generate_stage_schema.py")
    print("\n".join(diff))
    return 1
