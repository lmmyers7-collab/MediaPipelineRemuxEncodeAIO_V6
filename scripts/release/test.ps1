...
function Import-MediaPipelineReleasePolicy {
    $policyPath = Join-Path $script:BundleRoot 'scripts\release\release_policy.ps1'
...
foreach ($entry in @(
    ...
    @{ Label = 'Canonical setup launcher'; Path = (Join-Path $script:BundleRoot 'scripts\dev\setup.bat'); Type = 'Leaf' },
    @{ Label = 'Canonical run launcher'; Path = (Join-Path $script:BundleRoot 'scripts\dev\run.bat'); Type = 'Leaf' },
    @{ Label = 'Canonical API and browser launcher'; Path = (Join-Path $script:BundleRoot 'scripts\dev\start-api-and-browser.bat'); Type = 'Leaf' },
    @{ Label = 'Canonical local API launcher'; Path = (Join-Path $script:BundleRoot 'scripts\dev\start-local-api.bat'); Type = 'Leaf' },
    @{ Label = 'Canonical Tauri preview launcher'; Path = (Join-Path $script:BundleRoot 'scripts\dev\start-tauri-preview.bat'); Type = 'Leaf' },
    @{ Label = 'Canonical environment verifier'; Path = $verifier; Type = 'Leaf' },
    @{ Label = 'Canonical release build script'; Path = (Join-Path $script:BundleRoot 'scripts\release\build.ps1'); Type = 'Leaf' },
    @{ Label = 'Canonical release self-test script'; Path = (Join-Path $script:BundleRoot 'scripts\release\test.ps1'); Type = 'Leaf' },
    @{ Label = 'Canonical real-media validation worksheet helper'; Path = (Join-Path $script:BundleRoot 'scripts\operator\New-RealMediaValidationWorksheet.ps1'); Type = 'Leaf' },
    ...
    @{ Label = 'Application facade audit mixin'; Path = (Join-Path $script:BundleRoot 'app\audit\facade.py'); Type = 'Leaf' },
    @{ Label = 'Application facade completed mixin'; Path = (Join-Path $script:BundleRoot 'app\completed\facade.py'); Type = 'Leaf' },
    @{ Label = 'Application facade completed open mixin'; Path = (Join-Path $script:BundleRoot 'app\completed\open_facade.py'); Type = 'Leaf' },
    @{ Label = 'Application facade diagnostics mixin'; Path = (Join-Path $script:BundleRoot 'app\diagnostics\facade.py'); Type = 'Leaf' },
    @{ Label = 'Application facade failures mixin'; Path = (Join-Path $script:BundleRoot 'app\failures\facade.py'); Type = 'Leaf' },
    @{ Label = 'Application facade maintenance mixin'; Path = (Join-Path $script:BundleRoot 'app\maintenance\facade.py'); Type = 'Leaf' },
    @{ Label = 'Application facade maintenance backfill mixin'; Path = (Join-Path $script:BundleRoot 'app\maintenance\backfill_facade.py'); Type = 'Leaf' },
    @{ Label = 'Application facade maintenance commands mixin'; Path = (Join-Path $script:BundleRoot 'app\maintenance\commands_facade.py'); Type = 'Leaf' },
    @{ Label = 'Application facade maintenance release mixin'; Path = (Join-Path $script:BundleRoot 'app\maintenance\release_facade.py'); Type = 'Leaf' },
    @{ Label = 'Application facade pending-publish mixin'; Path = (Join-Path $script:BundleRoot 'app\publish\pending_facade.py'); Type = 'Leaf' },
    @{ Label = 'Application facade process mixin'; Path = (Join-Path $script:BundleRoot 'app\processes\preflight_facade.py'); Type = 'Leaf' },
    @{ Label = 'Application facade process audit launch mixin'; Path = (Join-Path $script:BundleRoot 'app\processes\audit_facade.py'); Type = 'Leaf' },
    @{ Label = 'Application facade process control mixin'; Path = (Join-Path $script:BundleRoot 'app\processes\control_facade.py'); Type = 'Leaf' },
    @{ Label = 'Application facade process guard mixin'; Path = (Join-Path $script:BundleRoot 'app\processes\guard_facade.py'); Type = 'Leaf' },
    @{ Label = 'Application facade process pipeline launch mixin'; Path = (Join-Path $script:BundleRoot 'app\processes\pipeline_facade.py'); Type = 'Leaf' },
    @{ Label = 'Application facade process rerun launch mixin'; Path = (Join-Path $script:BundleRoot 'app\processes\rerun_facade.py'); Type = 'Leaf' },
    @{ Label = 'Application facade process schedule mixin'; Path = (Join-Path $script:BundleRoot 'app\processes\schedule_facade.py'); Type = 'Leaf' },
    @{ Label = 'Application facade queue mixin'; Path = (Join-Path $script:BundleRoot 'app\queue\facade.py'); Type = 'Leaf' },
    @{ Label = 'Application facade rename mixin'; Path = (Join-Path $script:BundleRoot 'app\rename\facade.py'); Type = 'Leaf' },
    @{ Label = 'Application facade sample validation mixin'; Path = (Join-Path $script:BundleRoot 'app\sample_validation\facade.py'); Type = 'Leaf' },
    @{ Label = 'Application sample validation policy'; Path = (Join-Path $script:BundleRoot 'app\sample_validation\policy.py'); Type = 'Leaf' },
    ...
    @{ Label = 'Local API file command handlers'; Path = (Join-Path $script:BundleRoot 'app\api\commands_files.py'); Type = 'Leaf' },
    @{ Label = 'Local API maintenance command handlers'; Path = (Join-Path $script:BundleRoot 'app\api\commands_maintenance.py'); Type = 'Leaf' },
    @{ Label = 'Local API process command handlers'; Path = (Join-Path $script:BundleRoot 'app\api\commands_process.py'); Type = 'Leaf' },
    @{ Label = 'Local API rename command handlers'; Path = (Join-Path $script:BundleRoot 'app\api\commands_rename.py'); Type = 'Leaf' },
    @{ Label = 'Local API sample validation command handlers'; Path = (Join-Path $script:BundleRoot 'app\api\commands_sample_validation.py'); Type = 'Leaf' },
    @{ Label = 'Local API settings command handlers'; Path = (Join-Path $script:BundleRoot 'app\api\commands_settings.py'); Type = 'Leaf' },
...
$parseFiles = @(
    (Join-Path $script:BundleRoot 'scripts\verify-env.ps1'),
    (Join-Path $script:BundleRoot 'scripts\release\build.ps1'),
    (Join-Path $script:BundleRoot 'scripts\release\test.ps1'),
    (Join-Path $script:BundleRoot 'scripts\operator\New-RealMediaValidationWorksheet.ps1'),
...
$pythonSyntaxFiles = @(
    ...
    (Join-Path $script:BundleRoot 'app\audit\facade.py'),
    (Join-Path $script:BundleRoot 'app\completed\facade.py'),
    (Join-Path $script:BundleRoot 'app\completed\open_facade.py'),
    (Join-Path $script:BundleRoot 'app\diagnostics\facade.py'),
    (Join-Path $script:BundleRoot 'app\failures\facade.py'),
    (Join-Path $script:BundleRoot 'app\maintenance\facade.py'),
    (Join-Path $script:BundleRoot 'app\maintenance\backfill_facade.py'),
    (Join-Path $script:BundleRoot 'app\maintenance\commands_facade.py'),
    (Join-Path $script:BundleRoot 'app\maintenance\release_facade.py'),
    (Join-Path $script:BundleRoot 'app\publish\pending_facade.py'),
    (Join-Path $script:BundleRoot 'app\processes\preflight_facade.py'),
    (Join-Path $script:BundleRoot 'app\processes\audit_facade.py'),
    (Join-Path $script:BundleRoot 'app\processes\control_facade.py'),
    (Join-Path $script:BundleRoot 'app\processes\guard_facade.py'),
    (Join-Path $script:BundleRoot 'app\processes\pipeline_facade.py'),
    (Join-Path $script:BundleRoot 'app\processes\rerun_facade.py'),
    (Join-Path $script:BundleRoot 'app\processes\schedule_facade.py'),
    (Join-Path $script:BundleRoot 'app\queue\facade.py'),
    (Join-Path $script:BundleRoot 'app\rename\facade.py'),
    ...
    (Join-Path $script:BundleRoot 'app\api\commands_files.py'),
    (Join-Path $script:BundleRoot 'app\api\commands_maintenance.py'),
    (Join-Path $script:BundleRoot 'app\api\commands_process.py'),
    (Join-Path $script:BundleRoot 'app\api\commands_rename.py'),
    (Join-Path $script:BundleRoot 'app\api\commands_settings.py'),
