...
$releaseBuilder = Join-Path $projectRoot 'scripts\release\build.ps1'
$environmentVerifier = Join-Path $projectRoot 'scripts\verify-env.ps1'
$releaseVerifier = Join-Path $projectRoot 'scripts\release\test.ps1'
...
$subtitleFacadeText = Get-Content -LiteralPath (Join-Path $projectRoot 'engine\subtitles\subtitles.ps1') -Raw
$subtitleCommonText = Get-Content -LiteralPath (Join-Path $projectRoot 'engine\subtitles\common.ps1') -Raw
$subtitleSrtText = Get-Content -LiteralPath (Join-Path $projectRoot 'engine\subtitles\srt.ps1') -Raw
$subtitleAssModuleText = Get-Content -LiteralPath (Join-Path $projectRoot 'engine\subtitles\ass.ps1') -Raw
$subtitleTx3gText = Get-Content -LiteralPath (Join-Path $projectRoot 'engine\subtitles\tx3g.ps1') -Raw
$subtitleBdpgsText = Get-Content -LiteralPath (Join-Path $projectRoot 'engine\subtitles\bdpgs.ps1') -Raw
$subtitleBuildersText = Get-Content -LiteralPath (Join-Path $projectRoot 'engine\subtitles\builders.ps1') -Raw
$pendingManifestStoreText = Get-Content -LiteralPath (Join-Path $projectRoot 'engine\publish\pending_manifest_store.ps1') -Raw
$pendingTransactionsText = Get-Content -LiteralPath (Join-Path $projectRoot 'engine\publish\pending_transactions.ps1') -Raw
$pendingPublishIndexText = Get-Content -LiteralPath (Join-Path $projectRoot 'engine\publish\pending_publish_index.ps1') -Raw
$pendingPushText = Get-Content -LiteralPath (Join-Path $projectRoot 'engine\publish\pending_push.ps1') -Raw
$publishCompletionText = Get-Content -LiteralPath (Join-Path $projectRoot 'engine\publish\publish_completion.ps1') -Raw
$sidecarText = Get-Content -LiteralPath (Join-Path $projectRoot 'engine\publish\sidecar.ps1') -Raw
$pathHelpersText = Get-Content -LiteralPath (Join-Path $projectRoot 'engine\shared\path_helpers.ps1') -Raw
...
$failureCodesText = Get-Content -LiteralPath (Join-Path $projectRoot 'engine\shared\failure_codes.ps1') -Raw
$failureStateText = Get-Content -LiteralPath (Join-Path $projectRoot 'engine\failures\failure_state.ps1') -Raw
$routingText = Get-Content -LiteralPath (Join-Path $projectRoot 'engine\decide\routing.ps1') -Raw
$encodePolicyText = Get-Content -LiteralPath (Join-Path $projectRoot 'engine\decide\encode_policy.ps1') -Raw
$mediaConstantsText = Get-Content -LiteralPath (Join-Path $projectRoot 'engine\shared\media_constants.ps1') -Raw
$pipelineEngineText = Get-Content -LiteralPath (Join-Path $projectRoot 'engine\queue\pipeline_engine.ps1') -Raw
$pipelineProcessingText = Get-Content -LiteralPath (Join-Path $projectRoot 'engine\process\pipeline_processing.ps1') -Raw
$outputPathPlanningText = Get-Content -LiteralPath (Join-Path $projectRoot 'engine\paths\output_path_planning.ps1') -Raw
$queuePlanText = Get-Content -LiteralPath (Join-Path $projectRoot 'engine\queue\queue_plan.ps1') -Raw
$configSchemaModuleText = Get-Content -LiteralPath (Join-Path $projectRoot 'engine\config\config_schema.ps1') -Raw
$stateStoreText = Get-Content -LiteralPath (Join-Path $projectRoot 'engine\storage\state_store.ps1') -Raw
$progressStateText = Get-Content -LiteralPath (Join-Path $projectRoot 'engine\status\progress_state.ps1') -Raw
$libraryIndexText = Get-Content -LiteralPath (Join-Path $projectRoot 'engine\library\library_index.ps1') -Raw
...
Assert-True ($facadeMaintenanceReleaseText -match 'app\.maintenance\.command_policy' -and $facadeMaintenanceReleaseText -match 'release_dry_run_builder_kwargs' -and $facadeMaintenanceReleaseText -match 'release_dry_run_result' -and $facadeMaintenanceBackfillText -match 'completed_backfill_dry_run_result' -and $facadeMaintenanceCommandsText -match 'release_stdout_value' -and $facadeMaintenanceCommandPolicyText -match 'def release_dry_run_builder_kwargs' -and $facadeMaintenanceCommandPolicyText -match 'def release_dry_run_result' -and $facadeMaintenanceCommandPolicyText -match 'def completed_backfill_dry_run_result') "Maintenance release/backfill dry-run request, stdout parsing, and command-result policy must live in focused helpers while service/subprocess execution stays backend-owned."
...
Assert-True ($serviceConfigText -match 'app\.config\.validation' -and $serviceConfigText -match 'def validate_config_values' -and $serviceConfigText -match 'def _config_path_overlap_warning' -and $serviceConfigValidationText -match 'def validate_config_values' -and $serviceConfigValidationText -match 'def config_path_overlap_warning' -and $serviceConfigOptionPolicyText -match 'AudioTranscodeBitrate must look like 640k') "Desktop config value validation and root-overlap warnings must live in focused helpers behind ConfigProfileService compatibility wrappers."
...
Assert-True (
    ...
    $deployabilityChecklistText -match 'scripts\\release\\build\.ps1'
...
Assert-True (
    ...
    $releaseBuilderText -match 'scripts\\release\\test\.ps1'
