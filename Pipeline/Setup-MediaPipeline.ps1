...
$script:ConfigPath = if ($ConfigPath) {
    ...
    # Write the V7 convention name. If the operator has an existing
    # legacy `_chatgpt` config alongside, preserve it by reusing that
    # path so we don't end up with two configs.
    $newName = Join-Path $script:ScriptDir 'MediaPipeline_config.psd1'
    $legacyName = Join-Path $script:ScriptDir 'MediaPipeline_config_chatgpt.psd1'
...
$script:SubtitlePath = Join-Path $script:ScriptDir 'ass_to_srt.py'
...
$script:SetupSliceDir = Join-Path $script:ScriptDir 'Setup-MediaPipeline'
$script:SetupSlices = @(
    'UserInteraction.ps1',
    'Dependencies.ps1',
    'PathValidation.ps1',
