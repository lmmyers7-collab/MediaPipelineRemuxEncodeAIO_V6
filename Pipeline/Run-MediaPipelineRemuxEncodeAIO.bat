@echo off
setlocal EnableExtensions EnableDelayedExpansion

cd /d "%~dp0"

if /i "%~1"=="/?" goto :show_help
if /i "%~1"=="-h" goto :show_help
if /i "%~1"=="--help" goto :show_help
if /i "%~1"=="setup" (
    call "%~dp0Setup-MediaPipelineRemuxEncodeAIO.bat"
    exit /b !ERRORLEVEL!
)
if /i "%~1"=="validate" (
    call "%~dp0Setup-MediaPipelineRemuxEncodeAIO.bat" validate
    exit /b !ERRORLEVEL!
)

if not exist "%~dp0MediaPipeline_chatgpt.ps1" (
    echo ERROR: MediaPipeline_chatgpt.ps1 was not found in:
    echo   %~dp0
    pause
    exit /b 1
)

if not exist "%~dp0MediaPipeline_config_chatgpt.psd1" if not exist "%~dp0MediaPipeline_config.psd1" (
    echo ERROR: No config file was found next to the pipeline.
    echo Checked:
    echo   %~dp0MediaPipeline_config_chatgpt.psd1
    echo   %~dp0MediaPipeline_config.psd1
    echo.
    echo Run Setup-MediaPipelineRemuxEncodeAIO.bat first.
    pause
    exit /b 1
)

if exist "%~dp0Tools\ffmpeg\bin" set "PATH=%~dp0Tools\ffmpeg\bin;%PATH%"
if exist "%~dp0Tools\MKVToolNix" set "PATH=%~dp0Tools\MKVToolNix;%PATH%"
if exist "%~dp0Runtime\Python" set "PATH=%~dp0Runtime\Python;%PATH%"
if exist "%~dp0..\DesktopApp\Runtime\Python" set "PATH=%~dp0..\DesktopApp\Runtime\Python;%PATH%"

set "PWSH_PATH="
if exist "%~dp0pwsh.exe" set "PWSH_PATH=%~dp0pwsh.exe"

if not defined PWSH_PATH (
    for /d %%D in ("%~dp0PowerShell-*") do (
        if exist "%%~fD\pwsh.exe" (
            set "PWSH_PATH=%%~fD\pwsh.exe"
            goto :pwsh_resolved
        )
    )
)

if not defined PWSH_PATH (
    for /f "usebackq delims=" %%I in (`powershell.exe -NoProfile -Command "$cmd=Get-Command pwsh -ErrorAction SilentlyContinue; if($cmd){$cmd.Source}"`) do (
        set "PWSH_PATH=%%I"
    )
)

:pwsh_resolved
if defined PWSH_PATH (
    echo Launching pipeline with PowerShell 7: !PWSH_PATH!
    "!PWSH_PATH!" -NoProfile -ExecutionPolicy Bypass -File "%~dp0MediaPipeline_chatgpt.ps1"
) else (
    echo PowerShell 7 was not resolved by the launcher.
    echo Trying Windows PowerShell so the script can self-relaunch if a bundled runtime exists.
    powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%~dp0MediaPipeline_chatgpt.ps1"
)

set "EXIT_CODE=!ERRORLEVEL!"
echo.
echo MediaPipeline exited with code !EXIT_CODE!.
pause
exit /b !EXIT_CODE!

:show_help
echo MediaPipelineRemuxEncodeAIO run launcher 1.0
echo.
echo Usage:
echo   Run-MediaPipelineRemuxEncodeAIO.bat
echo   Run-MediaPipelineRemuxEncodeAIO.bat validate
echo   Run-MediaPipelineRemuxEncodeAIO.bat setup
echo.
echo Commands:
echo   no args   Run the pipeline
echo   validate  Run setup validation only
echo   setup     Open the setup wizard
echo.
pause
exit /b 0
