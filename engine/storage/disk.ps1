...
# scope at call time:
#
#   Test-IsUncPath, Get-UncShareRoot   — engine\shared\path_helpers.ps1
#   Invoke-NativeCommand, Start-StopAwareSleep — engine\shared\native.ps1
#   Write-Log                          — engine\observability\logging.ps1
        ...
        $copyProgressPoll = {
            ...
            try {
                if (Test-Path -LiteralPath $landedName -PathType Leaf -ErrorAction SilentlyContinue) {
                    $bytes = [long](Get-Item -LiteralPath $landedName -ErrorAction Stop).Length
                }
            } catch {
                $bytes = 0L
            }
