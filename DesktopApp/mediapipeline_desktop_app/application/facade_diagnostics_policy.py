...
from ..service_status_active_jobs import active_job_detail_rows
from .dto import split_summary_lines
...
def diagnostics_summary_lines(method_name: str, method: Callable[[Snapshot], object] | None, snapshot: Snapshot) -> list[str]:
    if not callable(method):
        return []
    try:
        return split_summary_lines(method(snapshot))  # type: ignore[misc]
    except Exception as exc:
