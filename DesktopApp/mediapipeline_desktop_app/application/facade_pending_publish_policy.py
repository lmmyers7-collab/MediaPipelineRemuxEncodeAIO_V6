...
from .dto import CommandResult, PendingPublishPreviewDto, json_safe
...
def pending_publish_preview_fields(raw: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "drain_summary": json_safe(raw.get("drain_summary") or {}),
...
def pending_publish_row_diagnostics(row: Mapping[str, Any]) -> dict[str, Any]:
    ...
    fields = {
        ...
        "diagnostic_severity": pending_publish_row_diagnostic_severity(status),
        "drain_recommendation": pending_publish_row_drain_recommendation(status),
...
def pending_publish_service_unavailable_result() -> PendingPublishPreviewDto:
    ...
    return PendingPublishPreviewDto(
        inventory_progress=progress,
        progress_bars=progress["progress_bars"],
...
def pending_publish_preview_result(raw: Mapping[str, Any]) -> PendingPublishPreviewDto:
    return PendingPublishPreviewDto(**pending_publish_preview_fields(raw))
...
def pending_publish_recovery_plan_row(row: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "proof_summary": json_safe(row.get("proof_summary") or []),
        "evidence_fields": json_safe(row.get("evidence_fields") or []),
        "recommended_open_targets": json_safe(row.get("recommended_open_targets") or []),
        "recommended_diagnostics_targets": json_safe(row.get("recommended_diagnostics_targets") or ["pending_publish", "run_logs", "last_stderr_log"]),
