...
from .dto import CompletedPreviewDto
from .facade_artifact_freshness import file_freshness_fields
...
def completed_row_trust_fields(row: dict[str, Any]) -> dict[str, Any]:
    ...
    missing_output = row.get("output_exists") is False or str(row.get("output_health") or "").strip().casefold() in {"missing", "missing_output", "unavailable"}
