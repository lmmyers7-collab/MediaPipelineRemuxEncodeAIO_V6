...
from .dto import CommandResult, RenamePreviewDto, json_safe
from .facade_rename_policy import (
...
class RenameFacadeMixin:
    ...
    def get_rename_preview(self, request: dict[str, Any]) -> RenamePreviewDto:
        rows = [json_safe(dict(row)) for row in self._build_rename_plan_from_request(request)]
