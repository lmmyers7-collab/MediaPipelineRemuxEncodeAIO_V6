...
from .dto import JsonMap, json_safe
from .facade_process_audit_policy import AUDIT_LIBRARY_ROOT_ERROR, resolve_audit_library_root
from .facade_process_pipeline_policy import (
...
from .facade_process_schedule_policy import continuous_schedule_stop_watcher_preflight_check
...
class ProcessFacadeMixin:
    ...
    def _pipeline_launch_preflight_checks(
        ...
        extra_args_error = pipeline_extra_args_error(extra_args, bool(request.get("allow_extra_args", False)))
        normalized = {
            "target": "pipeline",
            "mode": mode,
            ...
            "allow_extra_args": bool(request.get("allow_extra_args", False)),
        }
        checks: list[dict[str, Any]] = [
            _preflight_check(
                "mode",
            ...
            _preflight_check(
                ...
                f"extra_args_present={bool(extra_args)}; allow_extra_args={bool(request.get('allow_extra_args', False))}",
                "Keep local API launches on structured fields unless extra args are explicitly enabled by backend policy.",
