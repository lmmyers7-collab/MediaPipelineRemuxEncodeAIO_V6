...
from .service_constants import ACTIVE_JOB_SCHEMA_VERSION
from .service_runner_protocols import InfoWarningLogger, WarningLogger
    ...
    *,
    ...
    for record_path in records[:max_items]:
        ...
        if matches_record is True:
            messages.append(f"ActiveJobs record {record_path.name} reports {label} as {record.status} and PID {record.pid} is still running.")
        else:
            messages.append(f"ActiveJobs record {record_path.name} reports {label} as {record.status}; PID {record.pid} identity could not be verified.")
    return messages
...
def update_active_job_record(
    ...
    *,
    ...
    try:
        ...
        if logger is not None:
            logger.warning("ActiveJobs record %s could not be read during update; repairing with default fields: %s", record_path.name, exc)
        payload = {}
    now = datetime.now().astimezone().isoformat(timespec="seconds")
