...
function backendLifecycleState(closeReadiness = lastCloseReadiness) {
  return window.mediaPipelineAppLifecycle?.backendLifecycleState?.(closeReadiness) || { label: "Waiting", state: "unknown", canShutdown: false, reason: "Close-readiness has not loaded yet." };
}
...
function normalizeTauriBackendLifecycleEvent(payload) {
  return window.mediaPipelineAppLifecycle?.normalizeTauriBackendLifecycleEvent?.(payload) || { schema_version: "unknown", status: "unknown", detail: "No lifecycle detail was reported.", consecutive_failures: 0, emitted_at_unix_seconds: 0 };
}
...
function renderHomeReadiness(context = {}) {
  return window.mediaPipelineAppHome?.renderHomeReadiness?.(context);
}

function dailyDriverRows(context = {}) {
  return window.mediaPipelineAppHome?.dailyDriverRows?.(context) || [];
}

function dailyDriverStatusClass(status) {
  return window.mediaPipelineAppHome?.dailyDriverStatusClass?.(status) || "match";
}

function dailyDriverSummaryLines(rows = []) {
  return window.mediaPipelineAppHome?.dailyDriverSummaryLines?.(rows) || [];
}

function dependencyStatusLabel(status) {
  return window.mediaPipelineAppHome?.dependencyStatusLabel?.(status) || "review";
}

function homeAtAGlancePercent(value) {
  return window.mediaPipelineAppHome?.homeAtAGlancePercent?.(value) || "";
}
void [homeAtAGlancePercent];

function homeAtAGlanceStatusState(label) {
  return window.mediaPipelineAppHome?.homeAtAGlanceStatusState?.(label) || "loading";
}

function homeAtAGlanceQueueTitle(item = {}) {
  return window.mediaPipelineAppHome?.homeAtAGlanceQueueTitle?.(item) || "";
}

function homeAtAGlanceQueueMeta(item = {}) {
  return window.mediaPipelineAppHome?.homeAtAGlanceQueueMeta?.(item) || "";
}

function homeAtAGlanceModel(context = {}) {
  return window.mediaPipelineAppHome?.homeAtAGlanceModel?.(context) || {};
}

function homeAtAGlanceProgressBar(model = {}) {
  return window.mediaPipelineAppHome?.homeAtAGlanceProgressBar?.(model) || [];
}

function renderHomePendingCount(pending) {
  return window.mediaPipelineAppHome?.renderHomePendingCount?.(pending);
}
...
function externalDependencyRows(context = {}) {
  return window.mediaPipelineAppHome?.externalDependencyRows?.(context) || [];
}

function externalDependencyOverallStatus(context = {}) {
  return window.mediaPipelineAppHome?.externalDependencyOverallStatus?.(context) || "unknown";
}

function externalDependencySummaryLines(context = {}) {
  return window.mediaPipelineAppHome?.externalDependencySummaryLines?.(context) || [];
}

function externalDependencyEvidenceText(context = {}) {
  return window.mediaPipelineAppHome?.externalDependencyEvidenceText?.(context) || "external dependency evidence not loaded";
}

function renderExternalDependencyDigest(context = {}) {
  return window.mediaPipelineAppHome?.renderExternalDependencyDigest?.(context);
}
...
function renderHomeAtAGlanceQueue(items = []) {
  return window.mediaPipelineAppHome?.renderHomeAtAGlanceQueue?.(items);
}

function renderHomeAtAGlance(context = {}) {
  return window.mediaPipelineAppHome?.renderHomeAtAGlance?.(context);
}

function renderDailyDriverReadiness(context = {}) {
  return window.mediaPipelineAppHome?.renderDailyDriverReadiness?.(context);
}

void [dailyDriverRows, dailyDriverStatusClass, dailyDriverSummaryLines, dependencyStatusLabel, homeAtAGlanceModel, homeAtAGlanceProgressBar, homeAtAGlanceQueueMeta, homeAtAGlanceQueueTitle, homeAtAGlanceStatusState];
void [renderHomeNetworkRole, renderHomeAtAGlanceQueue];
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // V7 standalone Rename workbench wiring: 3-step workflow, confirm + result dialogs,
  // mode-based field visibility, drop zone, modern folder picker via folder_files mode.
  window.mediaPipelineRenameView?.renameInitWorkbenchV7Events?.();
