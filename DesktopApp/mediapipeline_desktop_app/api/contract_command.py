...
LOCAL_API_FILE_COMMAND_ROUTE_CONTRACT: tuple[dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "purpose": "Write per-file processing override state only for absolute paths under configured SourceMovies/SourceTV roots, or clear the override manifest. Route/video fields are validated sparse metadata for later processing consumption. This does not process or mutate media files.",
    },
    {
        "method": "POST",
        ...
        "purpose": "Preview route/remux/transcode impact for one backend-scoped source path using current queue snapshot route evidence. This does not write file_overrides.json, enqueue work, change queue strategy, process media, or mutate queue state.",
    },
    {
        "method": "POST",
        ...
        "purpose": "Preview the bounded impact of a folder-prefix audio/subtitle track override using known queue snapshot rows and cached track metadata only. This does not write file_overrides.json, scan source folders, run probes, enqueue work, change queue strategy, process media, or mutate queue state.",
    },
    {
        "method": "POST",
