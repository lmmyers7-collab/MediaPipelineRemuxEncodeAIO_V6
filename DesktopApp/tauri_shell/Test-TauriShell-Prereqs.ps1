...
Write-Host 'This check is non-mutating and does not replace the V5 fallback workspace.' -ForegroundColor Yellow
if ($CheckOnly) {
...
if ($python) {
    ...
    try {
        $helpOutput = & $python -m mediapipeline_desktop_app.local_api_main --help 2>&1 | Out-String
        if ($LASTEXITCODE -eq 0 -and $helpOutput -match 'Start the MediaPipeline local backend API') {
            ...
            Write-Check -Label 'Local API module import' -Ok $false -Detail 'python -m mediapipeline_desktop_app.local_api_main --help failed'
            $failed = $true
        }
    } finally {
        Pop-Location
    ...
    Write-Check -Label 'Backend Python runtime' -Ok $false -Detail 'DesktopApp\Runtime\Python\python.exe or Pipeline\Runtime\Python\python.exe missing'
    $failed = $true
}
