...
$bundleChecks = @(
    ...
    [pscustomobject]@{ Item = 'Bundled PowerShell'; Path = (Join-Path $bundleRoot 'Pipeline\PowerShell-7.6.0-win-x64\pwsh.exe'); State = Get-PathState -Path (Join-Path $bundleRoot 'Pipeline\PowerShell-7.6.0-win-x64\pwsh.exe'); Expected = 'present-file' },
    [pscustomobject]@{ Item = 'Bundled ffmpeg'; Path = (Join-Path $bundleRoot 'Pipeline\Tools\ffmpeg\bin\ffmpeg.exe'); State = Get-PathState -Path (Join-Path $bundleRoot 'Pipeline\Tools\ffmpeg\bin\ffmpeg.exe'); Expected = 'present-file' },
    [pscustomobject]@{ Item = 'Bundled ffprobe'; Path = (Join-Path $bundleRoot 'Pipeline\Tools\ffmpeg\bin\ffprobe.exe'); State = Get-PathState -Path (Join-Path $bundleRoot 'Pipeline\Tools\ffmpeg\bin\ffprobe.exe'); Expected = 'present-file' },
    [pscustomobject]@{ Item = 'Bundled mkvmerge'; Path = (Join-Path $bundleRoot 'Pipeline\Tools\MKVToolNix\mkvmerge.exe'); State = Get-PathState -Path (Join-Path $bundleRoot 'Pipeline\Tools\MKVToolNix\mkvmerge.exe'); Expected = 'present-file' },
    ...
    [pscustomobject]@{ Item = 'Pipeline tests'; Path = (Join-Path $bundleRoot 'Pipeline\Tests'); State = Get-PathState -Path (Join-Path $bundleRoot 'Pipeline\Tests'); Expected = 'absent' },
