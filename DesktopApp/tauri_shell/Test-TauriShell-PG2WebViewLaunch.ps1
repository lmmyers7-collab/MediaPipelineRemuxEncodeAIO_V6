...
function Wait-ActiveJobForSource {
    param(
        [Parameter(Mandatory)][string]$Directory,
        [Parameter(Mandatory)][string]$Source,
        [datetime]$After,
    ...
    $last = $null
    do {
        $last = Get-LatestActiveJobForSource -Directory $Directory -Source $Source -After $After
        if ($last) {
            $status = [string]$last.Payload.status
            if ($status -in @('completed', 'failed', 'killed', 'orphaned')) {
                return $last
            }
        }
        Start-Sleep -Seconds 2
    } while ((Get-Date) -lt $deadline)
    $detail = if ($last) { $last.Payload | ConvertTo-Json -Depth 8 -Compress } else { 'no matching ActiveJobs record' }
    throw "Timed out waiting for terminal ActiveJobs status for $Source. Last: $detail"
}
...
$stdoutLog = Join-Path $logRoot "tauri_pg2_$stamp.stdout.log"
$stderrLog = Join-Path $logRoot "tauri_pg2_$stamp.stderr.log"
$tokenCapturePath = Join-Path $logRoot "tauri_pg2_$stamp.backend_auth.json"
...
$previousMode = $env:MEDIA_PIPELINE_TAURI_TEST_AUTOLAUNCH_MODE
$previousSchedule = $env:MEDIA_PIPELINE_TAURI_TEST_AUTOLAUNCH_SCHEDULE_OVERRIDE
$previousTokenCapture = $env:MEDIA_PIPELINE_TAURI_TEST_TOKEN_CAPTURE_FILE
$env:MEDIA_PIPELINE_TAURI_TEST_AUTOMATION = 'pg2-webview-launch'
$env:MEDIA_PIPELINE_TAURI_TEST_AUTOLAUNCH_SINGLE_FILE = $resolvedSource
$env:MEDIA_PIPELINE_TAURI_TEST_AUTOLAUNCH_MODE = 'once'
$env:MEDIA_PIPELINE_TAURI_TEST_AUTOLAUNCH_SCHEDULE_OVERRIDE = 'run_once'
...
$launchAttemptStartedAt = Get-Date
...
try {
    ...
    Write-Host "Waiting for WebView-driven single-file pipeline terminal status."
    $activeJob = Wait-ActiveJobForSource -Directory $ActiveJobsDir -Source $resolvedSource -After $launchAttemptStartedAt -TimeoutSeconds $RunTimeoutSeconds
    $activeJobStatus = [string]$activeJob.Payload.status
    if ($activeJobStatus -ne 'completed') {
        throw "WebView-launched ActiveJobs record did not complete. Status=$activeJobStatus Path=$($activeJob.Path)"
    }

    $commands = Invoke-BackendJson -Method GET -Uri "$backendUrl/api/commands?limit=15" -Token $token
    $completed = Invoke-BackendJson -Method GET -Uri "$backendUrl/api/completed?limit=30" -Token $token
    $pending = Invoke-BackendJson -Method GET -Uri "$backendUrl/api/pending-publish" -Token $token
