...
from app.maintenance import change_ledger as change_ledger_module
from app.maintenance.change_ledger import change_ledger_payload, python_impact_for_files
...
from mediapipeline_desktop_app.api.routes_read import GET_ROUTE_HANDLERS
from mediapipeline_desktop_app.api.server import LocalApiServer
...
def _packet(change_id: str, **overrides: Any) -> dict[str, Any]:
    packet: dict[str, Any] = {
        ...
        "tests_added": ["DesktopApp/tests/test_maintenance_change_ledger.py"],
...
class MaintenanceChangeLedgerTests(unittest.TestCase):
    def test_python_impact_groups_touched_python_scripts(self) -> None:
        ...
        self.assertIn("app/maintenance", impact["summary"])
        self.assertIn("DesktopApp/tests", impact["summary"])
    ...
    def test_change_ledger_loads_unreleased_and_released_packets(self) -> None:
        with tempfile.TemporaryDirectory() as raw_root:
            ...
            _write_packet(root, "changes/unreleased/MP-CHANGE-2026-0604-001.json", _packet("MP-CHANGE-2026-0604-001"))
            _write_packet(
                root,
                "changes/released/1.0.0/MP-CHANGE-2026-0601-001.json",
        ...
        self.assertIn("app/maintenance", payload["python_impact"]["summary"])
    ...
    def test_change_ledger_reports_uncovered_worktree_paths_as_review(self) -> None:
        with tempfile.TemporaryDirectory() as raw_root:
            ...
            coverage = change_ledger_module.packet_coverage.CoverageResult(
                ...
                packet_paths=("changes/unreleased/MP-CHANGE-2026-0604-001.json",),
                errors=(),
            )
    ...
    def test_change_ledger_reports_invalid_and_incomplete_packets_as_hygiene(self) -> None:
        with tempfile.TemporaryDirectory() as raw_root:
            root = Path(raw_root)
            bad_json = root / "changes" / "unreleased" / "MP-CHANGE-2026-0604-002.json"
