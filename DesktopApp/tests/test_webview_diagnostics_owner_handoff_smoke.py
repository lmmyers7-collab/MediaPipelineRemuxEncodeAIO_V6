...
class WebViewDiagnosticsOwnerHandoffSmokeTests(unittest.TestCase):
    def test_diagnostics_owner_handoff_summarizes_queue_completed_pending_review_rows(self) -> None:
        static_root = Path(__file__).resolve().parents[1] / "mediapipeline_desktop_app" / "ui_web" / "static" / "assets"
        assets = [
            {
                "name": name,
