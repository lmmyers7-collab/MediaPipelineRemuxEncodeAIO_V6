...
from app.config.recovery import (  # noqa: E402
...
from app.paths.defaults import (  # noqa: E402
...
class ConfigRecoveryTests(unittest.TestCase):
    def _roots(self, tmp: Path) -> tuple[Path, Path]:
        # Mirror the runtime layout: app_root = <root>/DesktopApp,
        # workspace_root = <root>; live config lives in <root>/Pipeline.
        workspace_root = tmp
        app_root = tmp / "DesktopApp"
        (workspace_root / "Pipeline").mkdir(parents=True, exist_ok=True)
    ...
    def _pipeline(self, workspace_root: Path) -> Path:
        return workspace_root / "Pipeline"
