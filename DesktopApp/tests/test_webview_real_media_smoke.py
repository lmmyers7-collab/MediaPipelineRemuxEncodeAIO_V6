...
def _write_fixture_state(root: Path) -> tuple[object, Path, Path]:
    ...
    _write_json(
        pending_root / f"{output.name}.manifest.json",
        {
            ...
            "source_path": str(source),
            "sidecar_files": [{"local_file": str(pending_sidecar)}],
...
class WebViewRealMediaSmokeTests(unittest.TestCase):
    def test_backend_served_webview_fixture_has_real_media_validation_evidence(self) -> None:
        with tempfile.TemporaryDirectory() as raw_root:
            ...
            try:
                ...
                launch_view_status, launch_view_js, launch_view_type = _get_text(f"{server.url}/assets/launchView.js")
