...
from mediapipeline_desktop_app.network.coordinator_policy import compute_retry_after_seconds
from mediapipeline_desktop_app.network.protocol import ClaimResponse, DoneRequest, LogEntryRequest
from mediapipeline_desktop_app.network.registry import InFlightRegistry
from mediapipeline_desktop_app.network.poll_policy import resolve_worker_wait_seconds
from mediapipeline_desktop_app.network.worker import WorkerDispatcher
...
class WorkflowEnhancementTests(unittest.TestCase):
    ...
    def test_terminal_done_queue_removal_success_logs_scheduled_not_removed(self) -> None:
        ...
        with self.assertLogs("mediapipeline_desktop_app.network.coordinator", level="INFO") as logs:
            CoordinatorDispatcher._emit_done_outcome(
                dispatcher,
                job=job,
                success=False,
    ...
    def test_coordinator_inflight_save_failures_emit_cluster_diagnostics(self) -> None:
        ...
        with (
            ...
            self.assertLogs("mediapipeline_desktop_app.network.coordinator", level="ERROR") as claim_logs,
        ):
            CoordinatorDispatcher._http_claim(
                claim_dispatcher,
        ...
        with (
            ...
            self.assertLogs("mediapipeline_desktop_app.network.coordinator", level="WARNING") as release_logs,
        ):
        ...
        with (
            ...
            self.assertLogs("mediapipeline_desktop_app.network.coordinator", level="ERROR") as done_logs,
        ):
    ...
    def test_workers_response_carries_coordinator_now(self) -> None:
        from mediapipeline_desktop_app.network.protocol import WorkersResponse
    ...
    def test_claim_response_retry_after_seconds_round_trips(self) -> None:
        from mediapipeline_desktop_app.network.protocol import ClaimResponse
    ...
    def test_compute_retry_after_seconds_reflects_registry_activity(self) -> None:
        from mediapipeline_desktop_app.network.coordinator import (
            _RETRY_AFTER_ACTIVE_SECONDS, _RETRY_AFTER_IDLE_SECONDS,
        )
        ...
        self.assertEqual(
            ...
            _RETRY_AFTER_ACTIVE_SECONDS,
        )
        # Active hint must be strictly shorter than idle hint.
        self.assertLess(_RETRY_AFTER_ACTIVE_SECONDS, _RETRY_AFTER_IDLE_SECONDS)

    def test_compute_retry_after_seconds_logs_registry_count_failure(self) -> None:
        from mediapipeline_desktop_app.network.coordinator import _RETRY_AFTER_IDLE_SECONDS
        ...
        self.assertEqual(retry_after, _RETRY_AFTER_IDLE_SECONDS)
        self.assertIn(
