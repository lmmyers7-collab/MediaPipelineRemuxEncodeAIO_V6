...
from app.config.identity import (  # noqa: E402
...
class ConfigIdentityTests(unittest.TestCase):
    ...
    def test_identity_blocks_missing_template_and_underloaded_configs(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            template = root / "Pipeline" / "MediaPipeline_config_template.psd1"
            ...
            missing = build_config_identity(root / "missing.psd1", {}, app_root=root / "DesktopApp", workspace_root=root)
            low = build_config_identity(active, {"SourceMovies": r"C:\Movies"}, app_root=root / "DesktopApp", workspace_root=root)
