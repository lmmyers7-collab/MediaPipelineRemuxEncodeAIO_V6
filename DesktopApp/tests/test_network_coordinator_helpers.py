...
class NetworkCoordinatorHelperTests(unittest.TestCase):
    ...
    def test_network_probe_helpers_check_health_and_worker_auth(self) -> None:
        ...
        with patch("mediapipeline_desktop_app.network.probe.urllib.request.urlopen", side_effect=fake_urlopen):
    ...
    def test_coordinator_health_probe_normalizes_failure_details(self) -> None:
        with patch(
            "mediapipeline_desktop_app.network.probe.urllib.request.urlopen",
        ...
        with patch("mediapipeline_desktop_app.network.probe.urllib.request.urlopen", side_effect=http_error):
    ...
    def test_worker_auth_probe_normalizes_failure_details(self) -> None:
        ...
        with patch("mediapipeline_desktop_app.network.probe.urllib.request.urlopen", side_effect=unauthorized_error):
        ...
        with patch("mediapipeline_desktop_app.network.probe.urllib.request.urlopen", side_effect=forbidden_error):
    ...
    def test_coordinator_policy_helpers_coerce_config_and_retry_hints(self) -> None:
        ...
        with self.assertLogs("mediapipeline_desktop_app.network.coordinator_policy", level="WARNING") as logs:
    ...
    def test_prior_failure_policy_logs_uninspectable_records(self) -> None:
        ...
        with self.assertLogs("mediapipeline_desktop_app.network.failure_policy", level="WARNING") as logs:
