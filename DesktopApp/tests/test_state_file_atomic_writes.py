...
from app.ui_preferences import read_ui_preferences, write_ui_preferences
...
class StateFileAtomicWriteTests(unittest.TestCase):
    def test_ui_preferences_write_retries_transient_replace_permission_error(self) -> None:
        with tempfile.TemporaryDirectory() as raw_root:
            ...
            with patch("app.ui_preferences.os.replace", flaky_replace), patch("app.ui_preferences.time.sleep", lambda _delay: None):
    ...
    def test_command_journal_write_retries_transient_replace_permission_error(self) -> None:
        with tempfile.TemporaryDirectory() as raw_root:
            ...
            with (
                patch("mediapipeline_desktop_app.api.command_journal.os.replace", flaky_replace),
                patch("mediapipeline_desktop_app.api.command_journal.time.sleep", lambda _delay: None),
            ):
                journal.record(
                    {
