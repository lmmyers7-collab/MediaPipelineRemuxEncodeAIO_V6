...
from mediapipeline_desktop_app.network.coordinator import CoordinatorDispatcher
from mediapipeline_desktop_app.network.encode_config_snapshot import snapshot_encode_config
from mediapipeline_desktop_app.network.path_map import parse_source_path_map
from mediapipeline_desktop_app.network.protocol import DoneRequest
...
class NetworkInFlightRegistryTests(unittest.TestCase):
    ...
    def test_inflight_registry_logs_temp_cleanup_failure_after_save_failure(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            with (
                patch("mediapipeline_desktop_app.network.registry.os.replace", side_effect=OSError("replace denied")),
                patch("pathlib.Path.unlink", side_effect=OSError("cleanup denied")),
                self.assertLogs("mediapipeline_desktop_app.network.registry", level="WARNING") as logs,
            ):
    ...
    def test_inflight_registry_save_failure_propagates_to_coordinator_state_transition_diagnostics(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            with (
                ...
                self.assertLogs("mediapipeline_desktop_app.network.registry", level="ERROR") as registry_logs,
            ):
    ...
    def test_network_config_json_fields_reject_nonfinite_constants(self) -> None:
        with self.assertLogs("mediapipeline_desktop_app.network.path_map", level="WARNING") as path_logs:
        ...
        with self.assertLogs("mediapipeline_desktop_app.network.encode_config_snapshot", level="WARNING") as encode_logs:
