...
class NetworkWorkerRuntimeTests(unittest.TestCase):
    ...
    def test_start_daemon_thread_logs_start_failure_and_runs_handler(self) -> None:
        ...
        logger = logging.getLogger("mediapipeline_desktop_app.network.threading_helpers")

        with (
            patch("mediapipeline_desktop_app.network.threading_helpers.threading.Thread", BadThread),
            self.assertLogs("mediapipeline_desktop_app.network.threading_helpers", level="WARNING") as logs,
        ):
    ...
    def test_worker_source_path_map_logs_unparseable_entries(self) -> None:
        ...
        with (
            patch("mediapipeline_desktop_app.network.path_map.loads_strict_json", return_value=payload),
            self.assertLogs("mediapipeline_desktop_app.network.path_map", level="WARNING") as logs,
        ):
            mappings = parse_source_path_map("{}")
    ...
    def test_worker_status_callback_failure_is_logged(self) -> None:
        ...
        with self.assertLogs("mediapipeline_desktop_app.network.worker", level="DEBUG") as logs:
    ...
    def test_worker_cluster_log_thread_start_failure_is_logged(self) -> None:
        ...
        with (
            patch("mediapipeline_desktop_app.network.worker.threading.Thread", BadThread),
    ...
    def test_worker_cluster_log_post_failure_warns_with_event_context(self) -> None:
        ...
        with (
            patch("mediapipeline_desktop_app.network.worker.threading.Thread", InlineThread),
            self.assertLogs("mediapipeline_desktop_app.network.worker", level="WARNING") as logs,
        ):
            worker.log_cluster_event(level="INFO", event="job_started", message="movie.mkv")
    ...
    def test_worker_poll_does_not_treat_error_body_401_text_as_auth_failure(self) -> None:
        ...
        with self.assertLogs("mediapipeline_desktop_app.network.worker", level="WARNING"):
            WorkerDispatcher._poll_loop(worker)
    ...
    def test_worker_releases_claim_when_heartbeat_thread_start_fails(self) -> None:
        ...
        with (
            patch("mediapipeline_desktop_app.network.worker.threading.Thread", BadHeartbeatThread),
            self.assertLogs("mediapipeline_desktop_app.network.worker", level="ERROR") as logs,
        ):
    ...
    def test_worker_claimed_job_schedule_failure_diagnostic_is_bounded(self) -> None:
        ...
        with (
            patch("mediapipeline_desktop_app.network.worker.threading.Thread", StartedHeartbeatThread),
            self.assertLogs("mediapipeline_desktop_app.network.worker", level="ERROR") as logs,
        ):
            WorkerDispatcher._on_job_claimed(worker, job)
