...
from mediapipeline_desktop_app.network.coordinator import CoordinatorDispatcher, _CoordServer
...
class NetworkCoordinatorStartupTests(unittest.TestCase):
    ...
    def test_coordinator_http_bind_failure_raises_instead_of_marking_started(self) -> None:
        ...
        with patch(
            "mediapipeline_desktop_app.network.coordinator._CoordServer",
    ...
    def test_coordinator_http_thread_start_failure_closes_server(self) -> None:
        ...
        with (
            patch("mediapipeline_desktop_app.network.coordinator._CoordServer", return_value=server),
            patch("mediapipeline_desktop_app.network.coordinator.threading.Thread", BadThread),
    ...
    def test_coordinator_mdns_start_failure_logs_manual_url_fallback(self) -> None:
        ...
        with (
            patch(
                "mediapipeline_desktop_app.network.mdns.CoordinatorAdvertiser",
    ...
    def test_coordinator_mdns_registration_failure_does_not_mark_advertiser_active(self) -> None:
        ...
        with (
            patch("mediapipeline_desktop_app.network.mdns.CoordinatorAdvertiser", RegistrationFailedAdvertiser),
            self.assertLogs("mediapipeline_desktop_app.network.coordinator", level="WARNING") as logs,
        ):
            CoordinatorDispatcher._start_mdns(dispatcher)
